...
static int slim_get_port_idx(struct snd_kcontrol *kcontrol)
{
	...
	if (strnstr(kcontrol->id.name, "SLIM_0_RX", sizeof("SLIM_0_RX"))) {
		port_id = SLIM_RX_0;
	} else if (strnstr(kcontrol->id.name, "SLIM_2_RX",
			   sizeof("SLIM_2_RX"))) {
		port_id = SLIM_RX_2;
	} else if (strnstr(kcontrol->id.name, "SLIM_5_RX",
			   sizeof("SLIM_5_RX"))) {
		port_id = SLIM_RX_5;
	} else if (strnstr(kcontrol->id.name, "SLIM_6_RX",
			   sizeof("SLIM_6_RX"))) {
		port_id = SLIM_RX_6;
	} else if (strnstr(kcontrol->id.name, "SLIM_0_TX",
			   sizeof("SLIM_0_TX"))) {
		port_id = SLIM_TX_0;
	} else if (strnstr(kcontrol->id.name, "SLIM_1_TX",
			   sizeof("SLIM_1_TX"))) {
		port_id = SLIM_TX_1;
	} else {
...
static int ext_disp_get_port_idx(struct snd_kcontrol *kcontrol)
{
	...
	if (strnstr(kcontrol->id.name, "HDMI_RX", sizeof("HDMI_RX")))
		...
		pr_err("%s: unsupported BE: %s", __func__, kcontrol->id.name);
		idx = -EINVAL;
	}
			      ...
			      struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] = ext_disp_rx_cfg[idx].channels - 2;
			      ...
			      struct snd_ctl_elem_value *ucontrol)
{
	...
	ext_disp_rx_cfg[idx].channels = ucontrol->value.integer.value[0] + 2;
				  ...
				  struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ret) {
		...
		ucontrol->value.enumerated.item[0] =
		    tdm_get_sample_rate_val(tdm_rx_cfg[port.mode][port.channel].
					    sample_rate);
				  ...
				  struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ret) {
		...
		ucontrol->value.enumerated.item[0] =
		    tdm_get_sample_rate_val(tdm_tx_cfg[port.mode][port.channel].
					    sample_rate);
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ret) {
		...
		ucontrol->value.enumerated.item[0] =
		    tdm_get_format_val(tdm_rx_cfg[port.mode][port.channel].
				       bit_format);
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ret) {
		...
		ucontrol->value.enumerated.item[0] =
		    tdm_get_format_val(tdm_tx_cfg[port.mode][port.channel].
				       bit_format);
...
static int aux_pcm_get_port_idx(struct snd_kcontrol *kcontrol)
{
	...
	if (strnstr(kcontrol->id.name, "PRIM_AUX_PCM", sizeof("PRIM_AUX_PCM")))
		idx = PRIM_AUX_PCM;
		...
		pr_err("%s: unsupported port: %s", __func__, kcontrol->id.name);
		idx = -EINVAL;
	}
				  ...
				  struct snd_pcm_hw_params *params)
{
	...
	case MSM_BACKEND_DAI_SLIMBUS_5_TX:
		...
		config =
			msm_codec_fn.get_afe_config_fn(
					codec, AFE_SLIMBUS_SLAVE_PORT_CONFIG);
		if (config) {
			rc = afe_set_config(AFE_SLIMBUS_SLAVE_PORT_CONFIG,
					    config, SLIMBUS_5_TX);
			if (rc)
				pr_err
				    ("%s: Failed to set slimbus slave port config %d\n",
				     __func__, rc);
		}
		break;
	...
	case MSM_BACKEND_DAI_SLIMBUS_7_RX:
		...
		channels->min = channels->max = slim_rx_cfg[SLIM_RX_7].channels;
		break;

	case MSM_BACKEND_DAI_SLIMBUS_7_TX:
		rate->min = rate->max = slim_tx_cfg[SLIM_TX_7].sample_rate;
		channels->min = channels->max = slim_tx_cfg[SLIM_TX_7].channels;
		break;

	case MSM_BACKEND_DAI_SLIMBUS_8_TX:
		rate->min = rate->max = slim_tx_cfg[SLIM_TX_8].sample_rate;
		channels->min = channels->max = slim_tx_cfg[SLIM_TX_8].channels;
		break;
	...
	case MSM_BACKEND_DAI_SEC_AUXPCM_RX:
		rate->min = rate->max = aux_pcm_rx_cfg[SEC_AUX_PCM].sample_rate;
		channels->min = channels->max =
		    aux_pcm_rx_cfg[SEC_AUX_PCM].channels;
		break;

	case MSM_BACKEND_DAI_SEC_AUXPCM_TX:
		rate->min = rate->max = aux_pcm_tx_cfg[SEC_AUX_PCM].sample_rate;
		channels->min = channels->max =
	...
	case MSM_BACKEND_DAI_PRI_MI2S_RX:
		...
		channels->min = channels->max = mi2s_rx_cfg[PRIM_MI2S].channels;
		break;
	...
	case MSM_BACKEND_DAI_PRI_MI2S_TX:
		...
		channels->min = channels->max = mi2s_tx_cfg[PRIM_MI2S].channels;
		break;
	...
	case MSM_BACKEND_DAI_SECONDARY_MI2S_RX:
		...
		channels->min = channels->max = mi2s_rx_cfg[SEC_MI2S].channels;
		break;
	...
	case MSM_BACKEND_DAI_SECONDARY_MI2S_TX:
		...
		channels->min = channels->max = mi2s_tx_cfg[SEC_MI2S].channels;
		break;
	...
	case MSM_BACKEND_DAI_TERTIARY_MI2S_RX:
		...
		channels->min = channels->max = mi2s_rx_cfg[TERT_MI2S].channels;
		break;
	...
	case MSM_BACKEND_DAI_TERTIARY_MI2S_TX:
		...
		channels->min = channels->max = mi2s_tx_cfg[TERT_MI2S].channels;
		break;
	...
	case MSM_BACKEND_DAI_QUATERNARY_MI2S_RX:
		...
		channels->min = channels->max = mi2s_rx_cfg[QUAT_MI2S].channels;
		break;
	...
	case MSM_BACKEND_DAI_QUATERNARY_MI2S_TX:
		...
		channels->min = channels->max = mi2s_tx_cfg[QUAT_MI2S].channels;
		break;
...
static bool msm_swap_gnd_mic(struct snd_soc_codec *codec)
{
	...
	if (pdata->us_euro_gpio_p) {
		...
		if (value)
			msm_cdc_pinctrl_select_sleep_state(pdata->
							   us_euro_gpio_p);
		else
			msm_cdc_pinctrl_select_active_state(pdata->
							    us_euro_gpio_p);
...
static int msm_afe_set_config(struct snd_soc_codec *codec)
{
	...
	config_data =
		msm_codec_fn.get_afe_config_fn(codec,
...
static int msm_audrx_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	config_data = msm_codec_fn.get_afe_config_fn(codec, AFE_AANC_VERSION);
	if (config_data) {
	...
	if (!strcmp(dev_name(codec_dai->dev), "tasha_codec")) {
		config_data =
			msm_codec_fn.get_afe_config_fn(
					codec, AFE_CDC_CLIP_REGISTERS_CONFIG);
		if (config_data) {
		...
		if (config_data) {
			...
			if (ret) {
				pr_err
				    ("%s: Failed to set AFE bank selection %d\n",
				     __func__, ret);
				goto err_afe_cfg;
			}
		}
				 ...
				 struct snd_pcm_hw_params *params)
{
	...
	ret = snd_soc_dai_set_channel_map(cpu_dai, user_set_tx_ch, tx_ch, 0, 0);
	if (ret < 0)
		pr_err("%s: failed to set cpu chan map, err:%d\n",
		       __func__, ret);
err_ch_map:
...
static int msm_aux_pcm_snd_startup(struct snd_pcm_substream *substream)
{
	...
	if (++auxpcm_intf_conf[index].ref_cnt == 1) {
		if (mi2s_auxpcm_conf[index].pcm_i2s_sel_vt_addr) {
...
static int msm_mi2s_set_sclk(struct snd_pcm_substream *substream, bool enable)
{
	...
	ret = afe_set_lpass_clock_v2(port_id, &mi2s_clk[index]);
	if (ret < 0) {
		dev_err(rtd->card->dev,
...
msm_tasha_dai_links[ARRAY_SIZE(msm_common_dai_links) +
...
static struct snd_soc_dai_link
msm_tavil_dai_links[ARRAY_SIZE(msm_common_dai_links) +
...
static int msm_populate_dai_link_component_of_node(struct snd_soc_card *card)
{
	int i, index, ret = 0;
	struct device *cdev = card->dev;
	struct snd_soc_dai_link *dai_link = card->dai_link;
	...
	for (i = 0; i < card->num_links; i++) {
		...
		if (dai_link[i].platform_name && !dai_link[i].platform_of_node) {
...
static struct snd_soc_dai_link
    msm_stub_dai_links[ARRAY_SIZE(msm_stub_fe_dai_links) +
...
static struct snd_soc_card *populate_snd_card_dailinks(struct device *dev)
{
	...
	if (!strcmp(match->data, "tasha_codec")) {
		...
		memcpy(msm_tasha_dai_links + len_1,
		       msm_tasha_fe_dai_links, sizeof(msm_tasha_fe_dai_links));
		...
		memcpy(msm_tasha_dai_links + len_4,
		       msm_tasha_be_dai_links, sizeof(msm_tasha_be_dai_links));
		...
		if (of_property_read_bool(dev->of_node,
			...
			dev_dbg(dev,
				"%s(): External display audio support present\n",
				__func__);
			memcpy(msm_tasha_dai_links + total_links,
		...
		memcpy(msm_tavil_dai_links + len_1,
		       msm_tavil_fe_dai_links, sizeof(msm_tavil_fe_dai_links));
		...
		memcpy(msm_tavil_dai_links + len_4,
		       msm_tavil_be_dai_links, sizeof(msm_tavil_be_dai_links));
		...
		memcpy(msm_stub_dai_links,
		       msm_stub_fe_dai_links, sizeof(msm_stub_fe_dai_links));
		memcpy(msm_stub_dai_links + len_1,
		       msm_stub_be_dai_links, sizeof(msm_stub_be_dai_links));
...
static void i2s_auxpcm_deinit(void)
{
	...
	for (count = 0; count < PCM_I2S_SEL_MAX; count++)
		if (mi2s_auxpcm_conf[count].pcm_i2s_sel_vt_addr)
			iounmap(mi2s_auxpcm_conf[count].pcm_i2s_sel_vt_addr);
}
...
static int msm_asoc_machine_probe(struct platform_device *pdev)
{
	...
	match = of_match_node(msm8998_asoc_machine_of_match, pdev->dev.of_node);
	if (!match) {
	...
	if (ret) {
		dev_err(&pdev->dev,
			...
			mclk_freq_prop_name, pdev->dev.of_node->full_name, ret);
		goto err;
	}
	...
	if (!gpio_is_valid(pdata->us_euro_gpio) && (!pdata->us_euro_gpio_p)) {
		...
		dev_dbg(&pdev->dev, "%s detected", "qcom,us-euro-gpios");
