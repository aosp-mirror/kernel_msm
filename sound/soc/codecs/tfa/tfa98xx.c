...
static int tfa98xx_profile; /* HTC_AUDIO: bypass profile *//* store profile */
...
static int tfa98xx_mixer_profile; /* HTC_AUDIO: bypass profile */
...
static struct tfa98xx_rate rate_to_fssel[] = {
	{8000, 0}, {11025, 1},
	{12000, 2}, {16000, 3}, {22050, 4},
	{24000, 5}, {32000, 6}, {44100, 7},
	{48000, 8},
};
...
err_free_dev: input_free_device(input);
	return err;
}
 ...
 * If already registered, check if
 * still relevant and remove it if necessary.
		...
		bool unregister)
{
	...
	for (i = 0; i < tfaContMaxProfile(tfa98xx->handle); i++) {
		if (strnstr(tfaContProfileName(tfa98xx->handle, i),
			".tap",
			...
			dev_info(tfa98xx->codec->dev,
				...
				tfaContProfileName(tfa98xx->handle, i));
		}
	}
...
void tfa98xx_deferred_calibration_status(
		Tfa98xx_handle_t handle, int calibrateDone)
{
		...
		loff_t *ppos)
{
	...
	n += snprintf(&str[n], sizeof(str)-1-n,
			"Wait4Src\t= %d\n",
...
DEFINE_SIMPLE_ATTRIBUTE(tfa98xx_dbgfs_reg_##__reg##_fops, \
		...
		tfa98xx_dbgfs_reg_##__reg##_set, \
		"0x%llx\n")

#define VAL(str) #str
#define TOSTRING(str) VAL(str)
#define TFA98XX_DEBUGFS_REG_CREATE_FILE(__reg, __name) \
	debugfs_create_file(TOSTRING(__reg) \
	"-" TOSTRING(__name), S_IRUGO|S_IWUGO, dbg_reg_dir, \
...
static void tfa98xx_debug_init(struct tfa98xx *tfa98xx, struct i2c_client *i2c)
{
	...
	debugfs_create_file("OTC", S_IRUSR|S_IRGRP|S_IWUSR|S_IWGRP,
			tfa98xx->dbg_dir,
			i2c, &tfa98xx_dbgfs_calib_otc_fops);
	debugfs_create_file("MTPEX", S_IRUSR|S_IRGRP|S_IWUSR|S_IWGRP,
			tfa98xx->dbg_dir,
			i2c, &tfa98xx_dbgfs_calib_mtpex_fops);
	debugfs_create_file("TEMP", S_IRUSR|S_IRGRP|S_IWUSR|S_IWGRP,
			tfa98xx->dbg_dir,
			i2c, &tfa98xx_dbgfs_calib_temp_fops);
	debugfs_create_file("calibrate", S_IRUSR|S_IRGRP|S_IWUSR|S_IWGRP,
			tfa98xx->dbg_dir,
			i2c, &tfa98xx_dbgfs_calib_start_fops);
	debugfs_create_file("R", S_IRUSR|S_IRGRP,
			tfa98xx->dbg_dir,
			i2c, &tfa98xx_dbgfs_r_fops);
	debugfs_create_file("version", S_IRUSR|S_IRGRP,
			tfa98xx->dbg_dir,
			i2c, &tfa98xx_dbgfs_version_fops);
	debugfs_create_file("dsp-state", S_IRUSR|S_IRGRP|S_IWUSR|S_IWGRP,
			tfa98xx->dbg_dir,
			i2c, &tfa98xx_dbgfs_dsp_state_fops);
	debugfs_create_file("accounting", S_IRUSR|S_IRGRP,
			tfa98xx->dbg_dir,
...
/* HTC_AUD_START */
static int tfa98xx_get_manstate(struct snd_kcontrol *kcontrol,
...
static int tfa98xx_set_manstate(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int tfa98xx_info_manstate(struct snd_kcontrol *kcontrol,
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] =
			tfacont_get_max_vstep(0, profile) - vstep - 1;
	return 0;
}
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	/* int ready = 0; */
	/* unsigned int base_addr_inten =
	 * TFA_FAM(tfa98xx->handle,INTENVDDS) >> 8;
	 */
	/* HTC_AUD_END */
	...
	if ((vstep == value) || (value < 0) || (vsteps < (value + 1)))
		return 0;
	...
	if (profile == tfa98xx_profile) {
		...
		/* HTC_AUD_START -use queue rather
		 * could tfa98xx_tfa_start directly
		 */
		if (tfa98xx->dsp_init != TFA98XX_DSP_INIT_STOPPED) {
			cancel_delayed_work_sync(&tfa98xx->init_work);
			pr_info("switch to tfa vstep %d profile %d\n",
					tfa98xx_vsteps[0],
					profile);
			queue_delayed_work(tfa98xx->tfa98xx_wq,
					&tfa98xx->init_work, 0);
		...
		int profile)
{
	...
	list_for_each_entry(bprof, &profile_list, list) {
		if (strncmp(bprof->basename, basename, len) == 0) {
			...
			/* enter the (container)profile for
			 * this samplerate at the corresponding index
			 */
...
static int tfa98xx_create_controls(struct tfa98xx *tfa98xx)
{
	...
	for (prof = 0; prof < nprof; prof++) {
		...
		get_profile_basename(bprofile->basename,
				tfaContProfileName(0, prof));
		bprofile->len = strlen(bprofile->basename);

		/*
		 * search the profile list for a
		 * profile with basename, if it is not found then
		 * add it to the list and add a
		 * new mixer control (if it has vsteps)
		 * also, if it is a calibration
		 * profile, do not add it to the list
		 */
		if (is_profile_in_list(bprofile->basename, bprofile->len) == 0
			&& is_calibration_profile(tfaContProfileName(0,
			prof)) == 0) {
			...
			if (tfacont_get_max_vstep(0, prof)) {
				...
				tfa98xx_controls[mix_index].name =
						name;
		...
		/* look for the basename profile in the
		 * list of mixer profiles and add the
		 * container profile index to the supported
		 * samplerates of this mixer profile
		 */
		add_sr_to_profile(tfa98xx,
				bprofile->basename, bprofile->len, prof);
	}
	...
	/* HTC_AUD_START */
	tfa98xx_controls[mix_index].name = "NXP MANSTATE";
	tfa98xx_controls[mix_index].iface = SNDRV_CTL_ELEM_IFACE_MIXER;
	tfa98xx_controls[mix_index].info = tfa98xx_info_manstate;
	tfa98xx_controls[mix_index].get = tfa98xx_get_manstate;
	tfa98xx_controls[mix_index].put = tfa98xx_set_manstate;
	mix_index++;
	/* HTC_AUD_END */
...
static int tfa98xx_append_i2c_address(struct device *dev,
		struct i2c_client *i2c, struct snd_soc_dapm_widget *widgets,
		int num_widgets,
		struct snd_soc_dai_driver *dai_drv, int num_dai)
{
	 ...
	 * "AIF Playback", 0, SND_SOC_NOPM, 0, 0),
	 * into:
	 * SND_SOC_DAPM_AIF_IN("AIF IN",
...
static const struct snd_soc_dapm_route tfa98xx_dapm_routes_common[] = {
	{"OUTL", NULL, "AIF IN"}, {"AIF OUT", NULL, "AEC Loopback"},
};
...
static const struct snd_soc_dapm_route tfa9888_input_dapm_routes[] = {
	{"AIF OUT", NULL, "DMIC1"}, {"AIF OUT", NULL, "DMIC2"},
	{"AIF OUT", NULL, "DMIC3"}, {"AIF OUT", NULL, "DMIC4"},
};
 ...
 * The check in tfa98xx_register_dsp() is
 * implicitly done in tfa_probe() /tfa98xx_cnt_slave2idx(_)
 */
static int tfa98xx_register_dsp(struct tfa98xx *tfa98xx)
{
	...
	if (handle != -1) {
		...
		dev_info(&tfa98xx->i2c->dev, "Registered DSP instance with handle %d\n",
				handle);
		tfa98xx_registered_handles++;
		return handle;
	}
reg_err: dev_err(&tfa98xx->i2c->dev,
...
enum Tfa98xx_Error tfa98xx_read_data(Tfa98xx_handle_t handle, unsigned char reg,
		int len, unsigned char value[])
{
	...
	if (tfa98xx_devices[handle] && tfa98xx_devices[handle]->i2c) {
		...
		do {
			err = i2c_transfer(tfa98xx_client->adapter,
...
enum Tfa98xx_Error tfa98xx_write_raw(Tfa98xx_handle_t handle, int len,
		...
		unsigned int mask)
{
	...
	unsigned int errs = TFA98XX_STATUSREG_WDS | TFA98XX_STATUSREG_SPKS;
...
static void tfa98xx_tapdet_check_update(struct tfa98xx *tfa98xx)
{
	...
	if (!gpio_is_valid(tfa98xx->irq_gpio)) {
		...
		dev_dbg(tfa98xx->codec->dev,
				...
				tfa98xx->tapdet_open,
				tfa98xx->tapdet_profiles, tfa98xx_profile);
		...
		handles_local[tfa98xx->handle].interrupt_status[0] |= enable
				<< TFA98XX_STATUSREG_SPKS_POS;

		/* update interrupt_reg to match enabled interrupts */
		handles_local[tfa98xx->handle].interrupt_enable[0] &=
				~TFA98XX_INTERRUPT_REG_SPKD;
		handles_local[tfa98xx->handle].interrupt_enable[0] |= spkerr
				<< TFA98XX_INTERRUPT_REG_SPKD_POS;
	}

	/* check disabled => enabled transition to clear pending events */
	if (!tfa98xx->tapdet_enabled && enable) {
		/* clear pending event if any */
		err = tfa98xx_dsp_write_mem_word(tfa98xx->handle,
				XMEM_TAP_ACK, 0,
				Tfa98xx_DMEM_XMEM);
		if (err)
...
static void __tfa98xx_interrupt_enable(struct tfa98xx *tfa98xx, bool enable)
{
	...
	handles_local[tfa98xx->handle].interrupt_enable[0] |= enable
			<< TFA98XX_INTERRUPT_REG_INT_POS;
...
static void tfa98xx_monitor(struct work_struct *work)
{
	...
	if (!tfa98xx->input) {
		...
		if ((TFA98XX_STATUSREG_ACS & val)
			...
			if ((val & TFA98XX_STATUSREG_UP_MASK) ==
					TFA98XX_STATUSREG_UP_MASK)
				queue_delayed_work(tfa98xx->tfa98xx_wq,
						&tfa98xx->init_work, 0);
		}
...
static void tfa98xx_dsp_init(struct tfa98xx *tfa98xx)
{
	...
	int manstate = 0;
	int *vsteps = tfa98xx_vsteps;

	do_gettimeofday(&begin);
	if (tfa98xx->dsp_init == TFA98XX_DSP_INIT_INVALIDATED)
		from_mixer = true;
	/* HTC_AUD_END */
	...
	if (tfa98xx->init_count < TF98XX_MAX_DSP_START_TRY_COUNT) {
		...
		ret = tfa98xx_tfa_start(tfa98xx,
				tfa98xx_profile, vsteps);

		if (ret != Tfa98xx_Error_Ok) {
			/* It may fail as we may not
			 * have a valid clock at that
			 * time, so re-schedule and re-try later.
			 */
			manstate =
				TFA_GET_BF(tfa98xx->handle, MANSTATE);
			dev_err(&tfa98xx->i2c->dev,
				"tfa_start failed! (err %d) - %d manstate:%d\n",
				ret,
			...
			do_gettimeofday(&end);
			diff = 1000 * (end.tv_sec - begin.tv_sec)
			+ ((end.tv_usec - begin.tv_usec) / 1000);
			pr_info("tfa_start profile %d, vstep %d success with %ld ms\n",
				tfa98xx_profile, vsteps[0], diff);
			/* HTC_AUD_END */
			/* Subsystem ready, tfa init complete */
			dev_dbg(&tfa98xx->i2c->dev, "tfa_start success (%d)\n",
				tfa98xx->init_count);
		...
		/* exceeded max number ot
		 * start tentatives, cancel start
		 */
...
{
	struct tfa98xx
	*tfa98xx = container_of(work, struct tfa98xx, init_work.work);
...
{
	struct tfa98xx
	*tfa98xx = container_of(work, struct tfa98xx, interrupt_work.work);
	unsigned int base_addr_inten = TFA_FAM(tfa98xx->handle, INTENVDDS) >> 8;
	unsigned int base_addr_ist = TFA_FAM(tfa98xx->handle, ISTVDDS) >> 8;
	unsigned int base_addr_icl = TFA_FAM(tfa98xx->handle, ICLVDDS) >> 8;
	/* unsigned int base_addr_ipo =
	 * TFA_FAM(tfa98xx->handle,IPOVDDS) >> 8;
	 */
	u32 out1, out2, out3;
	...
	if (out2) {
		...
		if (TFA_GET_BF_VALUE(tfa98xx->handle, ISTMWSRC, out2) > 0) {
			...
			if (manwait1 > 0) {
				...
				/* set AMPC and AMPE to make
				 * sure the amp is enabled
				 */
				pr_info("setting AMPC and AMPE to 1 (default)\n");
				TFA_SET_BF(tfa98xx->handle, AMPC, 1);
				TFA_SET_BF(tfa98xx->handle, AMPE, 1);

				/* set MANSCONF here, the manager
				 * will continue if clock is there
				 */
				TFA_SET_BF(tfa98xx->handle, MANSCONF, 1);
			} else {
				/* Now we can switch profile with internal
				 * clock it is not required to call tfa_start
				 */
		...
		struct snd_soc_dai *dai)
{
	...
	char *basename = devm_kzalloc(tfa98xx->codec->dev, MAX_CONTROL_NAME,
			GFP_KERNEL);
	...
	get_profile_basename(basename,
			tfaContProfileName(tfa98xx->handle, tfa98xx_profile));
	len = strlen(basename);
	...
	for (prof = 0; prof < nprof; prof++) {
		if (strncmp(basename,
				...
				len) == 0) {
			/* Check which sample rate is
			 * supported with current profile,
			 * and enforce this.
			 */
...
/* HTC_AUD_START - add start on prepare
 * function and stop on shutdown function
 */
...
static const struct snd_soc_dai_ops tfa98xx_dai_ops = {
	...
	/* HTC_AUDIO: add shutdown function for stop tfa */
	.prepare = tfa98xx_prepare,
	/* HTC_AUDIO: add prepare for
	 * trigger tfa up with correct profile
	 */
	.set_fmt = tfa98xx_set_fmt,
	.set_sysclk = tfa98xx_set_dai_sysclk,
	.hw_params = tfa98xx_hw_params,
	/*.mute_stream = tfa98xx_mute, */
	/* HTC_AUDIO:No use mute_stream function */
};
...
static void tfa98xx_irq_9888(struct tfa98xx *tfa98xx)
{
	unsigned int base_addr_inten = TFA_FAM(tfa98xx->handle, INTENVDDS) >> 8;
	unsigned int base_addr_ist = TFA_FAM(tfa98xx->handle, ISTVDDS) >> 8;
	/* unsigned int base_addr_icl
	 * = TFA_FAM(tfa98xx->handle,ICLVDDS) >> 8;
	 */
	/* unsigned int base_addr_ipo
	 * = TFA_FAM(tfa98xx->handle,IPOVDDS) >> 8;
	 */
...
static void __tfa98xx_irq(struct tfa98xx *tfa98xx)
{
	...
	dev_info(&tfa98xx->i2c->dev,
			"interrupt: 0x%04x (enabled: 0x%04x)\n", val,
			ie);
#ifdef DEBUG
	if (!(val & ie)) {
		unsigned int ireg;
...
/* HTC_AUD_START */
	if (of_property_read_bool(np, "upper-spk"))
		tfa98xx->upper_spk = true;
	else
		tfa98xx->upper_spk = false;
/* HTC_AUD_END */
...
retry: ret = i2c_master_send(tfa98xx->i2c, data, count + 1);
	if (ret < 0) {
		pr_warn("i2c error, retries left: %d\n", retries);
		if (retries) {
			retries--;
...
retry: ret = i2c_transfer(tfa98xx->i2c->adapter, msgs, ARRAY_SIZE(msgs));
	if (ret < 0) {
		pr_warn("i2c error, retries left: %d\n", retries);
		if (retries) {
			retries--;
		...
		const struct i2c_device_id *id)
{
	...
	if (gpio_is_valid(tfa98xx->reset_gpio)) {
		...
		if (ret) {
			pr_info("%s request reset gpio fail\n", __func__);
			goto err;
		}
	}
...
err_off: tfa98xx_unregister_dsp(tfa98xx);
err: return ret;
}
