...
enum tfa_error tfa_load_cnt(void *cnt, int length)
{
	...
	if ((HDR(cntbuf->id[0], cntbuf->id[1])) != paramsHdr) {
		pr_err("wrong header type: 0x%02x 0x%02x\n", cntbuf->id[0],
				cntbuf->id[1]);
		/* HTC_AUD - Fix access NULL pointer */
		return tfa_error_container;
	}
...
int tfacont_get_max_vstep(int dev_idx, int prof_idx)
{
	...
	vp = (nxpTfaVolumeStep2File_t *)
			tfacont_getfiledata(dev_idx, prof_idx,
			volstepHdr);
	if (vp == NULL)
		return 0;
...
static void cont_get_devs(nxpTfaContainer_t *cont)
{
	...
	for (i = 0; i < g_devs; i++) {
		j = 0;
		count = 0;
		while ((liveD = tfaContGetDevLiveDataList
				(cont, i, j)) != NULL) {
			count++;
			g_lived[i][j++] = liveD;
		}
		g_liveds[i] = count;
		/* count the nr of livedata per device */
	}
}
		...
		nxpTfaVolumeStep2File_t *vp, int vstep)
{
	...
	if (vstep < vp->vsteps) {
		...
		err = tfa98xx_dsp_write_preset(dev_idx,
		...
		int vstep_idx, int vstep_msg_idx)
{
	...
	for (i = 0; i <= vstep_idx; i++) {
		regInfo = (struct nxpTfaVolumeStepRegisterInfo *)
				(vp->vstepsBin + size);
		msgInfo = (struct nxpTfaVolumeStepMessageInfo *)
				(vp->vstepsBin
		...
		for (j = 0; j < nrMessages; j++) {
			...
			if (i == vstep_idx &&
				((vstep_msg_idx >= TFA_MAX_VSTEP_MSG_MARKER)
				|| (vstep_msg_idx == j))) {
				/* If no vstepMsgIndex is passed on,
				 * all message needs to be send
				 */
				/*
				 * The algoparams and mbdrc msg
				 * id will be changed to the
				 * reset type when SBSL=0
				 * if SBSL=1 the msg will
				 * remain unchanged.
				 * It's up to the tuning
				 * engineer to choose the 'without_reset'
				 * types inside the vstep.
				 * In other words: the reset msg
				 * is applied during SBSL==0
				 * else it remains unchanged.
				 */
				if (TFA_GET_BF(dev_idx, SBSL) == 0) {
					if (msgInfo->MessageType == 0 &&
						msgInfo->CmdId[2] !=
						SB_PARAM_SET_ALGO_PARAMS) {
						/* If the messagetype(0)
						 * is AlgoParams
						 */
						/* Only do this
				...
				/* Messagetype(3) is
				 * Smartstudio Info! Dont send this!
				 */
 ...
 */
			/* Send payload to dsp
			 * (Remove 1 from the length for cmdid)
			 */
						err = tfa_dsp_msg_id(dev_idx,
							(msgLength - 1) * 3,
							(const char *)
							msgInfo->ParameterData,
							cmdid_changed);
					} else {
				/* Send cmdId + payload to dsp */
						err = tfa_dsp_msg(dev_idx,
							msgLength * 3,
							(const char *)
							msgInfo->CmdId);
					}
...
enum Tfa98xx_Error tfaContWriteDrcFile(int dev_idx, int size, uint8_t data[])
{
	...
	if (modified == 1) {
		...
		err = tfa_dsp_msg_id(dev_idx, size - 3, (const char *) data,
				cmdid_changed);
	} else {
		/* Send cmdId + payload to dsp */
...
/*
 * write a parameter file to the device
 * The VstepIndex and VstepMsgIndex are only
 * used to write a specific msg from the vstep file.
 */
		...
		int vstep_idx, int vstep_msg_idx)
{
	...
	case volstepHdr:
		if (tfa98xx_dev_family(dev_idx) == 2) {
			err = tfaContWriteVstepMax2(dev_idx,
				(nxpTfaVolumeStepMax2File_t *) hdr,
				vstep_idx,
				vstep_msg_idx);
		} else {
			err = tfaContWriteVstep(dev_idx,
	...
	case speakerHdr:
		if (tfa98xx_dev_family(dev_idx) == 2) {
			...
			err =
				tfa98xx_dsp_write_speaker_parameters
				(dev_idx, size,
				(const unsigned char *)
				((nxpTfaSpeakerFile_t *) hdr)->data);
		}
	...
	case drcHdr:
		if (hdr->version[0] == NXPTFA_DR3_VERSION) {
			...
			err = tfaContWriteDrcFile(dev_idx, size,
			 ...
			 * 127x3 (381) bytes first block
			 * contains the device and sample rate
			 * independent settings
			 * 127x3 (381) bytes block the
			 * device and sample rate specific values.
			 * The second block can always
			 * be recalculated from the first block,
			 * if vlsCal and the sample rate are known.
			 */
			/*size = hdr->size - sizeof(nxpTfaDrc_t); */
			size = 381; /* fixed size for first block */

			/*+381 is done to only send
			 * the second part of the drc block
			 */
			err =
				tfa98xx_dsp_write_drc(dev_idx, size,
				((const unsigned char *)
						((nxpTfaDrc_t *) hdr)->data
				+ 381));
		}
		break;
	case infoHdr:
		...
		nxpTfaBitfield_t bf)
{
	...
	pr_debug("bitfield: %s=%d (0x%x[%d..%d]=0x%x)\n",
			tfaContBfName(bfUni.field,
					tfa98xx_dev_revision(dev_idx)),
			value,
			bfUni.Enum.address,
			bfUni.Enum.pos,
			bfUni.Enum.pos+bfUni.Enum.len,
			value);
#endif
		...
		nxpTfaBitfield_t *bf)
{
	...
	error = tfa98xx_read_register16(dev_idx,
			(unsigned char) (bfUni.Enum.address), &regvalue);
	if (error)
		return error;
		...
		nxpTfaDspMem_t *cfmem)
{
	...
	for (i = 0; i < cfmem->size; i++) {
		...
		error = tfa98xx_dsp_write_mem_word(dev, cfmem->address++,
		...
		nxpTfaContBiquad_t *bq)
{
	...
	for (i = 0; i < runs; i++) {
		...
		if (tfa98xx_dev_family(dev) == 2) {
			error = tfa_dsp_cmd_id_write(dev, MODULE_FRAMEWORK,
					FW_PAR_ID_SET_MEMORY,
					sizeof(data), data);
		} else {
			error = tfa_dsp_cmd_id_write(dev,
	...
	for (i = 0; i < dev->length; i++) {
		...
		if (dev->list[i].type == dscRegister) {
			err =
				...
				(nxpTfaRegpatch_t *) (dev->list[i].offset
				+ (char *) g_cont));
		}
...
enum Tfa98xx_Error tfaContWritePatch(int dev_idx)
{
	...
	for (i = 0; i < dev->length; i++) {
		if (dev->list[i].type == dscPatch) {
			...
			/* size is total length */
			return tfa_dsp_patch(dev_idx, size,
					(const unsigned char *)
					patchfile->data);
		}
	...
	for (i = 0; i < dev->length; i++) {
		...
		if (dev->list[i].type == dscCfMem) {
			err =
					...
					(nxpTfaDspMem_t *) (dev->list[i].offset
					+ (uint8_t *) g_cont));
		}
		...
		int vstep_idx)
{
	...
	for (i = 0; i < prof->length; i++) {
		...
		case dscFile:
			file = (nxpTfaFileDsc_t *) (prof->list[i].offset
					+ (uint8_t *) g_cont);
			err = tfaContWriteFile(dev_idx, file, vstep_idx,
					TFA_MAX_VSTEP_MSG_MARKER);
			break;
		case dscFilter:
			/* Filters are not written during coldstart
			 * Since calibration, SBSL and many other
			 * actions can overwrite them again
		...
		case dscCfMem:
			err = tfaRunWriteDspMem(dev_idx,
					(nxpTfaDspMem_t *) (prof->list[i].offset
							+ (uint8_t *) g_cont));
			break;
		default:
...
unsigned int tfa98xx_get_profile_sr(int dev_idx, unsigned int prof_idx)
{
	...
	fs_profile, tfa98xx_sr_from_field(fs_profile), dev_idx, prof_idx);
	if (fs_profile != -1)
		return tfa98xx_sr_from_field(fs_profile);
		...
		nxpTfaProfileList_t *previous_prof, int fs_previous_profile)
{
	...
	for (i = 0; i < prof->length; i++) {
		/* We only want to write the
		 * values before the default section
		 */
		if (prof->list[i].type == dscDefault)
			break;
		/* search for AUDFS */
 ...
 */
	...
	if (fs_next_profile != fs_default_profile) {
		...
		/* But if we do not have a new
		 * AUDFS in the next profile and
		 * the AUDFS from the default
		 * profile is not the same as the AUDFS
		 * from the previous profile we
		 * also need to write new delay tables
		 */
...
enum Tfa98xx_Error tfaContWriteProfile(int dev_idx, int prof_idx, int vstep_idx)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
	nxpTfaProfileList_t *prof = tfaContProfile(dev_idx, prof_idx);
	nxpTfaProfileList_t *previous_prof = tfaContProfile(dev_idx,
	...
	/* We only make a power cycle
	 * when the profiles are not in the same group
	 */
	if (prof->group == previous_prof->group && prof->group != 0) {
		 ...
		 */
		/* HTC_AUD - remove this condition check
		 * or switch profile will always fail
		 */
	...
	if (prof->group != previous_prof->group || prof->group == 0) {
		...
		if (tfa98xx_dev_family(dev_idx) == 2) {
			 ...
			 * make sure DSP is out of
			 * Reset mode before setting files
			 */
	...
	err = get_sample_rate_info(dev_idx, prof, previous_prof,
			fs_previous_profile);
	if (err)
		return err;
	...
	if (tfa98xx_dev_family(dev_idx) == 2) {
		for (i = 0; i < previous_prof->length; i++) {
			...
			/* Only if we found the default
			 * section try writing the file
			 */
			if (i < previous_prof->length) {
	...
	for (i = j; i < prof->length; i++) {
		...
		case dscCmd:
			...
			if (tfa98xx_cnt_verbose) {
				cmd =
					(nxpTfaCmd_t *) (prof->list[i].offset
					+ (uint8_t *) g_cont);
				pr_debug("Writing cmd=0x%02x%02x%02x\n",
						cmd->value[0],
						cmd->value[1], cmd->value[2]);
		...
		int vstep_idx)
{
	...
	for (i = 0; i < prof->length; i++) {
		if (prof->list[i].type == dscFile) {
			...
			switch (type) {
			case volstepHdr:
				if (tfaContWriteFile(dev_idx, file, vstep_idx,
						TFA_MAX_VSTEP_MSG_MARKER))
					return Tfa98xx_Error_Bad_Parameter;
				break;
			default:
...
void individual_calibration_results(Tfa98xx_handle_t handle)
{
	...
	/* Read the calibration result in
	 * xmem (529=primary channel) (530=secondary channel)
	 */
...
/*
 * Get the name of the profile at
 * certain index for a device in the container file
 *  return profile name
 */
	...
	if (result) {
		...
		if (result == 0) {
			...
			if (currentValue + delta < 15) {
				TFA_SET_BF_VOLATILE(dev_idx,
						DCMCC, currentValue + delta);
				if (tfa98xx_cnt_verbose)
