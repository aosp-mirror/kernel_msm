...
/* these indexes and scaling factors should
 * be recovered from the ITF xml file.
 */
#define fs_IDX                128
...
#if !(defined(TFA9887) || defined(TFA9890) || \
	  defined(TFA9887B) || defined(TFA9897))
#ifndef TFA98XX_FULL
#define TFA98XX_FULL
#endif
#endif
...
enum Tfa98xx_Error {
	...
	/* Currently only used to keep in sync with tfa_error */
...
enum Tfa98xx_Status_ID {
	...
	/* Invalid sequence of commands, in case the
	 * DSP expects some commands in a specific order
	 */
	Tfa98xx_I2C_Req_Invalid_Param = 6,
	/* Generic error */
	Tfa98xx_I2C_Req_Buffer_Overflow = 7
	/* I2C buffer has overflowed: host has sent too
	 * many parameters, memory integrity is not guaranteed
	 */
};
 ...
 */
enum Tfa98xx_DAI {
	Tfa98xx_DAI_I2S = 0x01, Tfa98xx_DAI_TDM = 0x02, Tfa98xx_DAI_PDM = 0x04,
};
...
enum Tfa98xx_Mode {
	Tfa98xx_Mode_Normal = 0, Tfa98xx_Mode_RCV
};

enum Tfa98xx_Mute {
	Tfa98xx_Mute_Off, Tfa98xx_Mute_Digital, Tfa98xx_Mute_Amplifier
};
...
#define nxpTfaMsg_t struct nxpTfaMsg
struct nxpTfaMsg {
	uint8_t msg_size;
...
#define nxpTfaGroup_t struct nxpTfaGroup
struct nxpTfaGroup {
	uint8_t msg_size;
...
/**
 * If needed, this function can be used to
 * get a text version of the status ID code
 ...
 */
enum Tfa98xx_Error
tfa98xx_compare_features(Tfa98xx_handle_t handle, int features_from_MTP[3],
...
Tfa98xx_handle_t handle, int length, unsigned char *pSpeakerBytes);
...
Tfa98xx_handle_t handle, unsigned char bytes[], unsigned int *statesize);
...
enum Tfa98xx_Error tfa98xx_dsp_write_drc(Tfa98xx_handle_t handle, int length,
...
/**
 * get the bitfield number corresponding to
 * the bitfield name, checks for all devices
 * @param name is the bitfield name for
 * which to get the bitfield number
 */
...
/* write/read registers using a bit field
 * name to determine the register address
 */
