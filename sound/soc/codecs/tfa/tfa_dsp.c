...
int print_calibration(Tfa98xx_handle_t handle, char *str, size_t size)
{
	return snprintf(str, size, " Prim:%d mOhms, Sec:%d mOhms\n",
		handles_local[handle].mohm[0], handles_local[handle].mohm[1]);
}
...
void tfa98xx_set_spkr_select(Tfa98xx_handle_t dev_idx, char *configuration)
{
	...
	if (configuration == NULL)
		...
		switch (firstLetter) {
		case 'b':
			/* SC / both -> apply primary also to secondary */
			handles_local[dev_idx].spkr_select = 8;
			handles_local[dev_idx].spkr_count = 2;
			break;
		case 'l':
		case 'p':
			/* DS / left -> only use primary channel */
			handles_local[dev_idx].spkr_select = 4;
			handles_local[dev_idx].spkr_count = 1;
			break;
		case 'r':
		case 's':
			/* DP / right -> only use secondary channel */
			handles_local[dev_idx].spkr_select = 2;
			handles_local[dev_idx].spkr_count = 1;
			break;
		case 'd':
			/* DC / disable -> skip applying
...
{
	handles_local[dev_idx].slave_address =
...
#else
enum Tfa98xx_Error tfa_probe(bool upper_spk, unsigned char slave_address,
		Tfa98xx_handle_t *pHandle)
#endif
/* HTC_AUD_END */
{
	...
	*pHandle = -1;
	...
	handles_local[idx].upper_spk = upper_spk; /* HTC_AUD - save upper-spk information */
	...
	case 0x6E:
	case (0x1a << 1):
	/*TODO properly implement foreign i2c addressing like uda1355 */
		...
		unsigned short *address, int channel)
{
	...
	unsigned short bq_table[7][4] = {
		 ...
		 * Every index has 5 values except
		 * index 13 this one has 6 values
		 */
	};
		...
		int *pb_support_tCoef)
{
	...
	if (handles_local[dev_idx].support_tcoef != supportNotSet) {
		*pb_support_tCoef =
				(handles_local[dev_idx].support_tcoef
					== supportYes);
	}

	handles_local[dev_idx].support_tcoef =
			*pb_support_tCoef ? supportYes : supportNo;
		...
		enum Tfa98xx_saam *saam)
{
	...
	*saam = handles_local[handle].support_saam == supportYes ?
...
enum Tfa98xx_Error tfa98xx_dsp_reset(Tfa98xx_handle_t dev_idx, int state)
{
	...
	if (tfa98xx_handle_is_open(dev_idx)) {
		if (handles_local[dev_idx].dev_ops.tfa_dsp_reset)
			error = (*handles_local[dev_idx].dev_ops.tfa_dsp_reset)
			(dev_idx,
			state);
		else
			/* generic function */
...
enum Tfa98xx_Error tfa98xx_init(Tfa98xx_handle_t handle)
{
	 ...
	 *  Write the register directly to avoid the
	 *  read in the bitfield function.
	 *  The I2CR bit may overwrite the full register
	 *  because it is reset anyway.
		...
		int sample_rate)
{
	...
	if (handles_local[handle].dev_ops.tfa_dsp_write_tables)
		error = (*handles_local[handle].dev_ops.tfa_dsp_write_tables)
				(handle,
				sample_rate);
...
static enum Tfa98xx_Error tfa98xx_set_mute_tfa2(Tfa98xx_handle_t handle,
...
static enum Tfa98xx_Error tfa98xx_set_mute_tfa1(Tfa98xx_handle_t handle,
		enum Tfa98xx_Mute mute)
{
	enum Tfa98xx_Error error;
...
/*** patching ***/
static enum Tfa98xx_Error tfa98xx_process_patch_file(Tfa98xx_handle_t handle,
		int length, const unsigned char *bytes)
{
	unsigned short size;
	int index = 0;
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
 ...
 */
static enum Tfa98xx_Error tfa98xx_check_ic_rom_version(Tfa98xx_handle_t handle,
		const unsigned char patchheader[])
{
	...
	if (checkaddress != 0xFFFF) {
		...
		if (error == Tfa98xx_Error_Ok)
			error = tfa98xx_dsp_read_mem(handle,
					checkaddress,
					1,
					&value);
		if (error == Tfa98xx_Error_Ok) {
...
enum Tfa98xx_Error tfa_dsp_patch(Tfa98xx_handle_t handle, int patchLength,
...
/*  note the 'old' write_parameter() was more
 * efficient because all i2c was in one burst transaction
 */

/*TODO properly handle bitfields:
 * state should be restored! (now it will change eg dmesg field to xmem)
 */
		...
		const char *buffer)
{
	...
	while ((error == Tfa98xx_Error_Ok) && (remaining_bytes > 0)) {
		...
		error = tfa98xx_write_data(handle,
				FAM_TFA98XX_CF_MEM,
				chunk_size,
		...
		const char *buffer, uint8_t cmdid[3])
{
	...
	/* write cmd-id */
	error = tfa98xx_write_data(handle, FAM_TFA98XX_CF_MEM, 3,
		...
		unsigned char *bytes)
{
	...
	while (num_bytes > 0) {
		...
		error = tfa98xx_read_data(handle,
 ...
 *  and do not not wait for DSP reponse.
 */
		...
		const char *buf)
{
	...
	if (rpc_status != Tfa98xx_I2C_Req_Done) {
		...
		error = (enum Tfa98xx_Error)
				(rpc_status + Tfa98xx_Error_RpcBase);
...
/**
 *  write/read raw msg functions:
 *  the buffer is provided in little endian
 *  format, each word occupying 3 bytes, length is in bytes.
 *  The functions will return immediately
 *  and do not not wait for DSP reponse.
...
TFA_INTERNAL enum Tfa98xx_Error tfa98xx_check_rpc_status(
		Tfa98xx_handle_t handle, int *pRpcStatus)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
...
#define OPTIMIZED_RPC
#ifdef OPTIMIZED_RPC /* TODO this only save 2 cycles so remove? */
	{
		...
		error = tfa98xx_write_data(handle, FAM_TFA98XX_CF_CONTROLS,
...
/* wrapper for dsp_msg that adds opcode
 * and 3 bytes required for MBDrcDynamics
 */
...
enum Tfa98xx_Error tfa98xx_dsp_write_preset(Tfa98xx_handle_t handle, int length,
		const unsigned char *p_preset_bytes)
{
	...
	if (p_preset_bytes != 0) {
		...
		error = tfa_dsp_cmd_id_write(handle, MODULE_SPEAKERBOOST,
				SB_PARAM_SET_PRESET, length, p_preset_bytes);
	} else {
		error = Tfa98xx_Error_Bad_Parameter;
	}
	return error;
 ...
 */
enum Tfa98xx_Error tfa98xx_dsp_get_hw_feature_bits(Tfa98xx_handle_t handle,
		int *features)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
	uint32_t value;
...
enum Tfa98xx_Error tfa98xx_dsp_get_sw_feature_bits(Tfa98xx_handle_t handle,
		int features[2])
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
	unsigned char bytes[3 * 2];
	...
	if (handles_local[handle].sw_feature_bits[0] != -1) {
		...
		error = tfa_dsp_cmd_id_write_read(handle,
				MODULE_FRAMEWORK,
		...
		unsigned char bytes[], unsigned int *statesize)
{
	...
	if (err == Tfa98xx_Error_Ok) {
		if (bSupportFramework) {
			...
			/* old ROM code, ask SpeakerBoost
			 * and only do first portion
			 */
			stateSize = 8;
			err = tfa_dsp_cmd_id_write_read(handle,
					MODULE_SPEAKERBOOST,
...
enum Tfa98xx_Error tfa98xx_dsp_support_framework(Tfa98xx_handle_t handle,
...
enum Tfa98xx_Error tfa98xx_dsp_write_speaker_parameters(Tfa98xx_handle_t handle,
		int length, const unsigned char *p_speaker_bytes)
{
	...
	if (p_speaker_bytes != 0) {
		...
		error = tfa_dsp_cmd_id_write(handle, MODULE_SPEAKERBOOST,
				SB_PARAM_SET_LSMODEL, length, p_speaker_bytes);
	} else {
		error = Tfa98xx_Error_Bad_Parameter;
	}
...
enum Tfa98xx_Error tfa98xx_dsp_write_config(Tfa98xx_handle_t handle, int length,
		const unsigned char *p_config_bytes)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;

	error = tfa_dsp_cmd_id_write(handle, MODULE_SPEAKERBOOST,
			SB_PARAM_SET_CONFIG, length, p_config_bytes);
		...
		const uint16_t value)
{
	...
	/* Only write when the current register
	 * value is not the same as the new value
	 */
		...
		int enable)
{
	...
	if ((tfa98xx_get_device_dai(handle) &
		...
		int profile)
{
	 ...
	 *
	 * Note that the register settings were
	 * already done before loading the patch
	 *
	 * write all the files from the device list
	 * (speaker, vstep and all messages)
	 */
...
#ifdef __KERNEL__
/* Necessary otherwise we are thrown out of
 * operating mode in kernel (because of internal clock)
 */
	if ((strnstr(tfaContProfileName(handle, profile), ".cal",
		strlen(tfaContProfileName(handle, profile))) == NULL)
...
#define CF_CONTROL 0x8100
	...
	while (state != TFA_GET_BF(handle, ACS)) {
		...
		err = tfa98xx_dsp_write_mem_word(handle, CF_CONTROL, state,
				Tfa98xx_DMEM_IOMEM);
		PRINT_ASSERT(err);
...
enum Tfa98xx_Error tfaRunStartDSP(Tfa98xx_handle_t handle)
{
	...
	if (err == Tfa98xx_Error_Ok) {
		...
		/* in pair of tfa98xx_init() - tfaRunStartup() */
		PRINT_ASSERT(err);
	}
		...
		int *calibrateDone)
{
	...
	*calibrateDone = 0;
	...
	while ((*calibrateDone != 1) &&
		   (tries < TFA98XX_API_WAITRESULT_NTRIES)) {
		err = tfa98xx_dsp_read_mem(handle,
...
enum tfa_error tfa_start(int next_profile, int *vstep)
{
	...
	for (dev = 0; dev < devcount; dev++) {
		 ...
		 * Only if the user did not give a
		 * specific profile and coldstart
		 */
	...
	for (dev = 0; dev < devcount; dev++) {
		...
		if ((next_profile != active_profile && active_profile != -1)
				|| (istap_prof == 1)) {
			err = tfaContWriteProfile(dev,
					next_profile, vstep[dev]);
			if (err != Tfa98xx_Error_Ok)
				goto error_exit;
		}
		...
		if (strnstr(tfaContProfileName(dev, next_profile), ".standby",
			 strlen(tfaContProfileName(dev, next_profile)))
			!= NULL) {
			err = tfa98xx_powerdown(dev, 1);
...
#ifdef __KERNEL__
		/* To write something in state 6 we need to
		 * be sure that SBSL is also set in IOMEM!
		 * More information can be found in the
		 * document about the powerswitch
		 */
		if (tfa98xx_dev_family(dev) == 2) {
			if ((TFA_GET_BF(dev, RST) == 1) &&
				(TFA_GET_BF(dev, SBSL) == 0) &&
				(TFA_GET_BF(dev, MANSTATE) == 6)) {
		...
		if (vstep[dev] != tfaContGetCurrentVstep(dev)
			&& vstep[dev] != -1) {
			err = tfaContWriteFilesVstep
		...
		tfa_set_swvstep(dev,
				(unsigned short) tfaContGetCurrentVstep(dev));
	}
