...
static int deinit_handler(const char *kmessage,
                                 struct kernel_param *kp)
{
   return param_set_int(kmessage, kp);
}
...
/**---------------------------------------------------------------------------
  ...
  \brief deinit_handler() -

  Handler function for module param deinit when it is changed by userspace
  Dynamically linked - do nothing
  Statically linked - exit driver, as in rmmod
  \param  -

  \return -

  --------------------------------------------------------------------------*/
static int deinit_handler(const char *kmessage, struct kernel_param *kp)
{
   int ret;
...
module_param_call(deinit, deinit_handler, param_get_int, &deinit,
                    S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
