...
static int mnh_sm_mipi_rx_dphy_write_gen3(int command, int data,
					  uint32_t device)
{
	int ret = 0;
...
static int mnh_sm_mipi_tx_dphy_write_gen3(int command, int data,
					  uint32_t device)
{
	int ret = 0;
...
static int mnh_mipi_gen3_lprxpon_wa(struct device *dev, u32 device, int enable)
{
	int ret = 0;
				...
				uint32_t rate)
{
	...
	if (!mipi_debug && dev_ovr_cfg->use_ovrd) {
		...
		/*
		 * TX_DPHY_OUT(0x5E, dev_ovr_cfg->reg_0x5E,
		 * device);
		 */
		...
		/*
		 * TX_DPHY_OUT(0x64, dev_ovr_cfg->reg_0x64,
		 * device);
		 */
...
int mnh_mipi_stop(struct device *dev, struct mnh_mipi_config config)
{
	...
	/* Shutdown device */
	return mnh_mipi_stop_device(dev, txdev);
}
