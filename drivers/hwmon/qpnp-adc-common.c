...
static const struct qpnp_vadc_map_pt adcmap_zte_100k_pu[] = {
	{ -30,   1725 },
	{ -29,   1720 },
	{ -28,   1715 },
	{ -27,   1709 },
...
int32_t qpnp_adc_scale_zte_100k_pu(struct qpnp_vadc_chip *chip,
		int32_t adc_code,
		const struct qpnp_adc_properties *adc_properties,
		const struct qpnp_vadc_chan_properties *chan_properties,
		struct qpnp_vadc_result *adc_chan_result)
{
	int64_t temp_voltage = 0;

	qpnp_adc_scale_with_calib_param(adc_code,
			adc_properties, chan_properties, &temp_voltage);

	return qpnp_adc_map_temp_voltage(
			adcmap_zte_100k_pu,
			ARRAY_SIZE(adcmap_zte_100k_pu),
			temp_voltage,
			&adc_chan_result->physical);
}
EXPORT_SYMBOL(qpnp_adc_scale_zte_100k_pu);
