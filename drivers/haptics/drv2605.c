...
#define VIBRATOR_ENABLE	"vibrator,enable"
#define VIBRATOR_TRIGGER "vibrator,trigger"
...
int drv2605_gpio(struct drv2605_platform_data *pdata)
{
	struct device_node *np = NULL;

	np = of_find_compatible_node(NULL, NULL, VIBRATOR_ENABLE);
	if (!np) {
		printk(KERN_ERR "%s: %s node not found\n", __FUNCTION__, VIBRATOR_ENABLE);
		return -ENODEV;
	}

	/* get vibrator_enable */
	pdata->GpioEnable = of_get_named_gpio(np, "vibrator_enable", 0);
	if (pdata->GpioEnable >= 0) {
		printk(KERN_ERR "%s: GpioEnable:%d.\n", __FUNCTION__, pdata->GpioEnable);
	}
	if (gpio_request(pdata->GpioEnable, "vibrator_enable"))
		printk(KERN_ERR "%s: Failed to request gpio %d for GpioEnable\n",
			__FUNCTION__, pdata->GpioEnable);
	else
		printk(KERN_INFO "%s: gpio_request vibrator_enable done\n", __FUNCTION__);

	np = of_find_compatible_node(NULL, NULL, VIBRATOR_TRIGGER);
	if (!np) {
		printk(KERN_ERR "%s: %s node not found\n", __FUNCTION__, VIBRATOR_TRIGGER);
		return -ENODEV;
	}

	/* get vibrator_trigger*/
	pdata->GpioTrigger = of_get_named_gpio(np, "vibrator_trigger", 0);
	if (pdata->GpioTrigger >= 0) {
		printk(KERN_INFO "%s: GpioTrigger:%d.\n", __FUNCTION__, pdata->GpioTrigger);
	}

	if (gpio_request(pdata->GpioTrigger, "vibrator_trigger"))
		printk(KERN_ERR "%s: Failed to request gpio %d for GpioTrigger\n",
			__FUNCTION__, pdata->GpioTrigger);
	else
		printk(KERN_INFO "%s: gpio_request vibrator_trigger done\n", __FUNCTION__);

	/* Enable power to the chip */
	gpio_direction_output(pdata->GpioEnable, 1);
...
static int drv2605_probe(struct i2c_client* client, const struct i2c_device_id* id)
{
	...
	if(pDrv2605data->PlatData.GpioTrigger && pDrv2605data->PlatData.GpioEnable){
		err = drv2605_gpio(&pDrv2605data->PlatData);
		if(err < 0){
			printk("%s, gpio request fail (%d)\n", __FUNCTION__, err);
