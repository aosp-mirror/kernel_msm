...
#include "slimport7805_tx_reg.h"

#define DEFAULT_MIPI_FORMAT_IDX  7 // 640x480@60
...
//pixel_clk--Htotal---H active--Vtotal--V active-HFP---HSW--HBP--VFP--VSW--VBP
	...
	{15, "1280X720P@30", 37, 1650, 1280, 750, 720, 110, 40, 220, 5, 5, 20}
};
...
void SP_TX_LVTTL_Bit_Mapping(struct VideoFormat* pInputFormat)//the default mode is 12bit ddr
{
	...
	if(pInputFormat->bLVTTL_HW_Interface.DDR_Mode) {
		...
		case COLOR_8_BIT://correspond with ANX8770 24bit DDR mode
			...
			for(c=0; c<12; c++) {
				sp_write_reg(SP_TX_PORT2_ADDR, 0x40 + c, 0x06 + c);
			}
			for(c=0; c<12; c++) {
				sp_write_reg(SP_TX_PORT2_ADDR, 0x4d + c, 0x1e + c);
			}
			break;
...
void SP_TX_EnhaceMode_Set(void)
{
	...
	if(c & 0x80) {
		...
		sp_write_reg(SP_TX_PORT0_ADDR, SP_TX_SYS_CTRL4_REG,
...
void hdcp_encryption_enable(BYTE enable)
{
	...
	if(enable == 0)
		...
		c |= SP_TX_HDCP_CONTROL_0_HDCP_ENC_EN;
	sp_write_reg(SP_TX_PORT0_ADDR, SP_TX_HDCP_CONTROL_0_REG,
			c & ~SP_TX_HDCP_CONTROL_0_HDCP_ENC_EN);
}
...
void SP_TX_Show_Infomation(void)
{
	...
	pr_info("   Active resolution is %d * %d\n", h_act, v_act);
...
void SP_TX_Config_Audio_I2S(struct AudioFormat *bAudioFormat)
{
	...
	c = (c&~0xff) | (bAudioFormat->bI2S_FORMAT.SHIFT_CTRL<<3) |
		bAudioFormat->bI2S_FORMAT.DIR_CTRL <<2 |
		(bAudioFormat->bI2S_FORMAT.WS_POL <<1) |
		(bAudioFormat->bI2S_FORMAT.JUST_CTRL);
	...
	c =  (c&~0xe5)| (bAudioFormat->bI2S_FORMAT.EXT_VUCP <<2) |
		(bAudioFormat->bI2S_FORMAT.AUDIO_LAYOUT) |
...
BYTE SP_TX_BW_LC_Sel(struct VideoFormat* pInputFormat)
{
	...
	pr_info("%s The optimized BW =%.2x, Lane cnt=%.2x\n", LOG_TAG,
			(unsigned int)sp_tx_bw,
			(unsigned int)sp_tx_lane_count);
...
void SP_CTRL_Variable_Init(void)
{
	...
	MIPI_Format_Index_Set(DEFAULT_MIPI_FORMAT_IDX);
}
...
void SP_CTRL_Chip_Initial(void)
{
	...
	if(MIPI_EN) {
		...
		pr_info("%s MIPI interface selected!\n",LOG_TAG);
	} else {
