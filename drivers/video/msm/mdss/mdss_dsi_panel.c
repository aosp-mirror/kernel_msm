...
static char led_pwm1[3] = {0x51, 0x0f, 0x00};/* DTYPE_DCS_LWRITE */
static char led_pwm2[2] = {0x51, 0xff};
static struct dsi_cmd_desc backlight_cmd = {
	{DTYPE_DCS_LWRITE, 1, 0, 0, 1, sizeof(led_pwm1)},
	led_pwm1
};

static struct dsi_cmd_desc backlight_cmd_otm1901a = {
...
static void mdss_dsi_panel_bklt_dcs(struct mdss_dsi_ctrl_pdata *ctrl, int level)
{
	...
	if (!strncmp(ctrl->panel_data.panel_info.panel_name,
					"dijing OTM1901A", 15)) {
...
static int mdss_dsi_hx8399_read_status(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	if (!mdss_dsi_cmp_panel_reg(ctrl_pdata->status_buf,
	...
	struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	if (!rc) {
		...
		} else if (!strcmp(string, "reg_read_hx8399")) {
			ctrl->status_mode = ESD_REG;
			ctrl->check_read_status =
				mdss_dsi_hx8399_read_status;
