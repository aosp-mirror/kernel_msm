		...
		int enable)
{
	...
	if (0 == ctrl->idle_on_cmds.cmd_cnt || 0 == ctrl->idle_off_cmds.cmd_cnt)
		return;
...
int mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	if (enable) {
		...
		if (pinfo->off_pre_rst_delay) {
			...
			usleep_range(pinfo->off_pre_rst_delay * 1000, pinfo->off_pre_rst_delay * 1000);
		}
		...
		if (pinfo->off_post_rst_delay) {
			...
			usleep_range(pinfo->off_post_rst_delay * 1000, pinfo->off_post_rst_delay * 1000);
		}
			...
			struct mdss_panel_timing *timing)
{
	...
	ctrl->panel_on_cmds = pt->panel_on_cmds;
		...
		struct mdss_panel_data *panel_data)
{
	...
	mdss_dsi_parse_dcs_cmds(np, &pt->panel_on_cmds,
		"qcom,mdss-dsi-panel-on-command",
		"qcom,mdss-dsi-panel-on-command-state");
