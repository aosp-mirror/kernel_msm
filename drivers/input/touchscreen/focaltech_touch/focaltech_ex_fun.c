...
static void esd_process(u8 *writebuf, int buflen, bool flag)
{
	if (flag) {
		if ((writebuf[1] == 0xFC) && (writebuf[2] == 0x55) && (buflen == 0x03)) {
			/* Upgrade command */
			FTS_DEBUG("[ESD]: Upgrade command(%x %x %x)!!", writebuf[0], writebuf[1],
				  writebuf[2]);
			fts_esdcheck_switch(DISABLE);
		} else if ((writebuf[1] == 0x00) && (writebuf[2] == 0x40) &&
			   (buflen == 0x03)) {
			/* factory mode bit 4 5 6 */
			FTS_DEBUG("[ESD]: Entry factory mode(%x %x %x)!!", writebuf[0], writebuf[1],
				  writebuf[2]);
			fts_esdcheck_switch(DISABLE);
		} else if ((writebuf[1] == 0x00) && (writebuf[2] == 0x00) &&
			   (buflen == 0x03)) {
			/* normal mode bit 4 5 6 */
			FTS_DEBUG("[ESD]: Exit factory mode(%x %x %x)!!", writebuf[0], writebuf[1],
				  writebuf[2]);
		...
		if ((writebuf[1] == 0x07) && (buflen == 0x02)) {
			FTS_DEBUG("[ESD]: Upgrade finish-trigger reset(07)(%x %x)!!", writebuf[0],
				  writebuf[1]);
			fts_esdcheck_switch(ENABLE);
		} else {
			       ...
			       size_t count, loff_t *ppos)
{
	...
	case PROC_UPGRADE: {
		...
		if (fts_updatefun_curr.upgrade_with_app_bin_file)
			ret = fts_updatefun_curr.upgrade_with_app_bin_file(fts_i2c_client,
					upgrade_file_path);
#if FTS_ESDCHECK_EN
		fts_esdcheck_switch(ENABLE);
#endif
				 ...
				 struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (0 != retval) {
		FTS_ERROR("%s() - ERROR: Could not convert the given input to a number. The given input was: \"%s\"",
			  __FUNCTION__, buf);
		goto error_return;
	}
#if FTS_ESDCHECK_EN
				...
				struct device_attribute *attr, char *buf)
{
	...
	fts_i2c_read_reg(client, FTS_REG_LCD_BUSY_NUM,
			 &regvalue);       /*LCD Busy number*/
	...
	fts_i2c_read_reg(client, FTS_REG_CHARGER_MODE_EN,
			 &regvalue);       /* 3 charge in*/
	count += sprintf(tmp + count, "charge stat:0x%02x\n", regvalue);

	fts_i2c_read_reg(client, FTS_REG_INT_CNT,
			 &regvalue);       /*Interrupt counter*/
