...
void fts_get_chip_types(void)
{
	...
	FTS_INFO("CHIP TYPE ID = 0x%02x%02x", chip_types.chip_idh,
		 chip_types.chip_idl);
}
...
int fts_ctpm_auto_clb(struct i2c_client *client)
{
	...
	if ((chip_types.chip_idh == 0x11) || (chip_types.chip_idh == 0x12) ||
	    ...
	    (chip_types.chip_idh == 0x14)) { /*5x36,5x36i*/
		for (i = 0; i < 100; i++) {
...
enum FW_STATUS fts_ctpm_get_pram_or_rom_id(struct i2c_client *client)
{
	...
	FTS_DEBUG("[UPGRADE] Read ROM/PRAM/Bootloader id:0x%02x%02x", reg_val[0],
		  reg_val[1]);
	...
	if (bUpgradeFlag <= 0) {
		...
		do {
			...
			if (i_ret == 0) {
				/* upgrade success */
				fts_i2c_read_reg(client, FTS_REG_FW_VER, &uc_tp_fm_ver);
				FTS_DEBUG("[UPGRADE]********************Success upgrade to new fw version 0x%x********************",
					  uc_tp_fm_ver);
...
static bool fts_lic_need_upgrade(struct i2c_client *client)
{
	...
	FTS_DEBUG("tp init ver:%x, fw init ver:%x", initcode_ver_in_tp,
		  initcode_ver_in_host);
