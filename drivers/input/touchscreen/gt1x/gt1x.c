...
static s32 irq_is_disable = IRQ_FREE;
...
void gt1x_irq_enable(void)
{
	...
	if (irq_is_disable==IRQ_DISABLE) {
		enable_irq(gt1x_i2c_client->irq);
		irq_is_disable = IRQ_ENABLE;
	}
...
void gt1x_irq_disable(void)
{
	...
	spin_lock_irqsave(&irq_lock, irqflags);
	if (irq_is_disable==IRQ_ENABLE) {
		irq_is_disable = IRQ_DISABLE;
...
static int gt1x_parse_dt(struct device *dev)
{
	...
	gt1x_switch_gpio = of_get_named_gpio(np, "goodix,switch-gpio", 0);

	if (!gpio_is_valid(gt1x_int_gpio) || !gpio_is_valid(gt1x_rst_gpio) || !gpio_is_valid(gt1x_switch_gpio)) {
		GTP_ERROR("Invalid GPIO, irq-gpio:%d, rst-gpio:%d, switch-gpio:%d",
			gt1x_int_gpio, gt1x_rst_gpio, gt1x_switch_gpio);
		return -EINVAL;
	}
...
void gt1x_free_irq(void)
{
	if (gt1x_i2c_client && gt1x_i2c_client->irq)
		free_irq(gt1x_i2c_client->irq, gt1x_i2c_client);
	irq_is_disable = IRQ_FREE;
}
...
static int gt1x_pinctrl_init(struct device *dev)
{
	int rc = 0;
	GTP_DEBUG_FUNC();
	...
	if (IS_ERR_OR_NULL(pinctrl)) {
		GTP_ERROR("Target does not use pinctrl\n");
		rc = PTR_ERR(pinctrl);
		pinctrl = NULL;
		return rc;
	}

	gpio_state_active = pinctrl_lookup_state(pinctrl, "pmx_ts_active");
	if (IS_ERR_OR_NULL(gpio_state_active)) {
		GTP_ERROR("Cannot get pmx_ts_active state\n");
		rc = PTR_ERR(gpio_state_active);
		pinctrl = NULL;
		gpio_state_active = NULL;
		return rc;
	}
	gpio_state_suspend = pinctrl_lookup_state(pinctrl, "pmx_ts_suspend");
	if (IS_ERR_OR_NULL(gpio_state_suspend)) {
		GTP_ERROR("Cannot get pmx_ts_suspend state\n");
		rc = PTR_ERR(gpio_state_suspend);
		pinctrl = NULL;
...
#define PINCTRL_SUSPEND 0
static int gt1x_pinctrl_state(bool state)
{
	int rc;
	GTP_INFO("Set %s", state?"active":"suspend");

	if(state) {
		rc = pinctrl_select_state(pinctrl, gpio_state_active);
		if (rc) {
			GTP_ERROR("Cannot set active gpio\n");
			return rc;
		}
	} else {
		rc = pinctrl_select_state(pinctrl, gpio_state_suspend);
		if (rc) {
			GTP_ERROR("Cannot set suspend gpio\n");
			return rc;
		}
	}
...
static int gt1x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ret = gt1x_pinctrl_init(&client->dev);
	if (ret < 0) {
		GTP_ERROR("gt1x_pinctrl_init failed (%d)", ret);
	}
	ret = gt1x_pinctrl_state(PINCTRL_ACTIVE);
	if (ret < 0) {
		GTP_ERROR("gt1x_pinctrl_state failed (%d)", ret);
	}
