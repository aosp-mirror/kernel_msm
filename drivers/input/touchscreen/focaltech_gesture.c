...
int fts_read_Gestruedata(void)
{
	...
	ret = fts_i2c_read(fts_i2c_client, buf, 1,
				buf, FTS_GESTRUE_POINTS_HEADER);
	...
	if (fts_updateinfo_curr.CHIP_ID == 0x54 ||
		...
		if ((pointnum * 4 + 2) < 255) {
			...
			ret = fts_i2c_read(fts_i2c_client, buf, 0,
						buf + 255,
						(pointnum * 4 + 2) - 255);
		}
		if (ret < 0) {
	...
	else
	{
		if (0x24 == buf[0])
		{
			gestrue_id = 0x24;
			fts_check_gesture(fts_input_dev,gestrue_id);
			pr_info("%d check_gesture gestrue_id.\n", gestrue_id);
			return -1;
		}
