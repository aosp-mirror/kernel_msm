				  ...
				  unsigned int enable)
{
	struct fts_psensor_platform_data *psensor_pdata =
	    container_of(sensors_cdev, struct fts_psensor_platform_data,
			 ps_cdev);
...
static void fts_report_value(struct fts_ts_data *data)
{
	...
	if ((!big_area_enabled_flag)
		...
		if (print_gesture) {
			printk(KERN_INFO
			       "[fts]%s, enter press sleep key for suspend mode gesture_id : 0x%x\n",
	...
	if ((big_area_enabled_flag) && (event->gesture_id == GESTURE_UNKNOWN)) {
		...
		if (en_big_area_func) {
			...
			input_sync(data->input_dev);
			queue_delayed_work(data->ts_workqueue,
					   &data->touch_event_recovery_work,
					   msecs_to_jiffies(TP_RECOVERY_TIME));
		} else {
		...
		if (en_record_tp_data_func) {
			...
			tmp_buf_len =
			    sprintf(tmp_buf, "%d %d %d\n", event->au16_x[i],
				    event->au16_y[i],
				    event->au8_touch_event[i]);
			if ((TPdata_buf_len + tmp_buf_len) >= RECORD_SIZE) {
				printk(KERN_DEBUG "[fts]memory over\n");
			} else {
				/* snprintf(TPdata_buf + TPdata_buf_len, tmp_buf_len, "%s", tmp_buf); */
				sprintf(TPdata_buf + TPdata_buf_len, "%s",
					tmp_buf);
				TPdata_buf_len += tmp_buf_len;
				printk(KERN_DEBUG
				       "[fts]tp_data : %s [fts]memcpy size : %d\n",
...
static void fts_gesture_check(struct fts_ts_data *data)
{
	...
	if (data->tp_gesture_id == GESTURE_SMALL_AREA) {
		if (print_gesture) {
			printk(KERN_INFO
			       "[fts]%s, enter resume mode gesture_id : 0x%x\n",
...
static int fts_gpio_configure(struct fts_ts_data *data, bool on)
{
	...
	if (on) {
		...
		if (gpio_is_valid(data->pdata->reset_gpio)) {
			err =
			    gpio_request(data->pdata->reset_gpio,
					 "fts_reset_gpio");
			if (err) {
				dev_err(&data->client->dev,
...
int fts_ts_start(struct device *dev)
{
	...
	if (data->ts_pinctrl) {
		err =
		    pinctrl_select_state(data->ts_pinctrl,
...
#ifdef CONFIG_TOUCHSCREEN_FTS_PSENSOR
	if (fts_psensor_support_enabled() && data->pdata->psensor_support &&
	    device_may_wakeup(dev) && data->psensor_pdata->tp_psensor_opened) {
			     ...
			     struct fts_ts_platform_data *pdata)
{
	...
	if (!strcmp(name, "focaltech,panel-coords")) {
		...
		dev_err(dev, "[fts] x_max=%d, y_max=%d\n", pdata->x_max, pdata->y_max);
	} else {
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
	...
	pdata->no_force_update =
	    of_property_read_bool(np, "focaltech,no-force-update");
	/* reset, irq gpio info */
	pdata->reset_gpio =
	...
	pdata->info.AUTO_CLB =
	    of_property_read_bool(np, "focaltech,fw-auto-cal");

	pdata->fw_vkey_support =
	    of_property_read_bool(np, "focaltech,fw-vkey-support");

	pdata->ignore_id_check =
	    of_property_read_bool(np, "focaltech,ignore-id-check");
...
DEFINE_SIMPLE_ATTRIBUTE(debug_data_fops, fts_debug_data_get, fts_debug_data_set,
			"0x%02llX\n");
...
DEFINE_SIMPLE_ATTRIBUTE(debug_addr_fops, fts_debug_addr_get, fts_debug_addr_set,
			"0x%02llX\n");
			...
			const struct i2c_device_id *id)
{
	...
	data->tch_data =
	    devm_kzalloc(&client->dev, data->tch_data_len, GFP_KERNEL);
...
#ifdef FTS_GESTRUE_EN
	INIT_DELAYED_WORK(&data->touch_event_recovery_work,
			  fts_touch_recovery_work);
#endif
	...
	err =
	    request_threaded_irq(
				gpio_to_irq(data->pdata->irq_gpio), NULL,
				 fts_ts_interrupt,
				 pdata->
	...
	data->ts_info =
	    devm_kzalloc(&client->dev, FTS_INFO_MAX_LEN, GFP_KERNEL);
	if (!data->ts_info) {
		dev_err(&client->dev, "Not enough memory\n");
		goto free_debug_dir;
	}
...
exit_create_singlethread:
	printk(KERN_ERR "[fts]==singlethread error =\n");
