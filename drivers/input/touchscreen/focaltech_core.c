...
/* dean add, for TP Coverage > 50% suspend &  < 50% resume */
...
static void fts_report_value(struct fts_ts_data *data)
{
	...
	memcpy(buf, buf_touch_data+(((buf_count_neg-1)%30)*POINT_READ_BUF),
...
/* dean add, for TP Coverage > 50% suspend &  < 50% resume */
	...
	if (print_gesture) {
		pr_info("[fts]%s, gesture_id : 0x%x, buf[FTS_TOUCH_GESTURE_ID]:0x%x, big_area_enabled_flag:0x%x\n",
				__func__, event->gesture_id,
				buf[FTS_TOUCH_GESTURE_ID],
				big_area_enabled_flag);
	}

	if ((!big_area_enabled_flag) &&
		(event->gesture_id == GESTURE_LARGE_AREA)) {
		/* release all touches */
		...
		/*press sleep key*/
		if (print_gesture)
			pr_info("[fts]%s, enter press sleep key for suspend mode gesture_id : 0x%x\n",
					__func__, event->gesture_id);

		if (en_big_area_func) {
			/* press sleep key */
			input_report_key(data->input_dev, KEY_SLEEP, true);
	...
	if ((big_area_enabled_flag) && (event->gesture_id == GESTURE_UNKNOWN)) {
		/*release sleep key*/
		pr_info("[fts]%s, enter release sleep key for suspend mode gesture_id : 0x%x\n",
				__func__, event->gesture_id);
	...
	if (big_area_enabled_flag) {
		if (print_gesture)
			pr_info("[fts]%s, waiting for release sleep key, suspend mode gesture_id : 0x%x\n",
					__func__, event->gesture_id);
		return;
	}
	...
	for (i = 0; i < event->touch_point; i++) {
		...
		if (event->au8_touch_event[i] == FTS_TOUCH_DOWN ||
			...
			if (print_point)
				pr_info("[fts] finger id=%d x=%d y=%d press=%d\n",
						i,
						event->au16_x[i],
						event->au16_y[i],
						event->au8_touch_event[i]);
			...
			input_mt_report_slot_state(data->input_dev,
							MT_TOOL_FINGER, false);
			data->touchs &= ~BIT(event->au8_finger_id[i]);
			pr_info("[fts] finger id=%d x=%d y=%d release\n",
					i,
					event->au16_x[i],
					event->au16_y[i]);
		}
...
static int fts_power_init(struct fts_ts_data *data, bool on)
{
	...
	if (regulator_count_voltages(data->vcc_i2c) > 0) {
		...
		if (rc) {
			dev_err(&data->client->dev,
				"Regulator set failed vcc_i2c rc=%d\n", rc);
...
/* dean add, for TP Coverage > 50% suspend &  < 50% resume */
	...
	if (fts_irq_desc->depth > 0 && fts_wq_running == false) {
		pr_info("[fts]%s, enanle irq, disable depth : %u\n", __func__, fts_irq_desc->depth);
		enable_irq(fts_wq_data->client->irq);
...
/* dean add, for TP Coverage > 50% suspend &  < 50% resume */
	...
	/* release all touches */
	for (i = 0; i < fts_wq_data->pdata->num_max_touches; i++) {
		input_mt_slot(fts_wq_data->input_dev, i);
		input_mt_report_slot_state(fts_wq_data->input_dev,
						MT_TOOL_FINGER, 0);
	}
	input_mt_report_pointer_emulation(fts_wq_data->input_dev, false);
	input_sync(fts_wq_data->input_dev);

/* dean add, for TP Coverage > 50% suspend &  < 50% resume */
#ifdef FTS_GESTRUE_EN
	/* Entry Gesture mode */
	err = fts_write_reg(fts_i2c_client, FTS_GESTURE_SETTING_ADRESS, 0x01);

	/* only listen small area */
	err = fts_write_reg(fts_i2c_client, 0xd1, 0x20);

	if (err < 0) {
		pr_err("[fts]%s, TP set into gesture mode Fail !\n", __func__);
	} else {
		err = fts_i2c_read(fts_i2c_client, &reg_addr, 1, &reg_value, 1);
		pr_info("[fts]%s, FTS_GESTURE_SETTING_ADRESS, ret val : %d\n",
			__func__, reg_value);
	}
	...
	pr_info("[fts]%s, auto monitor, ret : %d\n", __func__, err);

	/* Entry Monitor mode */
	txbuf[0] = FTS_REG_PMODE;
	txbuf[1] = FTS_PMODE_MONITOR;
	err = fts_write_reg(fts_i2c_client, txbuf[0], txbuf[1]);
	pr_info("[fts]%s,  FTS_PMODE_MONITOR, ret : %d\n", __func__, err);
#else
	/* Entry Hibernate mode */
	txbuf[0] = FTS_REG_PMODE;
	txbuf[1] = FTS_PMODE_HIBERNATE;
	err = fts_write_reg(fts_i2c_client, txbuf[0], txbuf[1]);
	pr_info("[fts]%s, FTS_PMODE_HIBERNATE, ret : %d\n", __func__, err);
...
#ifdef FTS_GPIO_CONTROL
	err = fts_gpio_configure(fts_wq_data, false);
	if (err < 0) {
				 ...
				 unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == FB_EVENT_BLANK && fts_data &&
		...
		case FB_BLANK_POWERDOWN: /* for screen off */
		case FB_BLANK_VSYNC_SUSPEND: /* for screen on */
...
DEFINE_SIMPLE_ATTRIBUTE(debug_data_fops,
			fts_debug_data_get,
			fts_debug_data_set,
			"0x%02llX\n");
...
DEFINE_SIMPLE_ATTRIBUTE(debug_addr_fops,
			fts_debug_addr_get,
			fts_debug_addr_set,
			"0x%02llX\n");
...
DEFINE_SIMPLE_ATTRIBUTE(debug_suspend_fops,
			fts_debug_suspend_get,
			fts_debug_suspend_set,
			"%lld\n");
			...
			const struct i2c_device_id *id)
{
	...
	data->tch_data = devm_kzalloc(&client->dev, data->tch_data_len,
					GFP_KERNEL);
	...
	if (pdata->power_init) {
		...
		if (err) {
			dev_err(&client->dev,
				"pdata->power_init power init failed");
		...
		if (err) {
			dev_err(&client->dev,
				"fts_power_init power init failed");
	...
	if (err < 0) {
		dev_err(&client->dev, "version read failed, err %d", err);
		goto free_gpio;
	}
...
/* dean add, for TP Coverage > 50% suspend &  < 50% resume */
	...
	/*dean note : client->irq = gpio_to_irq(data->pdata->irq_gpio) */
	err = request_threaded_irq(gpio_to_irq(data->pdata->irq_gpio),
			NULL,
			fts_ts_interrupt,
	...
	data->ts_info = devm_kzalloc(&client->dev, FTS_INFO_MAX_LEN,
					GFP_KERNEL);
	if (!data->ts_info) {
		dev_err(&client->dev, "Not enough memory\n");
		goto free_debug_dir;
	...
	if (data->tp_vendor_id == TP_ID_PFW2_BLACK_ORIGIN ||
		data->tp_vendor_id == TP_ID_PFW2_BLACK) {
		pr_info("[fts]TP is PFW2 Black, 1.1 mm : %x\n",
				data->tp_vendor_id);
		data->shipping_fw_version = SHIPPING_FW_VERSION_PFW2_BLACK;
	} else if (data->tp_vendor_id == TP_ID_PFW2_GOLD) {
		pr_info("[fts]TP is PFW2 Gold, 1.1 mm : %x\n",
				data->tp_vendor_id);
		data->shipping_fw_version = SHIPPING_FW_VERSION_PFW2_GOLD;
	} else if (data->tp_vendor_id == TP_ID_PFW2_ROSE_GOLD) {
		pr_info("[fts]TP is PFW2 Rose Gold, 1.1 mm : %x\n",
				data->tp_vendor_id);
		data->shipping_fw_version = SHIPPING_FW_VERSION_PFW2_ROSE_GOLD;
	} else if (data->tp_vendor_id == TP_ID_PFW2_SILVER) {
		pr_info("[fts]TP is PFW2 Sliver, 1.1 mm : %x\n",
				data->tp_vendor_id);
		data->shipping_fw_version = SHIPPING_FW_VERSION_PFW2_SILVER;
	} else if (data->tp_vendor_id == TP_ID_PFW2_SABLE) {
		pr_info("[fts]TP is PFW2 Sable, 1.1 mm : %x\n",
				data->tp_vendor_id);
		data->shipping_fw_version = SHIPPING_FW_VERSION_PFW2_SABLE;
	} else if (data->tp_vendor_id == TP_ID_PFW2_NAVY) {
		pr_info("[fts]TP is PFW2 Navy, 1.1 mm : %x\n",
				data->tp_vendor_id);
		data->shipping_fw_version = SHIPPING_FW_VERSION_PFW2_NAVY;
	} else if (data->tp_vendor_id == TP_ID_PFW3_BLACK) {
		pr_info("[fts]TP is PFW3 Black, 1.1 mm, 2.5D, R0.2 : %x\n",
				data->tp_vendor_id);
		data->shipping_fw_version = SHIPPING_FW_VERSION_PFW3_BLACK;
	} else if (data->tp_vendor_id == TP_ID_PFW4_BLACK) {
		pr_info("[fts]TP is PFW4 Black, 1.1 mm, 2.5D, R0.25 : %x\n",
				data->tp_vendor_id);
		data->shipping_fw_version = SHIPPING_FW_VERSION_PFW4_BLACK;
	} else if (data->tp_vendor_id == TP_ID_PFW5_BLACK) {
		pr_info("[fts]TP is PFW5 Black, 1.8 mm, Step : %x\n",
				data->tp_vendor_id);
		data->shipping_fw_version = SHIPPING_FW_VERSION_PFW5_BLACK;
	} else {
		pr_info("[fts]TP isn't correct: %x, err : 0x%x\n",
				data->tp_vendor_id, err);
		data->shipping_fw_version = 0;
	}
	...
	pr_info("[fts] fw_vendor_id : %x , tp_vendor_id : %x\n",
	...
	FTS_STORE_TS_INFO(data->ts_info, data->family_id, data->pdata->name,
		...
		if (fts_rw_iic_drv_init(client) < 0) {
			dev_err(&client->dev,
				"%s:[FTS]create iic driver failed\n", __func__);
		}
	#endif
...
/* dean add, for TP Coverage > 50% suspend &  < 50% resume */
	...
	if (data->tp_vendor_id >= 0) {
		pr_info("[fts] Enter CTP Auto Upgrade\n");
		if (data->fw_ver[0] == 0) /* recovery */
			pr_info("[fts] FW ver is 0x00, Enter Recovery mode\n");

		fts_ctpm_auto_upgrade_for_cci(client,
				data->tp_vendor_id, false);
	} else {
		pr_err("[fts] tp_vendor_id is Fail, %d\n", data->tp_vendor_id);
	}
	#endif
