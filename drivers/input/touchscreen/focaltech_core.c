...
static void fts_report_value(struct fts_ts_data *data)
{
	...
	for (i = 0; i < event->touch_point; i++)
	{
		...
		if (event->au8_touch_event[i] == FTS_TOUCH_DOWN || event->au8_touch_event[i] == FTS_TOUCH_CONTACT)
		{
			...
			if (phase_hwid <= 1) {
				input_report_abs(data->input_dev, ABS_MT_POSITION_X, LCM_SIZE_X-event->au16_x[i]);
				input_report_abs(data->input_dev, ABS_MT_POSITION_Y, LCM_SIZE_Y-event->au16_y[i]);
			} else {
		...
		else
		{
			...
			data->touchs &= ~BIT(event->au8_finger_id[i]);
			if (phase_hwid <= 1)
				printk(KERN_INFO "[fts] finger id=%d x=%d y=%d release\n", i, LCM_SIZE_X-event->au16_x[i], LCM_SIZE_Y-event->au16_y[i]);
			else
