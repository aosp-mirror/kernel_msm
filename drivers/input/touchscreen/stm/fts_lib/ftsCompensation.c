...
int requestCompensationData(u8 type){
	...
	while (retry < RETRY_COMP_DATA_READ) {
		ret = writeSysCmd(SYS_CMD_LOAD_DATA,  &type, 1);										//send the request to the chip to load in memory the Compensation Data
		if(ret<OK){
		...
		{
			logError(0,"%s %s: Request Compensation data FINISHED! \n", tag, __func__);
			return OK;
		}
	}

	logError(1,"%s %s: Requesting compensation data... ERROR %08X \n", tag, __func__, ret|ERROR_REQU_COMP_DATA);
...
int readMutualSenseGlobalData(u64 *address, MutualSenseData *global) {
	...
	logError(0,"%s force_len = %d sense_len = %d CX1 = %d \n", tag, global->header.force_node, global->header.sense_node, global->cx1);
...
int readMutualSenseNodeData(u64 address, MutualSenseData *node) {
	...
	if (node->node_data == NULL) {
		logError(1,"%s %s: can not allocate node_data... ERROR %08X", tag, __func__, ERROR_ALLOC);
		return ERROR_ALLOC;
	}
	...
	if ( ret < OK) {
		logError(1,"%s %s: error while reading node data ERROR %08X \n", tag, __func__, ret);
...
int readSelfSenseGlobalData(u64 *address, SelfSenseData *global) {
	...
	ret = fts_writeReadU8UX(FTS_CMD_FRAMEBUFFER_R, BITS_16, *address, data, COMP_DATA_GLOBAL, DUMMY_FRAMEBUFFER);
	if ( ret < OK) {
		logError(1,"%s %s: error while reading the data... ERROR %08X \n", tag, __func__, ret);
		return ret;
	}
	...
	logError(0,"%s force_len = %d sense_len = %d  f_ix1 = %d   s_ix1 = %d   f_cx1 = %d   s_cx1 = %d \n", tag, global->header.force_node, global->header.sense_node, global->f_ix1, global->s_ix1, global->f_cx1, global->s_cx1);
...
int readSelfSenseNodeData(u64 address, SelfSenseData *node) {
	...
	if (node->ix2_fm == NULL) {
		logError(1, "%s %s: can not allocate memory for ix2_fm... ERROR %08X", tag, __func__, ERROR_ALLOC);
		return ERROR_ALLOC;
	}

	node->cx2_fm = (i8*)kmalloc(node->header.force_node*(sizeof(i8)), GFP_KERNEL);
	if (node->cx2_fm == NULL) {
		logError(1, "%s %s: can not allocate memory for cx2_fm ... ERROR %08X", tag, __func__, ERROR_ALLOC);
		kfree(node->ix2_fm);
		return ERROR_ALLOC;
	}
	node->ix2_sn = (u8*)kmalloc(node->header.sense_node*(sizeof(u8)), GFP_KERNEL);
	if (node->ix2_sn == NULL) {
		logError(1, "%s %s: can not allocate memory for ix2_sn ERROR %08X", tag, __func__, ERROR_ALLOC);
		kfree(node->ix2_fm);
		kfree(node->cx2_fm);
		return ERROR_ALLOC;
	}
	...
	if (node->cx2_sn == NULL) {
		logError(1, "%s %s: can not allocate memory for cx2_sn ERROR %08X", tag, __func__, ERROR_ALLOC);
		kfree(node->ix2_fm);
		kfree(node->cx2_fm);
		kfree(node->ix2_sn);
		return ERROR_ALLOC;
...
int readSelfSenseCompensationData(u8 type, SelfSenseData *data) {
	...
	if (!(type == LOAD_CX_SS_TOUCH || type == LOAD_CX_SS_TOUCH_IDLE || type == LOAD_CX_SS_KEY || type == LOAD_CX_SS_FORCE)) {
		logError(1,"%s %s: Choose a SS type of compensation data ERROR %08X\n", tag, __func__, ERROR_OP_NOT_ALLOW);
		return ERROR_OP_NOT_ALLOW;
	}

	ret = requestCompensationData(type);
	if ( ret< 0) {
		logError(1,"%s %s: error while requesting data... ERROR %08X\n", tag, __func__, ERROR_REQU_COMP_DATA);
		return (ret|ERROR_REQU_COMP_DATA);
	}
...
int readTotMutualSenseCompensationData(u8 type, TotMutualSenseData *data) {
	...
	if (!(type==LOAD_PANEL_CX_TOT_MS_TOUCH || type == LOAD_PANEL_CX_TOT_MS_LOW_POWER || type==LOAD_PANEL_CX_TOT_MS_KEY || type == LOAD_PANEL_CX_TOT_MS_FORCE)) {
		logError(1,"%s %s: Choose a TOT MS type of compensation data ERROR %08X\n", tag, __func__, ERROR_OP_NOT_ALLOW);
		return ERROR_OP_NOT_ALLOW;
	}
...
int readTotSelfSenseNodeData(u64 address, TotSelfSenseData *node) {
	...
	node->cx_fm = (short*)kmalloc(node->header.force_node*(sizeof(short)), GFP_KERNEL);
	if (node->cx_fm == NULL) {
		logError(1, "%s %s: can not allocate memory for cx2_fm ... ERROR %08X", tag, __func__, ERROR_ALLOC);
		kfree(node->ix_fm);
		return ERROR_ALLOC;
	...
	node->cx_sn = (short*)kmalloc(node->header.sense_node*(sizeof(short)), GFP_KERNEL);
	if (node->cx_sn == NULL) {
		logError(1, "%s %s: can not allocate memory for cx2_sn ERROR %08X", tag, __func__, ERROR_ALLOC);
		kfree(node->ix_fm);
		kfree(node->cx_fm);
...
int readTotSelfSenseCompensationData(u8 type, TotSelfSenseData *data) {
	...
	if (!(type == LOAD_PANEL_CX_TOT_SS_TOUCH || type == LOAD_PANEL_CX_TOT_SS_TOUCH_IDLE || type == LOAD_PANEL_CX_TOT_SS_KEY || type == LOAD_PANEL_CX_TOT_SS_FORCE)) {
		logError(1,"%s %s: Choose a TOT SS type of compensation data ERROR %08X\n", tag, __func__, ERROR_OP_NOT_ALLOW);
		return ERROR_OP_NOT_ALLOW;
	}
