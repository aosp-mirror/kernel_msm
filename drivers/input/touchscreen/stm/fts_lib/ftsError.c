...
int isI2cError(int error){
	if(((error & 0x000000FF) >= (ERROR_BUS_R & 0x000000FF)) && ((error & 0x000000FF) <= (ERROR_BUS_O & 0x000000FF)))
		return 1;
	else
		return 0;
}
		...
		if(ret<OK){
			...
			for(i=0; i<ERROR_DUMP_ROW_SIZE*ERROR_DUMP_COL_SIZE; i++){
				if(i%ERROR_DUMP_COL_SIZE==0){
					logError(1, KERN_ERR "\n%s %s: %d) ", tag, __func__,i/ERROR_DUMP_COL_SIZE);
				}
...
int addErrorIntoList(u8 *event, int size){
	...
	if(i>0){
		logError(0,"%s Filling last %d bytes of the event with zero...\n", tag, i);
...
int resetErrorList(void){
	errors.count=0;
	errors.last_index=0;
	memset(errors.list,0,FIFO_DEPTH*FIFO_EVENT_SIZE); //if count is not considered is better reset also the list in order to avoid to read data previously copied into the list
	return OK;
}
