...
u16** array1dTo2d_u16(u16* data, int size, int columns)
{
	...
	int i;
	u16** matrix = (u16**) kmalloc(((int) (size / columns)) * sizeof (u16 *), GFP_KERNEL);
	if (matrix != NULL)
	{
		for (i = 0; i < (int) (size / columns); i++)
		{
...
i8** array1dTo2d_i8(i8* data, int size, int columns)
{
	...
	int i;
	i8** matrix = (i8**) kmalloc(((int) (size / columns)) * sizeof (i8 *), GFP_KERNEL);
	if (matrix != NULL)
	{
		for (i = 0; i < (int) (size / columns); i++)
		{
...
int fromIDtoMask(u8 id, u8 *mask, int size) {
	if(((int)((id) / 8)) < size){
		logError(0, "%s %s: ID = %d Index = %d Position = %d !\n", tag, __func__, id, ((int)((id) / 8)), (id % 8));
		mask[((int)((id) / 8))] |= 0x01 << (id % 8);
		return OK;
	}else{
		logError(1, "%s %s: Bitmask too small! Impossible contain ID = %d %d>=%d! ERROR %08X\n", tag, __func__, id, ((int)((id) / 8)), size, ERROR_OP_NOT_ALLOW);
		return ERROR_OP_NOT_ALLOW;
	}
}
