...
static LimitFile limit_file;													///< variable which contains the limit file during test
...
int checkLimitsGap(short *data, int row, int column, int threshold)
{
	...
	if (row == 0 || column == 0)
	{
		logError(1, "%s checkLimitsGap: invalid number of rows = %d or columns = %d  ERROR %08X\n", tag, row, column, ERROR_OP_NOT_ALLOW);
		return ERROR_OP_NOT_ALLOW;
	}
...
/**
 * Perform an ITO test setting all the possible options (see @link ito_opt ITO Options @endlink) and checking MS Raw ADJ if enabled
...
int production_test_ito(char* path_limits, TestToDo *todo)
{
	...
	if (todo->MutualRawAdjITO == 1)
	{
		...
		if (res < OK)
		{
			logError(1, "%s %s: computeAdjHoriz failed... ERROR %08X \n", tag, __func__, ERROR_PROD_TEST_ITO);
			goto ERROR;
		}
		...
		{
			logError(1, "%s %s: parseProductionTestLimits MS_RAW_ITO_ADJH failed... ERROR %08X \n", tag, __func__, ERROR_PROD_TEST_DATA);
			goto ERROR;
		}
		...
		if (res != OK)
		{
			logError(1, "%s production_test_data: checkLimitsAdj MS RAW ITO ADJH failed... ERROR COUNT = %d \n", tag, res);
			logError(0, "%s MS RAW ITO ADJ HORIZONTAL TEST:.................FAIL \n\n", tag);
			print_frame_short("MS Raw ITO frame =", array1dTo2d_short(msRawFrame.node_data, msRawFrame.node_data_size, msRawFrame.header.sense_node), msRawFrame.header.force_node, msRawFrame.header.sense_node);
			res = ERROR_PROD_TEST_ITO;
			goto ERROR;
		} else
			logError(0, "%s MS RAW ITO ADJ HORIZONTAL TEST:.................OK  \n", tag);
		...
		if (res < OK)
		{
			logError(1, "%s %s: computeAdjVert failed... ERROR %08X \n", tag, __func__, ERROR_PROD_TEST_ITO);
			goto ERROR;
		}
		...
		{
			logError(1, "%s %s: parseProductionTestLimits MS_RAW_ITO_ADJV failed... ERROR %08X \n", tag, __func__, ERROR_PROD_TEST_ITO);
			goto ERROR;
		}
		...
		if (res != OK)
		{
			logError(1, "%s %s: checkLimitsAdj MS RAW ITO ADJV failed... ERROR COUNT = %d \n", tag, __func__, res);
			logError(0, "%s MS RAW ITO ADJ VERTICAL TEST:.................FAIL \n\n", tag);
			print_frame_short("MS Raw ITO frame =", array1dTo2d_short(msRawFrame.node_data, msRawFrame.node_data_size, msRawFrame.header.sense_node), msRawFrame.header.force_node, msRawFrame.header.sense_node);
			res = ERROR_PROD_TEST_ITO;
			goto ERROR;
		} else
			logError(0, "%s MS RAW ITO ADJ VERTICAL TEST:.................OK  \n", tag);
...
int production_test_initialization(u8 type)
{
	...
	{
		logError(1, "%s production_test_initialization: Type incompatible! Type = %02X ERROR %08X \n", tag, type, ERROR_OP_NOT_ALLOW | ERROR_PROD_TEST_INITIALIZATION);
	...
	res |= readSysInfo(1); //need to update the chipInfo in order to refresh several versions

	if (res < 0)
	{
		logError(1, "%s production_test_initialization: read sys info ERROR %08X\n", tag, ERROR_PROD_TEST_INITIALIZATION);
...
int production_test_main(char * pathThresholds, int stop_on_fail, int saveInit, TestToDo *todo)
{
	...
	if (saveInit == 1)
	{
		...
		if (ret < 0)
		{
			logError(1, "%s production_test_main: system reset ERROR %08X\n", tag, ret);
			res |= ret;
			if (stop_on_fail) goto END;
		}
		logError(0, "%s \n", tag);
...
int production_test_ms_raw(char *path_limits, int stop_on_fail, TestToDo *todo)
{
	...
	if (todo->MutualRaw == 1 || todo->MutualRawGap == 1 || todo->MutualRawAdj == 1)
	{
		...
		if (ret < OK)
		{
			logError(1, "%s production_test_data: getMSFrame failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
		...
		if (todo->MutualRaw == 1)
		{
			...
			{
				logError(1, "%s production_test_data: parseProductionTestLimits MS_RAW_MIN_MAX failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
		...
		if (todo->MutualRawGap == 1)
		{
			...
			{
				logError(1, "%s production_test_data: parseProductionTestLimits MS_RAW_GAP failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
				ret |= ERROR_PROD_TEST_DATA;
				goto ERROR_LIMITS;
			}
			...
			if (ret != OK)
			{
				logError(1, "%s production_test_data: checkLimitsGap MS RAW failed... ERROR = %08X \n", tag, ret);
				count_fail += 1;
				if (stop_on_fail == 1) goto ERROR;
		...
		if (todo->MutualRawAdj == 1)
		{
			...
			{
				logError(1, "%s production_test_data: parseProductionTestLimits MS_RAW_ADJH failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
				ret |= ERROR_PROD_TEST_DATA;
				goto ERROR_LIMITS;
			}
			...
			if (ret != OK)
			{
				logError(1, "%s production_test_data: checkLimitsAdj MS RAW ADJH failed... ERROR COUNT = %d \n", tag, ret);
				logError(0, "%s MS RAW ADJ HORIZONTAL TEST:.................FAIL \n\n", tag);
				count_fail += 1;
				if (stop_on_fail == 1) goto ERROR;
			} else
				logError(0, "%s MS RAW ADJ HORIZONTAL TEST:.................OK  \n", tag);
			...
			if (ret < OK)
			{
				logError(1, "%s production_test_data: computeAdjVert failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
				ret |= ERROR_PROD_TEST_DATA;
				goto ERROR_LIMITS;
			}
			...
			{
				logError(1, "%s production_test_data: parseProductionTestLimits MS_RAW_ADJV failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
				ret |= ERROR_PROD_TEST_DATA;
				goto ERROR_LIMITS;
			}
			...
			if (ret != OK)
			{
				logError(1, "%s production_test_data: checkLimitsAdj MS RAW ADJV failed... ERROR COUNT = %d \n", tag, ret);
				logError(0, "%s MS RAW ADJ VERTICAL TEST:.................FAIL \n\n", tag);
				count_fail += 1;
				if (stop_on_fail == 1) goto ERROR;
			} else
				logError(0, "%s MS RAW ADJ VERTICAL TEST:.................OK  \n", tag);
			kfree(thresholds);
			thresholds = NULL;
			...
			logError(0, "%s MS RAW ADJ TEST:.................SKIPPED  \n", tag);
	...
	if (todo->MutualKeyRaw == 1)
	{
		...
		if (ret < 0)
		{
			logError(1, "%s production_test_data: production_test_ms_key_raw failed... ERROR = %08X \n", tag, ret);
			count_fail += 1;
			if (count_fail == 1)
			{
	...
	if (ret < 0) {
		logError(1, "%s production_test_data: production_test_ms_raw_lp failed... ERROR = %08X \n", tag, ret);
		count_fail += 1;
		if (count_fail == 1) {
			logError(0, "%s MS RAW DATA TEST:.................FAIL fails_count = %d\n\n", tag, count_fail);
			goto ERROR_LIMITS;
...
int production_test_ms_raw_lp(char *path_limits, int stop_on_fail, TestToDo *todo)
{
	...
	if (todo->MutualRawLP == 1 || todo->MutualRawGapLP == 1 || todo->MutualRawAdjLP == 1)
	{
		...
		if (todo->MutualRawLP == 1)
		{
			...
			{
				logError(1, "%s production_test_data: parseProductionTestLimits MS_RAW_LP_MIN_MAX failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
				ret |= ERROR_PROD_TEST_DATA;
				goto ERROR_LIMITS;
			}
		...
		if (todo->MutualRawGapLP == 1)
		{
			...
			if (ret < 0 || (trows != 1 || tcolumns != 1))
			{
				logError(1, "%s production_test_data: parseProductionTestLimits MS_RAW_LP_GAP failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
				ret |= ERROR_PROD_TEST_DATA;
				goto ERROR_LIMITS;
			}
			...
			if (ret != OK)
			{
				logError(1, "%s production_test_data: checkLimitsGap MS RAW LP failed... ERROR = %08X \n", tag, ret);
				count_fail += 1;
				if (stop_on_fail == 1) goto ERROR;

			} else
				logError(0, "%s MS RAW LP GAP TEST:.................OK \n\n", tag);
			kfree(thresholds);
			thresholds = NULL;
		} else
			logError(0, "%s MS RAW LP GAP TEST:.................SKIPPED  \n", tag);
		...
		if (todo->MutualRawAdjLP == 1)
		{
			...
			{
				logError(1, "%s production_test_data: parseProductionTestLimits MS_RAW_LP_ADJH failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
				ret |= ERROR_PROD_TEST_DATA;
				goto ERROR_LIMITS;
			}
			...
			if (ret != OK)
			{
				logError(1, "%s production_test_data: checkLimitsAdj MS RAW LP ADJH failed... ERROR COUNT = %d \n", tag, ret);
				logError(0, "%s MS RAW LP ADJ HORIZONTAL TEST:.................FAIL \n\n", tag);
				count_fail += 1;
				if (stop_on_fail == 1) goto ERROR;
			} else
				logError(0, "%s MS RAW LP ADJ HORIZONTAL TEST:.................OK  \n", tag);
			...
			{
				logError(1, "%s production_test_data: checkLimitsAdj MS RAW ADJV failed... ERROR COUNT = %d \n", tag, ret);
				logError(0, "%s MS RAW LP ADJ VERTICAL TEST:.................FAIL \n\n", tag);
				count_fail += 1;
				if (stop_on_fail == 1) goto ERROR;
			} else
				logError(0, "%s MS RAW LP ADJ VERTICAL TEST:.................OK  \n", tag);
			kfree(thresholds);
			thresholds = NULL;
			...
			logError(0, "%s MS RAW LP ADJ TEST:.................SKIPPED  \n", tag);

	} else
		logError(0, "%s MS RAW LP FRAME TEST:.................SKIPPED  \n", tag);
...
ERROR:
	...
	{
		if (msRawFrame.node_data != NULL){ 
			print_frame_short("MS Raw LP frame =", array1dTo2d_short(msRawFrame.node_data, msRawFrame.node_data_size, msRawFrame.header.sense_node), msRawFrame.header.force_node, msRawFrame.header.sense_node);
			kfree(msRawFrame.node_data);
		}
		if (thresholds != NULL) kfree(thresholds);
		if (adj!= NULL)	kfree(adj);
...
int production_test_ms_key_raw(char *path_limits)
{
	...
	if (ret < 0)
	{
		logError(1, "%s production_test_data: getMSKeyFrame failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
		return (ret | ERROR_PROD_TEST_DATA);
	}

	ret = parseProductionTestLimits(path_limits, &limit_file, MS_KEY_RAW_MIN_MAX, &thresholds, &trows, &tcolumns);
	if (ret < 0 || (trows != 1 || tcolumns != 2))
	{
		logError(1, "%s production_test_data: parseProductionTestLimits MS_KEY_RAW_MIN_MAX failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
		ret |= ERROR_PROD_TEST_DATA;
		goto ERROR_LIMITS;
	}
...
int production_test_ms_cx(char *path_limits, int stop_on_fail, TestToDo *todo)
{
	...
	ret = readMutualSenseCompensationData(LOAD_CX_MS_TOUCH, &msCompData); //read MS compensation data
	if (ret < 0)
	{
		logError(1, "%s production_test_data: readMutualSenseCompensationData failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
		return (ret | ERROR_PROD_TEST_DATA);
	}

	ret = readTotMutualSenseCompensationData(LOAD_PANEL_CX_TOT_MS_TOUCH, &totCompData); //read  TOT MS compensation data
	if (ret < 0)
	{
		logError(1, "%s production_test_data: readTotMutualSenseCompensationData failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
	...
	if (todo->MutualCx1 == 1)
	{
		...
		if (ret < 0 || (trows != 1 || tcolumns != 2))
		{
			logError(1, "%s production_test_data: parseProductionTestLimits MS_CX1_MIN_MAX failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
			ret |= ERROR_PROD_TEST_DATA;
			goto ERROR_LIMITS;
		}
	...
	if (todo->MutualCx2 == 1)
	{
		ret = parseProductionTestLimits(path_limits, &limit_file, MS_CX2_MAP_MIN, &thresholds_min, &trows, &tcolumns); //load min thresholds
		if (ret < 0 || (trows != msCompData.header.force_node || tcolumns != msCompData.header.sense_node))
		{
			logError(1, "%s production_test_data: parseProductionTestLimits MS_CX2_MAP_MIN failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
			ret |= ERROR_PROD_TEST_DATA;
			goto ERROR_LIMITS;
		}
		...
		if (ret < 0 || (trows != msCompData.header.force_node || tcolumns != msCompData.header.sense_node))
		{
			logError(1, "%s production_test_data: parseProductionTestLimits MS_CX2_MAP_MAX failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
			ret |= ERROR_PROD_TEST_DATA;
			goto ERROR_LIMITS;
		}
	...
	if (todo->MutualCx2Adj == 1)
	{
		...
		{
			logError(1, "%s production_test_data: parseProductionTestLimits MS_CX2_ADJH_MAP_MAX failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
			ret |= ERROR_PROD_TEST_DATA;
			goto ERROR_LIMITS;
		}
		...
		{
			logError(1, "%s production_test_data: parseProductionTestLimits MS_CX2_ADJV_MAP_MAX failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
			ret |= ERROR_PROD_TEST_DATA;
			goto ERROR_LIMITS;
		}
	...
	if (todo->MutualCxTotal == 1 || todo->MutualCxTotalAdj == 1)
	{
		...
		{
			ret = parseProductionTestLimits(path_limits, &limit_file, MS_TOTAL_CX_MAP_MIN, &thresholds_min, &trows, &tcolumns); //load min thresholds
			if (ret < 0 || (trows != totCompData.header.force_node || tcolumns != totCompData.header.sense_node))
			{
				logError(1, "%s production_test_data: parseProductionTestLimits MS_TOTAL_CX_MAP_MIN failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
				ret |= ERROR_PROD_TEST_DATA;
				goto ERROR_LIMITS;
			}

			ret = parseProductionTestLimits(path_limits, &limit_file, MS_TOTAL_CX_MAP_MAX, &thresholds_max, &trows, &tcolumns); //load max thresholds
			if (ret < 0 || (trows != totCompData.header.force_node || tcolumns != totCompData.header.sense_node))
			{
				logError(1, "%s production_test_data: parseProductionTestLimits MS_TOTAL_CX_MAP_MAX failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
				ret |= ERROR_PROD_TEST_DATA;
				goto ERROR_LIMITS;
			}
		...
		if (todo->MutualCxTotalAdj == 1)
		{
			...
			{
				logError(1, "%s production_test_data: parseProductionTestLimits MS_TOTAL_CX_ADJH_MAP_MAX failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
				ret |= ERROR_PROD_TEST_DATA;
				goto ERROR_LIMITS;
			}
			...
			{
				logError(1, "%s production_test_data: parseProductionTestLimits MS_TOTAL_CX_ADJV_MAP_MAX failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
				ret |= ERROR_PROD_TEST_DATA;
				goto ERROR_LIMITS;
			}
	...
	if ((todo->MutualKeyCx1 | todo->MutualKeyCx2 | todo->MutualKeyCxTotal) == 1)
	{
		...
		{
			count_fail += 1;
			logError(1, "%s production_test_data: production_test_ms_key_cx failed... ERROR = %08X \n", tag, ret);
...
ERROR:
	...
	{
		print_frame_i8("MS Init Data (Cx2) =", array1dTo2d_i8(msCompData.node_data, msCompData.node_data_size, msCompData.header.sense_node), msCompData.header.force_node, msCompData.header.sense_node);
		print_frame_short(" TOT MS Init Data (Cx) =", array1dTo2d_short(totCompData.node_data, totCompData.node_data_size, totCompData.header.sense_node), totCompData.header.force_node, totCompData.header.sense_node);
		logError(0, "%s MS CX testes finished!.................FAILED  fails_count = %d\n\n", tag, count_fail);
		if (thresholds != NULL) kfree(thresholds);
		...
		if (totCompData.node_data != NULL) kfree(totCompData.node_data);
...
int production_test_ms_key_cx(char *path_limits, int stop_on_fail, TestToDo *todo)
{
	...
	ret = readMutualSenseCompensationData(LOAD_CX_MS_KEY, &msCompData); //read MS compensation data
	if (ret < 0)
	{
		logError(1, "%s production_test_data: readMutualSenseCompensationData failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
		return (ret | ERROR_PROD_TEST_DATA);
	...
	if (todo->MutualKeyCx1 == 1)
	{
		...
		if (ret < 0 || (trows != 1 || tcolumns != 2))
		{
			logError(1, "%s production_test_data: parseProductionTestLimits MS_KEY_CX1_MIN_MAX failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
			ret |= ERROR_PROD_TEST_DATA;
			goto ERROR_LIMITS;
		}
	...
	{
		ret = parseProductionTestLimits(path_limits, &limit_file, MS_KEY_CX2_MAP_MIN, &thresholds_min, &trows, &tcolumns); //load min thresholds
		if (ret < 0 || (trows != msCompData.header.force_node  || tcolumns != msCompData.header.sense_node))
		{
			logError(1, "%s production_test_data: parseProductionTestLimits MS_KEY_CX2_MAP_MIN failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
			ret |= ERROR_PROD_TEST_DATA;
			goto ERROR_LIMITS;
		}

		ret = parseProductionTestLimits(path_limits, &limit_file, MS_KEY_CX2_MAP_MAX, &thresholds_max, &trows, &tcolumns); //load max thresholds
		if (ret < 0 || (trows != msCompData.header.force_node  || tcolumns != msCompData.header.sense_node ))
		{
			logError(1, "%s production_test_data: parseProductionTestLimits MS_KEY_CX2_MAP_MAX failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
			ret |= ERROR_PROD_TEST_DATA;
			goto ERROR_LIMITS;
		}
	...
	if (todo->MutualKeyCxTotal == 1)
	{
		...
		ret = parseProductionTestLimits(path_limits, &limit_file, MS_KEY_TOTAL_CX_MAP_MIN, &thresholds_min, &trows, &tcolumns); //load min thresholds
		if (ret < 0 || (trows != totCompData.header.force_node || tcolumns != totCompData.header.sense_node))
		{
			logError(1, "%s production_test_data: parseProductionTestLimits MS_KEY_TOTAL_CX_MAP_MIN failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
			ret |= ERROR_PROD_TEST_DATA;
			goto ERROR_LIMITS;
		}
		...
		if (ret < 0 || (trows != totCompData.header.force_node  || tcolumns != totCompData.header.sense_node))
		{
			logError(1, "%s production_test_data: parseProductionTestLimits MS_KEY_TOTAL_CX_MAP_MAX failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
			ret |= ERROR_PROD_TEST_DATA;
			goto ERROR_LIMITS;
		}
...
ERROR:
	...
	{
		print_frame_i8("MS Key Init Data (Cx2) =", array1dTo2d_i8(msCompData.node_data, msCompData.node_data_size, msCompData.header.sense_node), msCompData.header.force_node, msCompData.header.sense_node);
...
int production_test_ss_raw(char *path_limits, int stop_on_fail, TestToDo *todo)
{
	...
	if (ret < 0)
	{
		logError(1, "%s production_test_data: getSSFrame failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
		return (ret | ERROR_PROD_TEST_DATA);
	}
	...
	if (todo->SelfForceRaw == 1 || todo->SelfForceRawGap == 1)
	{
		...
		if (todo->SelfForceRaw == 1)
		{
			...
			if (ret < 0 || (trows != 1 || tcolumns != 2))
			{
				logError(1, "%s production_test_data: parseProductionTestLimits SS_RAW_FORCE_MIN_MAX failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
				ret |= ERROR_PROD_TEST_DATA;
				goto ERROR_LIMITS;
			}
			...
			if (ret != OK)
			{
				logError(1, "%s production_test_data: checkLimitsMinMax SS RAW FORCE failed... ERROR COUNT = %d \n", tag, ret);
				...
				logError(0, "%s SS RAW FORCE MIN MAX TEST:.................OK \n\n", tag);

			kfree(thresholds);
			thresholds = NULL;
		} else
			logError(0, "%s SS RAW FORCE MIN MAX TEST:.................SKIPPED \n\n", tag);
		...
		if (todo->SelfForceRawGap == 1)
		{
			...
			if (ret < 0 || (trows != 1 || tcolumns != 1))
			{
				logError(1, "%s production_test_data: parseProductionTestLimits SS_RAW_FORCE_GAP failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
				ret |= ERROR_PROD_TEST_DATA;
				goto ERROR_LIMITS;
			}
			...
			if (ret != OK)
			{
				logError(1, "%s production_test_data: checkLimitsGap SS RAW FORCE GAP failed... ERROR = %08X \n", tag, ret);
				logError(0, "%s SS RAW FORCE GAP TEST:.................FAIL \n\n", tag);
				count_fail += 1;
				print_frame_short("SS Raw force frame =", array1dTo2d_short(ssRawFrame.force_data, rows*columns, columns), rows, columns);
				if (stop_on_fail)
				{
				...
				logError(0, "%s SS RAW FORCE GAP TEST:.................OK \n\n", tag);

			kfree(thresholds);
			thresholds = NULL;
		} else
			logError(0, "%s SS RAW FORCE GAP TEST:.................SKIPPED \n\n", tag);

		kfree(ssRawFrame.force_data);
		ssRawFrame.force_data = NULL;
	} else
		logError(0, "%s SS RAW FORCE TEST:.................SKIPPED \n\n", tag);
	...
	if (todo->SelfSenseRaw == 1 || todo->SelfSenseRawGap == 1)
	{
		...
		if (todo->SelfSenseRaw == 1)
		{
			...
			if (ret < 0 || (trows != 1 || tcolumns != 2))
			{
				logError(1, "%s production_test_data: parseProductionTestLimits SS_RAW_SENSE_MIN_MAX failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
				ret |= ERROR_PROD_TEST_DATA;
				goto ERROR_LIMITS;
			}
			...
			if (ret != OK)
			{
				logError(1, "%s production_test_data: checkLimitsMinMax SS RAW SENSE failed... ERROR COUNT = %d \n", tag, ret);
				logError(0, "%s SS RAW SENSE MIN MAX TEST:.................FAIL \n", tag);
				count_fail += 1;
				...
				logError(0, "%s SS RAW SENSE MIN MAX TEST:.................OK \n", tag);

			kfree(thresholds);
			thresholds = NULL;
		} else
			logError(0, "%s SS RAW SENSE MIN MAX TEST:.................SKIPPED \n", tag);
		...
		if (todo->SelfSenseRawGap == 1)
		{
			...
			if (ret < 0 || (trows != 1 || tcolumns != 1))
			{
				logError(1, "%s production_test_data: parseProductionTestLimits SS_RAW_SENSE_GAP failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
				ret |= ERROR_PROD_TEST_DATA;
				goto ERROR_LIMITS;
			}
			...
			if (ret != OK)
			{
				logError(1, "%s production_test_data: checkLimitsGap SS RAW SENSE GAP failed... ERROR = %08X \n", tag, ret);
				logError(0, "%s SS RAW SENSE GAP TEST:.................FAIL \n", tag);
				count_fail += 1;
				print_frame_short("SS Raw sense frame =", array1dTo2d_short(ssRawFrame.sense_data, rows*columns, columns), rows, columns);
				if (stop_on_fail)
				{
				...
				logError(0, "%s SS RAW SENSE GAP TEST:.................OK \n", tag);

			kfree(thresholds);
			thresholds = NULL;
		} else
			logError(0, "%s SS RAW SENSE GAP TEST:.................SKIPPED \n", tag);
	...
	if (ret < OK)
	{
		logError(1, "%s production_test_data: production_test_ss_raw_lp failed... ERROR = %08X \n", tag, ret);
		count_fail += 1;
	}
...
int production_test_ss_raw_lp(char *path_limits, int stop_on_fail, TestToDo *todo)
{
	...
	if (todo->SelfForceRawLP == 1 || todo->SelfForceRawGapLP == 1)
	{
		...
		if (todo->SelfForceRawLP == 1)
		{
			...
			{
				logError(1, "%s production_test_data: checkLimitsMinMax SS RAW FORCE failed... ERROR COUNT = %d \n", tag, ret);
				logError(0, "%s SS RAW LP FORCE MIN MAX TEST:.................FAIL \n\n", tag);
				count_fail += 1;
				...
				logError(0, "%s SS RAW LP FORCE MIN MAX TEST:.................OK \n\n", tag);

			kfree(thresholds);
			thresholds = NULL;
		} else
			logError(0, "%s SS RAW LP FORCE MIN MAX TEST:.................SKIPPED \n\n", tag);
		...
		if (todo->SelfForceRawGapLP == 1)
		{
			...
			{
				logError(1, "%s production_test_data: checkLimitsGap SS RAW FORCE GAP failed... ERROR = %08X \n", tag, ret);
				logError(0, "%s SS RAW LP FORCE GAP TEST:.................FAIL \n\n", tag);
				count_fail += 1;
				print_frame_short("SS Raw LP force frame =", array1dTo2d_short(ssRawFrame.force_data, rows*columns, columns), rows, columns);
				if (stop_on_fail)
				{
				...
				logError(0, "%s SS RAW LP FORCE GAP TEST:.................OK \n\n", tag);

			kfree(thresholds);
			thresholds = NULL;
		} else
			logError(0, "%s SS RAW LP FORCE GAP TEST:.................SKIPPED \n\n", tag);

		kfree(ssRawFrame.force_data);
		ssRawFrame.force_data = NULL;
	} else
		logError(0, "%s SS RAW LP FORCE TEST:.................SKIPPED \n\n", tag);
	...
	if (todo->SelfSenseRawLP == 1 || todo->SelfSenseRawGapLP == 1)
	{
		...
		if (todo->SelfSenseRawLP == 1)
		{
			...
			{
				logError(1, "%s production_test_data: checkLimitsMinMax SS RAW SENSE failed... ERROR COUNT = %d \n", tag, ret);
				logError(0, "%s SS RAW LP SENSE MIN MAX TEST:.................FAIL \n", tag);
				count_fail += 1;
			...
			logError(0, "%s SS RAW LP SENSE MIN MAX TEST:.................SKIPPED \n", tag);
		...
		if (todo->SelfSenseRawGapLP == 1)
		{
			...
			{
				logError(1, "%s production_test_data: checkLimitsGap SS RAW SENSE GAP failed... ERROR = %08X \n", tag, ret);
				logError(0, "%s SS RAW LP SENSE GAP TEST:.................FAIL \n", tag);
				count_fail += 1;
				print_frame_short("SS Raw LP sense frame =", array1dTo2d_short(ssRawFrame.sense_data, rows*columns, columns), rows, columns);
				if (stop_on_fail)
				{
				...
				logError(0, "%s SS RAW LP SENSE GAP TEST:.................OK \n", tag);

			kfree(thresholds);
			thresholds = NULL;
		} else
			logError(0, "%s SS RAW LP SENSE GAP TEST:.................SKIPPED \n", tag);
	...
	if (count_fail == 0)
	{
		logError(0, "%s SS RAW LP testes finished!.................OK\n\n", tag);
		return OK;
	} else
	{
...
int production_test_ss_ix_cx(char *path_limits, int stop_on_fail, TestToDo *todo)
{
	...
	ret = readSelfSenseCompensationData(LOAD_CX_SS_TOUCH, &ssCompData); //read the SS compensation data
	if (ret < 0)
	{
		logError(1, "%s production_test_data: readSelfSenseCompensationData failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
		return (ret | ERROR_PROD_TEST_DATA);
	}

	ret = readTotSelfSenseCompensationData(LOAD_PANEL_CX_TOT_SS_TOUCH, &totCompData); //read the TOT SS compensation data
	if (ret < 0)
	{
		logError(1, "%s production_test_data: readTotSelfSenseCompensationData failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
	...
	if (todo->SelfForceIx1 == 1)
	{
		...
		if (ret < 0 || (trows != 1 || tcolumns != 2))
		{
			logError(1, "%s production_test_data: parseProductionTestLimits SS_IX1_FORCE_MIN_MAX failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
			ret |= ERROR_PROD_TEST_DATA;
			goto ERROR_LIMITS;
		}
	...
	if (todo->SelfForceIx2 == 1)
	{
		...
		{
			logError(1, "%s production_test_data: parseProductionTestLimits SS_IX2_FORCE_MAP_MIN failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
			ret |= ERROR_PROD_TEST_DATA;
			goto ERROR_LIMITS;
		}
		...
		if (ret < 0 || (trows != ssCompData.header.force_node || tcolumns != 1))
		{
			logError(1, "%s production_test_data: parseProductionTestLimits SS_IX2_FORCE_MAP_MAX failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
			ret |= ERROR_PROD_TEST_DATA;
			goto ERROR_LIMITS;
		}
	...
	if (todo->SelfForceIx2Adj == 1)
	{
		...
		if (ret < 0)
		{
			logError(1, "%s production_test_data: computeAdjVert SS IX2 FORCE ADJV failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
			ret |= ERROR_PROD_TEST_DATA;
			goto ERROR_LIMITS;
		}
		...
		{
			logError(1, "%s production_test_data: parseProductionTestLimits SS_IX2_FORCE_ADJV_MAP_MAX failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
			ret |= ERROR_PROD_TEST_DATA;
			goto ERROR_LIMITS;
		}
	...
	if (todo->SelfForceIxTotal == 1 || todo->SelfForceIxTotalAdj == 1)
	{
		...
		if (todo->SelfForceIxTotal == 1)
		{
			...
			{
				logError(1, "%s production_test_data: parseProductionTestLimits SS_TOTAL_IX_FORCE_MAP_MIN failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
				ret |= ERROR_PROD_TEST_DATA;
				goto ERROR_LIMITS;
			}
			...
			if (ret < 0 || (trows != totCompData.header.force_node || tcolumns != 1))
			{
				logError(1, "%s production_test_data: parseProductionTestLimits SS_TOTAL_IX_FORCE_MAP_MAX failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
				ret |= ERROR_PROD_TEST_DATA;
				goto ERROR_LIMITS;
			}
		...
		if (todo->SelfForceIxTotalAdj == 1)
		{
			...
			if (ret < 0)
			{
				logError(1, "%s production_test_data: computeAdjVert SS TOTAL IX FORCE ADJV failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
				ret |= ERROR_PROD_TEST_DATA;
				goto ERROR_LIMITS;
			}
			...
			{
				logError(1, "%s production_test_data: parseProductionTestLimits SS_TOTAL_IX_FORCE_ADJV_MAP_MAX... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
				ret |= ERROR_PROD_TEST_DATA;
				goto ERROR_LIMITS;
			}
	...
	if (todo->SelfSenseIx1 == 1)
	{
		...
		if (ret < 0 || (trows != 1 || tcolumns != 2))
		{
			logError(1, "%s production_test_data: parseProductionTestLimits SS_IX1_SENSE_MIN_MAX failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
			ret |= ERROR_PROD_TEST_DATA;
			goto ERROR_LIMITS;
		}
	...
	if (todo->SelfSenseIx2 == 1)
	{
		...
		{
			logError(1, "%s production_test_data: parseProductionTestLimits SS_IX2_SENSE_MAP_MIN failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
			ret |= ERROR_PROD_TEST_DATA;
			goto ERROR_LIMITS;
		}
		...
		if (ret < 0 || (trows != 1 || tcolumns != ssCompData.header.sense_node))
		{
			logError(1, "%s production_test_data: parseProductionTestLimits SS_IX2_SENSE_MAP_MAX failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
			ret |= ERROR_PROD_TEST_DATA;
			goto ERROR_LIMITS;
		}
	...
	if (todo->SelfSenseIx2Adj == 1)
	{
		...
		if (ret < 0)
		{
			logError(1, "%s production_test_data: computeAdjHoriz SS IX2 SENSE ADJH failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
			ret |= ERROR_PROD_TEST_DATA;
			goto ERROR_LIMITS;
		}
		...
		{
			logError(1, "%s production_test_data: parseProductionTestLimits SS_IX2_SENSE_ADJH_MAP_MAX failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
			ret |= ERROR_PROD_TEST_DATA;
			goto ERROR_LIMITS;
		}
	...
	if (todo->SelfSenseIxTotal == 1 || todo->SelfSenseIxTotalAdj == 1)
	{
		...
		if (todo->SelfSenseIxTotal == 1)
		{
			...
			{
				logError(1, "%s production_test_data: parseProductionTestLimits SS_TOTAL_IX_SENSE_MAP_MIN failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
				ret |= ERROR_PROD_TEST_DATA;
				goto ERROR_LIMITS;
			}
			...
			if (ret < 0 || (trows != 1 || tcolumns != totCompData.header.sense_node))
			{
				logError(1, "%s production_test_data: parseProductionTestLimits SS_TOTAL_IX_SENSE_MAP_MAX failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
				ret |= ERROR_PROD_TEST_DATA;
				goto ERROR_LIMITS;
			}
		...
		if (todo->SelfSenseIxTotalAdj == 1)
		{
			...
			if (ret < 0)
			{
				logError(1, "%s production_test_data: computeAdjHoriz SS TOTAL IX SENSE ADJH failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
				ret |= ERROR_PROD_TEST_DATA;
				goto ERROR_LIMITS;
			}
			...
			{
				logError(1, "%s production_test_data: parseProductionTestLimits SS_TOTAL_IX_SENSE_ADJH_MAP_MAX failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
				ret |= ERROR_PROD_TEST_DATA;
				goto ERROR_LIMITS;
			}
	...
	if (todo->SelfForceCx1 == 1)
	{
		...
		if (ret < 0 || (trows != 1 || tcolumns != 2))
		{
			logError(1, "%s production_test_data: parseProductionTestLimits SS_CX1_FORCE_MIN_MAX failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
			ret |= ERROR_PROD_TEST_DATA;
			goto ERROR_LIMITS;
		}
	...
	if (todo->SelfForceCx2 == 1)
	{
		...
		if (ret < 0 || (trows != ssCompData.header.force_node || tcolumns != 1))
		{
			logError(1, "%s production_test_data: parseProductionTestLimits SS_CX2_FORCE_MAP_MIN failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
			ret |= ERROR_PROD_TEST_DATA;
			goto ERROR_LIMITS;
		}
		...
		if (ret < 0 || (trows != ssCompData.header.force_node || tcolumns != 1))
		{
			logError(1, "%s production_test_data: parseProductionTestLimits SS_CX2_FORCE_MAP_MAX failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
			ret |= ERROR_PROD_TEST_DATA;
			goto ERROR_LIMITS;
		}
	...
	if (todo->SelfForceCx2Adj == 1)
	{
		...
		if (ret < 0)
		{
			logError(1, "%s production_test_data: computeAdjVert SS CX2 FORCE ADJV failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
			ret |= ERROR_PROD_TEST_DATA;
			goto ERROR_LIMITS;
		}
		...
		if (ret < 0 || (trows != ssCompData.header.force_node - 1 || tcolumns != 1))
		{
			logError(1, "%s production_test_data: parseProductionTestLimits SS_CX2_FORCE_ADJV_MAP_MAX failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
			ret |= ERROR_PROD_TEST_DATA;
			goto ERROR_LIMITS;
		}
	...
	if (todo->SelfForceCxTotal == 1 || todo->SelfForceCxTotalAdj == 1)
	{
		...
		if (todo->SelfForceCxTotal == 1)
		{
			...
			if (ret < 0 || (trows != totCompData.header.force_node || tcolumns != 1))
			{
				logError(1, "%s production_test_data: parseProductionTestLimits SS_TOTAL_CX_FORCE_MAP_MIN failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
				ret |= ERROR_PROD_TEST_DATA;
				goto ERROR_LIMITS;
			}
			...
			if (ret < 0 || (trows != totCompData.header.force_node || tcolumns != 1))
			{
				logError(1, "%s production_test_data: parseProductionTestLimits SS_TOTAL_CX_FORCE_MAP_MAX failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
				ret |= ERROR_PROD_TEST_DATA;
				goto ERROR_LIMITS;
			}
		...
		if (todo->SelfForceCxTotalAdj == 1)
		{
			...
			ret = computeAdjVertTotal(totCompData.cx_fm, totCompData.header.force_node, 1, &total_adjvert); //comepute the ADJV for CX2  FORCE
			if (ret < 0)
			{
				logError(1, "%s production_test_data: computeAdjVert SS TOTAL CX FORCE ADJV failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
				ret |= ERROR_PROD_TEST_DATA;
				goto ERROR_LIMITS;
			}
			...
			if (ret < 0 || (trows != totCompData.header.force_node - 1 || tcolumns != 1))
			{
				logError(1, "%s production_test_data: parseProductionTestLimits SS_TOTAL_CX_FORCE_ADJV_MAP_MAX failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
				ret |= ERROR_PROD_TEST_DATA;
				goto ERROR_LIMITS;
			}
	...
	if (todo->SelfSenseCx1 == 1)
	{
		...
		if (ret < 0 || (trows != 1 || tcolumns != 2))
		{
			logError(1, "%s production_test_data: parseProductionTestLimits SS_CX1_SENSE_MIN_MAX failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
			ret |= ERROR_PROD_TEST_DATA;
			goto ERROR_LIMITS;
		}
	...
	if (todo->SelfSenseCx2 == 1)
	{
		...
		if (ret < 0 || (trows != 1 || tcolumns != ssCompData.header.sense_node))
		{
			logError(1, "%s production_test_data: parseProductionTestLimits SS_CX2_SENSE_MAP_MIN failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
			ret |= ERROR_PROD_TEST_DATA;
			goto ERROR_LIMITS;
		}
		...
		if (ret < 0 || (trows != 1 || tcolumns != ssCompData.header.sense_node))
		{
			logError(1, "%s production_test_data: parseProductionTestLimits SS_CX2_SENSE_MAP_MAX failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
			ret |= ERROR_PROD_TEST_DATA;
			goto ERROR_LIMITS;
		}
	...
	if (todo->SelfSenseCx2Adj == 1)
	{
		...
		if (ret < 0)
		{
			logError(1, "%s production_test_data: computeAdjHoriz SS CX2 SENSE ADJH failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
			ret |= ERROR_PROD_TEST_DATA;
			goto ERROR_LIMITS;
		}
	...
	if (todo->SelfSenseCxTotal == 1 || todo->SelfSenseCxTotalAdj == 1)
	{
		...
		if (todo->SelfSenseCxTotal == 1)
		{
			...
			if (ret < 0 || (trows != 1 || tcolumns != totCompData.header.sense_node))
			{
				logError(1, "%s production_test_data: parseProductionTestLimits SS_TOTAL_CX_SENSE_MAP_MIN failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
				ret |= ERROR_PROD_TEST_DATA;
				goto ERROR_LIMITS;
			}
			...
			if (ret < 0 || (trows != 1 || tcolumns != totCompData.header.sense_node))
			{
				logError(1, "%s production_test_data: parseProductionTestLimits SS_TOTAL_CX_SENSE_MAP_MAX failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
				ret |= ERROR_PROD_TEST_DATA;
				goto ERROR_LIMITS;
			}
		...
		if (todo->SelfSenseCxTotalAdj == 1)
		{
			...
			if (ret < 0)
			{
				logError(1, "%s production_test_data: computeAdjHoriz SS TOTAL CX SENSE ADJH failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
				ret |= ERROR_PROD_TEST_DATA;
				goto ERROR_LIMITS;
			}
			...
			if (ret < 0 || (trows != 1 || tcolumns != totCompData.header.sense_node - 1))
			{
				logError(1, "%s production_test_data: parseProductionTestLimits SS_TOTAL_CX_SENSE_ADJH_MAP_MAX failed... ERROR %08X \n", tag, ERROR_PROD_TEST_DATA);
				ret |= ERROR_PROD_TEST_DATA;
				goto ERROR_LIMITS;
			}
...
ERROR:
	...
	} else
	{
		...
		if (totCompData.ix_fm != NULL) kfree(totCompData.ix_fm);
		if (totCompData.ix_sn != NULL) kfree(totCompData.ix_sn);
		if (totCompData.cx_fm != NULL) kfree(totCompData.cx_fm);
		if (totCompData.cx_sn != NULL) kfree(totCompData.cx_sn);
		return (ERROR_TEST_CHECK_FAIL | ERROR_PROD_TEST_DATA);
	}
...
int production_test_data(char *path_limits, int stop_on_fail, TestToDo *todo)
{
	...
	if (todo == NULL)
	{
		logError(1, "%s production_test_data: No TestToDo specified!! ERROR = %08X \n", tag, (ERROR_OP_NOT_ALLOW | ERROR_PROD_TEST_DATA));
	...
	if (ret < 0)
	{
		logError(1, "%s production_test_data: production_test_ms_raw failed... ERROR = %08X \n", tag, ret);
	...
	if (ret < 0)
	{
		logError(1, "%s production_test_data: production_test_ms_cx failed... ERROR = %08X \n", tag, ret);
		if (stop_on_fail == 1) goto END;
	}
	...
	if (ret < 0)
	{
		logError(1, "%s production_test_data: production_test_ss_raw failed... ERROR = %08X \n", tag, ret);
		if (stop_on_fail == 1) goto END;
	}
	...
	if (ret < 0)
	{
		logError(1, "%s production_test_data: production_test_ss_ix_cx failed... ERROR = %08X \n", tag, ret);
		if (stop_on_fail == 1) goto END;
	}
...
int getLimitsFile(char *path, LimitFile *file)
{
	...
	if (file->data != NULL)
	{ //to avoid memory leak on consecutive call of the function with the same pointer
		logError(0, "%s Pointer to Limits Data already contains something... freeing its content!\n", tag);
		kfree(file->data);
		file->data = NULL;
		file->size = 0;
	}
	...
	if (strncmp(path, "NULL", 4) == 0)
	{
		...
		if (file->data != NULL)
		{
			memcpy(file->data, (char *) (LIMITS_ARRAY_NAME), file->size);
			return OK;
		} else
		{
		...
		if (dev != NULL)
		{
			...
			if (fd == 0)
			{
				...
				if (file->data != NULL)
				{
					...
					logError(0, "%s Limit file Size = %d \n", tag, file->size);
					release_firmware(fw);
					return OK;
				} else
				{
			...
			{
				logError(1, "%s Request the file %s failed... ERROR %08X \n", tag, path, ERROR_FILE_NOT_FOUND);
				return ERROR_FILE_NOT_FOUND;
			}
		} else
		{
...
int parseProductionTestLimits(char *path, LimitFile *file, char *label, int **data, int *row, int *column)
{
	...
	while (find == 0)
	{
		...
		{ //each header row start with *  ex. *label,n_row,n_colum
			...
			if (line2 == NULL)
			{
				logError(1, "%s parseProductionTestLimits: kstrdup ERROR %08X\n", tag, ERROR_ALLOC);
				ret = ERROR_ALLOC;
				goto END;
			}
			buf = line2;
			...
			{ //if the row is the wanted one i retrieve rows and columns info
				...
				{
					logError(1, "%s parseProductionTestLimits 1: ERROR %08X\n", tag, ERROR_FILE_PARSE);
				...
				{
					logError(1, "%s parseProductionTestLimits 2: ERROR %08X\n", tag, ERROR_FILE_PARSE);
					ret = ERROR_FILE_PARSE;
					goto END;
				}
				...
				*data = (int *) kmalloc(((*row)*(*column)) * sizeof (int), GFP_KERNEL); //allocate the memory for containing the data
				...
				logError(1, "%s parseProductionTestLimits 3: ERROR %08X\n", tag, ERROR_FILE_PARSE);
				ret = ERROR_FILE_PARSE;
				goto END;
			}
			kfree(buf);
