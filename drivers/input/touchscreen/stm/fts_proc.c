...
#define CMD_GETLIMITSFILE									0x08				///< Get the Production Limits File and print its content into the shell: need to pass: path(optional) otherwise select the approach chosen at compile time
#define CMD_GETFWFILE										0x09				///< Get the FW file and print its content into the shell: need to pass: path (optional) otherwise select the approach chosen at compile time
#define CMD_VERSION											0x0A				///< Get the driver version and other driver setting info
#define CMD_READCONFIG										0x0B				///< Read The config memory, need to pass: addr[0] addr[1] byteToRead0 byteToRead1


//GUI utils byte ver
#define CMD_READ_BYTE										0xF0				///< Byte output version of I2C/SPI read @see CMD_READ
...
#define CMD_WRITETHENWRITEREAD_BYTE							0xF3				///< Byte output version of I2C/SPI write then writeRead @see CMD_WRITETHENWRITEREAD
#define CMD_WRITEU8UX_BYTE									0xF4				///< Byte output version of I2C/SPI writeU8UX @see CMD_WRITEU8UX
#define CMD_WRITEREADU8UX_BYTE								0xF5				///< Byte output version of I2C/SPI writeReadU8UX @see CMD_WRITEREADU8UX
#define CMD_WRITEU8UXTHENWRITEU8UX_BYTE						0xF6				///< Byte output version of I2C/SPI writeU8UX then writeU8UX @see CMD_WRITEU8UXTHENWRITEU8UX
#define CMD_WRITEU8UXTHENWRITEREADU8UX_BYTE					0xF7				///< Byte output version of I2C/SPI writeU8UX  then writeReadU8UX @see CMD_WRITEU8UXTHENWRITEREADU8UX
...
#define CMD_INITTEST										0x51				///< Perform an Initialization test: need to pass: type
#define CMD_MSRAWTEST										0x52				///< Perform MS raw test: need to pass stop_on_fail
#define CMD_MSINITDATATEST									0x53				///< Perform MS Init data test: need to pass stop_on_fail
 ...
 * } = end byte \n
 */
static ssize_t fts_driver_test_write(struct file *file, const char __user *buf, size_t count, loff_t *pos)
{
	int numberParam = 0;
	...
	if (numberParam >= 1)
	{
		...
		case CMD_WRITEREAD_BYTE:
			...
			{ //need to pass: cmd[0]  cmd[1] … cmd[cmdLength-1] byteToRead1 byteToRead0 dummyByte
				...
				readData = (u8*) kmalloc((byteToRead+mess.dummy) * sizeof (u8), GFP_KERNEL);
		...
		case CMD_READ_BYTE:
			if (numberParam >= 3)
			{ //need to pass: byteToRead1 byteToRead0 (optional) dummyByte
		...
		case CMD_WRITEU8UXTHENWRITEREADU8UX_BYTE:
			...
			if (numberParam >= 8)
			{
				...
				{
					u8ToU64_be(&cmd[5], &addr, cmd[2] + cmd[4]);
					logError(1, "%s %s: cmd[5] = %02X, addr =  %016X \n", tag, __func__,cmd[5], addr);
					u8ToU16_be(&cmd[numberParam - 3], &byteToRead);
					readData = (u8*) kmalloc(byteToRead * sizeof (u8), GFP_KERNEL);
				...
				{
					logError(1, "%s Wrong total address size! \n", tag);
					res = ERROR_OP_NOT_ALLOW;
				}
		...
		case CMD_POLLFOREVENT:
			if (numberParam >= 5)
			{ //need to pass: eventLength event[0] event[1] … event[eventLength-1] timeTowait1 timeTowait0
		...
		case CMD_REQCOMPDATA: //request comp data
			if (numberParam == 2)
			{
				...
				if (res < OK)
				{
					logError(0, "%s Error requesting compensation data ERROR %08X \n", tag, res);
				} else
				{
		...
		case CMD_READCOMPDATAHEAD: //read comp data header
			if (numberParam == 2)
			{
				...
				} else
				{
					...
					if (res < OK)
					{
						logError(0, "%s Read Compensation Data Header ERROR %08X\n", tag, res);
					} else
					{
		...
		case CMD_READSSCOMPDATA:
			...
			{ //read self comp data
				...
				} else
				{
					...
					print_frame_i8("SS Data Ix2_fm = ", array1dTo2d_i8(comData.ix2_fm, comData.header.force_node, comData.header.force_node), 1, comData.header.force_node);
					print_frame_i8("SS Data Cx2_fm = ", array1dTo2d_i8(comData.cx2_fm, comData.header.force_node, comData.header.force_node), 1, comData.header.force_node);
					print_frame_i8("SS Data Ix2_sn = ", array1dTo2d_i8(comData.ix2_sn, comData.header.sense_node, comData.header.sense_node), 1, comData.header.sense_node);
		...
		case CMD_READTOTMSCOMPDATA: //read mutual comp data
			if (numberParam == 2)
			{
				logError(0, "%s Get TOT MS Compensation Data \n", tag);
				res = readTotMutualSenseCompensationData(cmd[1], &totCompData);

				if (res < OK)
				{
					logError(0, "%s Error reading TOT MS compensation data ERROR %08X \n", tag, res);
				} else
				{
					logError(0, "%s TOT MS Compensation Data Reading Finished! \n", tag);
					size = (totCompData.node_data_size*sizeof (short) + 9);
					print_frame_short("MS Data (TOT Cx) =", array1dTo2d_short(totCompData.node_data, totCompData.node_data_size, totCompData.header.sense_node), totCompData.header.force_node, totCompData.header.sense_node);
				}
			} else
			{
				logError(1, "%s Wrong number of parameters! \n", tag);
		...
		case CMD_READTOTSSCOMPDATA:
			if (numberParam == 2)
			{ //read self comp data
				logError(0, "%s Get TOT SS Compensation Data... \n", tag);
				res = readTotSelfSenseCompensationData(cmd[1], &totComData);
				if (res < OK)
				{
					logError(0, "%s Error reading TOT SS compensation data ERROR %08X\n", tag, res);
				} else
				{
					logError(0, "%s TOT SS Compensation Data Reading Finished! \n", tag);
					size = ((totComData.header.force_node + totComData.header.sense_node)*2*sizeof(short) + 9);
					print_frame_u16("SS Data TOT Ix_fm = ", array1dTo2d_u16(totComData.ix_fm, totComData.header.force_node, totComData.header.force_node), 1, totComData.header.force_node);
					print_frame_short("SS Data TOT Cx_fm = ", array1dTo2d_short(totComData.cx_fm, totComData.header.force_node, totComData.header.force_node), 1, totComData.header.force_node);
					print_frame_u16("SS Data TOT Ix_sn = ", array1dTo2d_u16(totComData.ix_sn, totComData.header.sense_node, totComData.header.sense_node), 1, totComData.header.sense_node);
					print_frame_short("SS Data TOT Cx_sn = ", array1dTo2d_short(totComData.cx_sn, totComData.header.sense_node, totComData.header.sense_node), 1, totComData.header.sense_node);
				}
			} else
			{
				logError(1, "%s Wrong number of parameters! \n", tag);
		...
		case CMD_FLASHERASEUNLOCK:
			...
			if (res < OK)
			{
				logError(0, "%s Error during flash erase unlock... ERROR %08X\n", tag, res);
			} else
			{
		...
		case CMD_FLASHERASEPAGE:
			...
			{ //need to pass: keep_cx
				logError(0, "%s Starting Flashing Page Erase... \n", tag);
				res = flash_erase_page_by_page(cmd[1]);
				if (res < OK)
				{
					logError(0, "%s Error during flash page erase... ERROR %08X\n", tag, res);
				} else
				{
					logError(0, "%s Flash Page Erase Finished! \n", tag);
				}
			} else
			{
				logError(1, "%s Wrong number of parameters! \n", tag);
		...
		case CMD_MAINTEST:
			if (numberParam == 3) //need to specify if stopOnFail and saveInit
				res = production_test_main(LIMITS_FILE, cmd[1], cmd[2], &tests);
			else
			{
				logError(1, "%s Wrong number of parameters! \n", tag);
				res = ERROR_OP_NOT_ALLOW;
		...
		case CMD_DIAGNOSTIC:
			...
			readData = (u8 *) kmalloc((ERROR_DUMP_ROW_SIZE*ERROR_DUMP_COL_SIZE)*sizeof(u8), GFP_KERNEL);
			if(driver_test_buff== NULL || readData==NULL){
				res = ERROR_ALLOC;
				logError(1, "%s Impossible allocate memory for buffers! ERROR %08X! \n", tag, res);
				goto END;
			}
			...
			if(temp!= ((DCHIP_ID_1<<8)|DCHIP_ID_0)){
				logError(1, "%s Wrong CHIP ID, Diagnostic failed! \n", tag, res);
				res = ERROR_OP_NOT_ALLOW;
				goto END_DIAGNOSTIC;
			}
			...
			for(temp=0;temp<DIAGNOSTIC_NUM_FRAME;temp++){
				...
				for(addr=0; addr<3;addr++){
					...
					if(frameMS.node_data!=NULL) kfree(frameMS.node_data);
				}
				for(addr=0; addr<3;addr++){
					switch(addr){
						case 0:
					...
					if(frameSS.force_data!=NULL) kfree(frameSS.force_data);
			...
			if(temp<OK){
				...
				for(temp=0; temp<ERROR_DUMP_ROW_SIZE*ERROR_DUMP_COL_SIZE; temp++){
...
END: //here start the reporting phase, assembling the data to send in the file node
	if (driver_test_buff != NULL)
	{
		logError(1, "%s Consecutive echo on the file node, free the buffer with the previous result\n", tag);
		kfree(driver_test_buff);
	}
	...
	if(driver_test_buff == NULL){
		logError(0, "%s Unable to allocate driver_test_buff! ERROR %08X\n", tag, ERROR_ALLOC);
		goto ERROR;
	}
		...
		if (res >= OK)
		{
			...
			case CMD_POLLFOREVENT:
				...
				for (; j < byteToRead+mess.dummy; j++)
				{
					...
					snprintf(&driver_test_buff[index], 3, "%02X", readData[j]);		//this approach is much more faster
					index+=2;
				}
				//logError(0, "\n");
				break;
	...
	} else
	{
		...
		case CMD_GETFWFILE_BYTE:
			if(bin_output==1){
				...
				{
					logError(0, "%s readData = NULL... returning junk data!", tag);
				}
				index += addr; //in this case the byte to read are stored in addr because it is a u64 end byte need to be inserted at the end of the padded memory
			}else{
				//snprintf(&driver_test_buff[1], 3, "%02X", (((fileSize + 3) / 4)&0xFF00) >> 8);
				//snprintf(&driver_test_buff[3], 3, "%02X", ((fileSize + 3) / 4)&0x00FF);
				for(j=0;j<fileSize; j++){
					index+=snprintf(&driver_test_buff[index], 3, "%02X", (u8) readData[j]);
				}
				for(;j<addr;j++)
...
ERROR:
	numberParam = 0; //need to reset the number of parameters in order to wait the next command, comment if you want to repeat the last comand sent just doing a cat
	//logError(0,"%s numberParameters = %d \n",tag, numberParam);
	if(readData!=NULL) kfree(readData);
