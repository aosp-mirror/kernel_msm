...
static int fts_cmd_completion_check(struct fts_ts_info *info, uint8_t event1, uint8_t event2, uint8_t event3);
...
static int fts_panel_ito_test(struct fts_ts_info *info)
{
	...
	unsigned char wregAdd[3] = {0xA7, 0x01, 0x00}; //H/W
	uint8_t	*errortypes[16] = {"F open", "S open", "F2G short", "S2G short", "F2V short", "S2V short", "F2F short", "S2S short",
							"F2S short", "FPC F open", "FPC S open", "Key F open", "Key S open", "Reserved", "Reserved", "Reserved"};
	int retry = 0;
	int result = -1;
		...
		if ((data[0] == 0x0F) && (data[1] == 0x05)) {
			switch (data[2]) {
				case	NO_ERROR:
					if (data[3] == 0x00)
					{
						printk("ITO open / short test PASS!!\n");
						return	1;
					}
					break;
				case ITO_FORCE_OPEN:
				...
				case ITO_MAX_ERR_REACHED:
					printk("ITO open / short test FAIL!! Error Type : %s, Channel : %d", errortypes[data[2]], data[3]);
					break;
			}
			break;
		}
		 ...
		 short *max)
{
	...
	tsp_debug_info(true, &info->client->dev, "===> fts_read_frame digital rev( %d ) sense ( %d ) force (%d) \n",
...
#endif
		if (info->digital_rev == FTS_DIGITAL_REV_1) {
			info->fts_read_reg(info, &pFrameAddress[0], 3, pRead, readbytes);
			remained -= readbytes;
			...
			info->fts_read_reg(info, &pFrameAddress[0], 3, pRead, readbytes + 1);
			remained -= readbytes;
...
static void fts_read_ix_data(struct fts_ts_info *info, bool allnode)
{
	...
	if (mbuff != NULL) {
		...
		for (i = 0; i < info->ForceChannelLength; i++) {
			...
			*pBuf++ = '0';
			*pBuf++ = ',';
			tsp_debug_info(true, &info->client->dev, "Force[%d] %d\n", i, force_ix_data[i]);
		}
		for (i = 0; i < info->SenseChannelLength; i++) {
		num =  sense_ix_data[i];
		n = 100000;
		...
		*pBuf++ = '0';
		if (i < (info->SenseChannelLength - 1))
			*pBuf++ = ',';
		tsp_debug_info(true, &info->client->dev, "Sense[%d] %d\n", i, sense_ix_data[i]);
		}
...
//	fts_command(info, SLEEPIN); // Sleep In for INT disable
	...
	info->fts_read_reg(info, regAdd, 3, &ReadData[0], info->SenseChannelLength * 2 + 1);
	...
	if(allnode == true){
		buff_size = (info->ForceChannelLength + info->SenseChannelLength + 2)*10;
		mbuff = kzalloc(buff_size, GFP_KERNEL);
	}
	if (mbuff != NULL) {
		char *pBuf = mbuff;
...
static void run_cx_data_read(void *device_data)
{
	...
	if (pStr == NULL) {
		tsp_debug_info(true, &info->client->dev, "%s: pStr kzalloc failed\n", __func__);
		return;
	}
	...
	disable_irq(info->irq);
	tsp_debug_info(true, &info->client->dev, "%s: disable_irq\n", __func__);
	info->fts_interrupt_set(info, INT_DISABLE);
	tsp_debug_info(true, &info->client->dev, "%s: fts_interrupt_set\n", __func__);
	info->fts_command(info, SENSEOFF);
	tsp_debug_info(true, &info->client->dev, "%s: fts_command\n", __func__);
	fts_delay(50);
	tsp_debug_info(true, &info->client->dev, "%s: senseoff\n", __func__);
	...
	regAdd[2] = 0x00;
	info->fts_write_reg(info, &regAdd[0], 3);
	tsp_debug_info(true, &info->client->dev, "%s: Writing Request compensation data\n", __func__);
	fts_cmd_completion_check(info, EVENTID_STATUS_REQUEST_COMP, regAdd[1], regAdd[2]);
	...
	comp_header_addr = buff[1] + (buff[2] << 8);
	tsp_debug_info(true, &info->client->dev, "%s:Read an address of compensation data\n", __func__);
	...
	tsp_debug_info(true, &info->client->dev, "%s:Read header of compensation area data\n", __func__);
	tsp_debug_info(true, &info->client->dev, "%s: Tx num ( %d ) Rx num ( %d ) \n", __func__, tx_num, rx_num);
	tsp_debug_info(true, &info->client->dev, "%s: comp_stat_addr : 0x%x\n", __func__, comp_start_addr);
	...
	tsp_debug_info(true, &info->client->dev, "%s:Read compensation data\n", __func__);
	if (info->cx_data) {
	for (j = 0; j < tx_num; j++) {
		for(i = 1; i < rx_num + 1; i++)
			//info->cx_data[(j * rx_num) + i] = ReadData[j][i];
			info->cx_data[(j * rx_num) + i -1] = ReadData[j][i];
		}
	}
	tsp_debug_err(true, &info->client->dev, "===================> %s : Rx diff \n", __func__);
		for(j = 0; j < tx_num; j++) {
				memset(pStr, 0x0, 4 * (rx_num + 1));
				snprintf(pTmp, sizeof(pTmp), "Tx%02d | ", j);
				strncat(pStr, pTmp, 16);
				for(i = 0; i < (rx_num-1); i++) {
					cxdiffData_rx[(j*rx_num)+i] = info->cx_data[(j*rx_num)+i] - info->cx_data[(j*rx_num)+i+1];
					if((j==0)&&(i==0)){
		...
		tsp_debug_err(true, &info->client->dev, "======>  Max : %d / Low : %d <=========\n", Max_cxdiffData_rx, Low_cxdiffData_rx);
		tsp_debug_err(true, &info->client->dev, " %s : Tx diff \n  <=======================", __func__);
			for(j = 0; j < (tx_num -1); j++) {
				memset(pStr, 0x0, 4 * (rx_num + 1));
				snprintf(pTmp, sizeof(pTmp), "Tx%02d | ", j);
				strncat(pStr, pTmp, 16);
				for(i = 0; i < rx_num; i++) {
					cxdiffData_tx[(j*rx_num)+i] = info->cx_data[(j*rx_num)+i] - info->cx_data[((j+1)*rx_num)+i];
					if((j==0)&&(i==0)){
