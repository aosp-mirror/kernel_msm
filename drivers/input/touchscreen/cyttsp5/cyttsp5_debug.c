...
static const DEVICE_ATTR(int_count, S_IRUSR | S_IWUSR,
...
static const DEVICE_ATTR(formated_output, S_IRUSR | S_IWUSR,
	cyttsp5_formated_output_show, cyttsp5_formated_output_store);

static const struct attribute *cyttsp5_debug_attributes[] = {
	&dev_attr_int_count.attr,
	&dev_attr_formated_output.attr,
	NULL
};

static const struct attribute_group cyttsp5_debug_attribute_group = {
	.attrs = (struct attribute **) cyttsp5_debug_attributes
};
...
static int cyttsp5_debug_probe(struct device *dev, void **data)
{
	...
	rc = sysfs_create_group(&dev->kobj, &cyttsp5_debug_attribute_group);
	if (rc) {
		dev_err(dev, "%s: Error, could not create sysfs group\n",
				__func__);
		goto cyttsp5_debug_probe_create_sysfs_failed;
	}
...
cyttsp5_debug_probe_sysinfo_failed:
	sysfs_remove_group(&dev->kobj, &cyttsp5_debug_attribute_group);
cyttsp5_debug_probe_create_sysfs_failed:
	kfree(dd);
