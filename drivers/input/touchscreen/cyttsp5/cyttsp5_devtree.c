...
static void *create_and_get_mt_pdata(struct device_node *dev_node)
{
	...
	if (!rc) {
		pdata->flags = value;
#ifdef CONFIG_OPPO
		if (oppo_panel_id == 0x048005)
		{
			pdata->flags |= 0x10;
		}
		else if (oppo_panel_id == EVERDISPLAY_320X360_CMD_SIGNATURE)
		{
			pdata->flags |= 0x00;
		}
		else if (oppo_panel_id == VISIONOX_402X476_CMD_SIGNATURE)
		{
			pdata->flags |= 0x08;
		}
		pr_err("%s: panel_id=%x,flag=%x\n", __func__,oppo_panel_id,pdata->flags);
#endif
	}
