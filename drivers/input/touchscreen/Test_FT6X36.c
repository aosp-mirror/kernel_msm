...
static void MergeAllTestData(void)
{
	...
	printk("[fts] %s lenStoreMsgArea=%d,  lenStoreDataArea = %d\n", __func__, g_lenStoreMsgArea, g_lenStoreDataArea);
}
...
unsigned char FT6X36_TestItem_RawDataTest(bool * bTestResult)
{
	...
	printk("[fts]==============================Test Item: -------- RawData Test \n");	
	...
	if (ReCode == ERROR_CODE_OK)
	{
		printk("[fts]======= Test Data:  \n");
		ShowRawData();
	}
	else
	{
		printk("[fts]RawData Test is Error. Failed to get Raw Data!!\n");
		btmpresult = false;
		goto TEST_END;
	}
	...
	for (i = 0; i < g_ScreenSetParam.iChannelsNum + g_ScreenSetParam.iKeyNum; i++)
	{
		...
		if (m_RawData[i] < RawDataMin || m_RawData[i] > RawDataMax)
		{
			...
			if (iNgNum == 0) printk("[fts]======= NG Data: \n");

			if (i < g_ScreenSetParam.iChannelsNum)
				printk("[fts]Ch_%02d: %d Set_Range=(%d, %d) ,	", i+1, m_RawData[i], RawDataMin, RawDataMax);
			else
				printk("[fts]Key_%d: %d Set_Range=(%d, %d) ,	", i+1 - g_ScreenSetParam.iChannelsNum, m_RawData[i], RawDataMin, RawDataMax);
			if (iNgNum % 6 == 0)
			{
				printk("\r\n" );
			}
	...
	printk("[fts] Max Raw Value: %d, Min Raw Value: %d, Deviation Value: %d, Average Value: %d", iMax, iMin, iMax - iMin, iAvg);
...
TEST_END:
	...
	{
		* bTestResult = true;
		printk("[fts]RawData Test is OK!\r\n");
	}
	else
	{
		* bTestResult = false;		
		printk("[fts]RawData Test is NG!\r\n");
	}
	return ReCode;
}
...
unsigned char FT6X36_TestItem_CbTest(bool * bTestResult)
{
	...
	{
		btmpresult = false;
		printk("[fts]=========  CB test Failed! \n");		
	}
	else
	{
		printk("[fts]Get Proof_NoWaterProof CB Data... \n");
	...
	printk("\r\n");
	printk("[fts]Proof_Level0 CB Test...\r\n");
	for (i = 0; i < 3; i++)
	{
	...
	{
		printk("[fts]Get Proof_Level0 CB Data...\n");
		...
		printk("\n");
		iAvg /= g_ScreenSetParam.iChannelsNum + g_ScreenSetParam.iKeyNum;	
		printk("[fts] Max CB Value: %d, Min CB Value: %d, Deviation Value: %d, Average Value: %d\n", iMax, iMin, iMax - iMin, iAvg);
			
		if (btmpresult)
		{
			printk("[fts]CB Test is OK!\r\n");
			* bTestResult = 1;
		}
		else
		{
			printk("[fts]CB Test is NG!\r\n");
			* bTestResult = 0;
		}
	}
...
unsigned char FT6X36_TestItem_DeltaCbTest(unsigned char * bTestResult)
{
	...
	printk("[fts]==============================Test Item: -------- Delta CB Test \n");
	...
	if (bUseSort1)
	{
		if (Delta_Ci_Differ <= Sort1Max - Sort1Min)
		{
			if (bUseCriticalValue)
			{
				...
				printk("[fts] Condition1: Max of Delta CB_Differ: %d, Min of Delta CB_Differ: %d, Get Deviation: %d, Set Critical Deviation of Sort1: %d",
					Sort1Max, Sort1Min, Sort1Max - Sort1Min, Critical_Delta_S1);
			}
			else
			{
				btmpresult = false;
				printk("[fts] Condition1: Max of Delta CB_Differ: %d, Min of Delta CB_Differ: %d, Get Deviation: %d, Set Deviation of Sort1: %d",
		...
		printk("\r\n[fts]Max Deviation,Sort1: %d, ", Sort1Max - Sort1Min);		
	}
	if (bUseSort2)
	{
	...
	printk("\n[fts] Condition2: Get Max Differ Data of Delta_CB(abs): %d, Set Max Differ Data of Delta_CB(abs): %d",Delta_Max, set_Delta_Cb_Max);
	...
	printk("\n[fts]Max Differ Data of Delta_CB(abs): %d ", Delta_Max);
...
unsigned char FT6X36_TestItem_ChannelsDeviationTest(unsigned char * bTestResult)
{
	...
	for (i=0; i < g_ScreenSetParam.iChannelsNum/*readlen*/; i++)
	{
		...
		if (g_stCfg_SCap_DetailThreshold.DeltaCxTest_Sort[i] == 3)
		{
			...
			{
				if (Sort3LastNum + 1 == i)//\AC۾F\B3q\B9D
				{
