				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	if (ts->is_suspend)
		dev_info(dev, "[touch]Raydium IC is_suspend at %s\n",
				__func__);
	...
	do {
		...
		if (ret < 0)
			goto exit_i2c_error;
		dev_info(dev, "[touch]Raydium %s return 0x%02x!!\n", __func__,
			   rbuffer[0]);
			...
			struct device_attribute *attr, char *buf)
{
	...
	snprintf(buf, ATR_MAX_SIZE, "[touch]Touch check i2c : %02X%02X\n"
			, rbuffer[3],
			 rbuffer[2]);
	buf_len = strlen(buf);
	ret = buf_len + 1;
			  ...
			  unsigned long ul_fw_addr, unsigned char type)
{
	...
	if (type == RAYDIUM_TEST_FW) {
		ui_checksum = rc_crc32((firmware_data + 4),
				ul_fw_size, ui_checksum);
...
static int
raydium_wait_ft_int(struct i2c_client *client)
{
	int ret = SUCCESS;
	unsigned int retry, retry_time = 1000;
	unsigned int irq_value;
...
raydium_load_test_fw(struct i2c_client *client)
{
	...
	if (is_MCU_HOLD == 0) {
		retry = retry_time;
		while (retry != 0 && g_uc_raydium_flag !=
...
static int
raydium_fw_upgrade(struct i2c_client *client, unsigned char type,
		   unsigned char isSkipLoad, unsigned char isCheckCRCError)
{
	int ret = 0;
	unsigned char buf[4];
	unsigned int retry, retry_time = 200;
	...
	//#init_code:0x01, baseline:0x02, COMP:0x04, param:0x08
	//FW:0x10, bootloader:0x20
	memset(buf, 0, sizeof(buf));
	buf[0] = type;
	...
	//#init_code:0x800,
	//Baseline:0xA00, COMP:0xCD4, para:0xF1C, FW:0x1000, BOOT:0x5000
	...
	if (retry == 0) {
		dev_err(&dev,
			"[touch]Error,gu8syncdata.cmd_type is WRT_PRAM %d\n",
			   buf[0]);
		ret = -1;
		goto exit_upgrade;
	}
	...
	if (type == RAYDIUM_INIT) {
		...
		ret =
			raydium_fw_upgrade_with_image(client, 0x4000,
					RAYDIUM_BOOTLOADER);
		if (ret < 0)
			goto exit_upgrade;
	}
	...
	if (ret < 0)
		goto exit_upgrade;
	//#wait(checksum okay)
	//ACK cmd(gu8I2CSyncData.cmd_type=0xa5 at 0x20000210)
	buf[0] = 0;
	retry = retry_time;
	...
	if (retry == 0) {
		dev_err(&dev, "[touch]Error, wait(checksum okay) ACK cmd 0x%x\n",
			   buf[0]);
		ret = -1;
		goto exit_upgrade;
	}
	...
	if (retry == 0) {
		dev_err(&dev, "[touch]Error, confirm ACK cmd result 0x%x\n",
				buf[0]);
		ret = -1;
		goto exit_upgrade;
	}
	...
	//#adb shell "echo 20000210 1 A5 >
	///sys/bus/i2c/drivers/raydium_ts/1-0039/raydium_i2c_pda_access"
	//#above command can be ignored, due to previous
	//while loop has check its value.
	memset(buf, 0, sizeof(buf));
	buf[0] = 0x5a;
	...
	if (retry == 0) {
		dev_info(dev, "[touch]Error, wait for input unlock key 0x%x\n",
			   buf[0]);
		ret = -1;
		goto exit_upgrade;
	}
	...
	//#confirm burning_state
	//result(gu8I2CSyncData.burning_state=
	...
	if (retry == 0) {
		dev_err(&dev, "[touch]%s, FW not ready,retry error!\n",
				__func__);
		ret = ERROR;
		goto exit_upgrade;
	}
	...
	while (buf[0] != 130 && retry != 0) {
		...
		retry--;
		dev_err(&dev,
			"[touch]wait software reset finished %d\n", buf[0]);
	}
	if (retry == 0) {
		dev_err(&dev,
			"[touch]Error, wait software reset finished %d\n",
			   buf[0]);
		ret = -1;
		goto exit_upgrade;
	}
	...
	if (type == RAYDIUM_BASELINE || type == RAYDIUM_INIT
		...
		//#confirm
		//gu8I2CSyncData.normal_state=NORMAL_CRC_NOTIFY at 0x20000208
		buf[0] = 0;
		retry = retry_time;
			...
			struct device_attribute *attr, char *buf)
{
	...
	raydium_irq_control(ts, DISABLE);
	dev_err(&dev, "[touch]Raydium %s burn type is %d!!\n", __func__,
		   upgrade_type);
...
static int
raydium_fw_upgrade_with_bin_file(struct i2c_client *client, char *arguments,
								 size_t count)
{
	int ret = 0;
	...
	while (uc_fw_amount--) {
		...
		fw_fd = open_namei(AT_FDCWD, token, O_RDONLY, 0);
		if (IS_ERR(fw_fd)) {
		...
		memset(firmware_data, 0, ul_fw_size);
		ul_fw_read_size =
			fw_fd->f_op->read(fw_fd, firmware_data, ul_fw_size,
					&fw_fd->f_pos);
		ui_checksum = rc_crc32(firmware_data, ul_fw_size, ui_checksum);
				   ...
				   const char *buf, size_t count)
{
	...
	snprintf(fwname, ATR_MAX_SIZE, "%s", buf);
	fwname[count - 1] = '\0';

	if (ts->is_suspend)
		dev_err(&dev, "[touch]Raydium IC is_suspend at %s\n",
				__func__);
				...
				struct device_attribute *attr, char *buf)
{
	...
	snprintf(buf, ATR_MAX_SIZE, "0x%08lX : 0x%02X%02X%02X%02X\n", g_ul_addr,
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	snprintf(buf, ATR_MAX_SIZE, "0x%04X : 0x%02X%02X%02X%02X\n", g_uc_addr,
			 rbuffer[3], rbuffer[2], rbuffer[1], rbuffer[0]);
	buf_len = strlen(buf);
					...
					const char *buf, size_t count)
{
	...
	temp_buf = kzalloc(count + 1, GFP_KERNEL);
	if (temp_buf == NULL) {
		dev_err(&dev, "[touch]kzalloc temp_buf failed\n");
		return -ENOMEM;
	}
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	while (ui_read_offset < g_ui_raw_data_length) {
		if ((ui_read_offset + MAX_READ_PACKET_SIZE) <
			...
			us_read_length =
				(unsigned short)
				(g_ui_raw_data_length - ui_read_offset);
		}
					   ...
					   const char *buf, size_t count)
{
	...
	result =
		raydium_i2c_pda2_write(client,
				RAYDIUM_PDA2_HOST_CMD_ADDR, w_data, 1);
	if (result < 0) {
		mutex_unlock(&ts->lock);
		ret = result;
		goto exit_error;
	...
	result =
		raydium_i2c_pda2_write(client,
				RAYDIUM_PDA2_HOST_CMD_ADDR, w_data,
				RAYDIUM_FT_CMD_LENGTH);
	mutex_unlock(&ts->lock);
	if (result < 0) {
		ret = result;
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	snprintf(buf,
		ATR_MAX_SIZE,  "[touch]Raydium flag : %d\n", g_uc_raydium_flag);
...
static ssize_t
raydium_wait_ft_int_show(struct device *dev,
		 struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = container_of(dev, struct i2c_client, dev);
	unsigned int ret;
	...
	if (ret < 0)
		return ret;
	snprintf(buf, ATR_MAX_SIZE, "Raydium raydium ftint : %d\n", ret);
	return strlen(buf) + 1;
}

static ssize_t
raydium_wait_ft_int_store(struct device *dev,
					  struct device_attribute *attr,
					  const char *buf, size_t count)
{
	return count;
...
raydium_hw_reset_fun(struct i2c_client *client)
{
	...
	//HW reset
	dev_info(&client->dev, "[touch]Raydium HW reset\n");
	gpio_direction_output(ts->rst, 1);
	gpio_direction_output(ts->rst, 0);
	msleep(RAYDIUM_RESET_INTERVAL_MSEC);
	gpio_direction_output(ts->rst, 1);
						...
						const char *buf, size_t count)
{
	...
	ret =
		raydium_i2c_pda2_write(client,
				RAYDIUM_PDA2_PALM_AREA_ADDR, w_data, 1);
	if (ret < 0) {
		mutex_unlock(&ts->lock);
		goto exit_error;
	}
...
static ssize_t
raydium_int_flag_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	int buf_len = 0;

	snprintf(buf, ATR_MAX_SIZE, "%d", g_uc_raydium_int_flag);
...
static ssize_t
raydium_int_flag_store(struct device *dev,
					   struct device_attribute *attr,
					   const char *buf, size_t count)
{
	int ret = 0;
...
static ssize_t
raydium_selftest_flag_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int buf_len = 0;

	snprintf(buf, ATR_MAX_SIZE, "%d", g_uc_raydium_selftest_flag);
	buf_len = strlen(buf);
	return buf_len + 1;
}

static ssize_t
raydium_selftest_flag_store(struct device *dev,
						struct device_attribute *attr,
						const char *buf, size_t count)
{
	int ret = 0;
	...
	dev_info(dev, "[touch]Raydium selftest flag store = %d!!\n",
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	snprintf(buf, ATR_MAX_SIZE, "Raydium Driver Version : 0x%X\n",
			RAYDIUM_VER);
	buf_len = strlen(buf);
	ret = buf_len + 1;
	return ret;
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	if (ret < 0)
		goto exit_i2c_error;
	ret =
		raydium_i2c_pda2_read(client, RAYDIUM_PDA2_FW_VERSION_ADDR,
				rbuffer, 4);
	if (ret < 0)
		goto exit_i2c_error;
	snprintf(buf, ATR_MAX_SIZE,
		"Raydium Touch FW Version : %02X%02X%02X%02X\n", rbuffer[0],
		rbuffer[1], rbuffer[2], rbuffer[3]);

	fw_version =
		(rbuffer[0] << 24) | (rbuffer[1] << 16) |
		(rbuffer[2] << 8) | rbuffer[3];
	dev_errr(&dev, "[touch]Raydium IC FW version is 0x%x\n", fw_version);
	...
	image_version =
		...
		(RadFWImage[0xF23]
				<< 8) |
		RadFWImage[0xF24];
	dev_errr(&dev, "[touch]Raydium Image FW version is 0x%x\n",
		   image_version);
	...
	if (fw_version != image_version)
		printk(KERN__ERR "[touch]%s, FW need upgrade.\n", __func__);
		...
		struct device_attribute *attr, char *buf)
{
	...
	ret =
		raydium_i2c_pda2_read(client,
				RAYDIUM_PDA2_PANEL_VERSION_ADDR, rbuffer,
							  8);
	if (ret < 0)
		goto exit_i2c_error;
	snprintf(buf, ATR_MAX_SIZE,
...
fw_check_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (ts->fw_version == 0)
		...
		count += snprintf(buf + count, ATR_MAX_SIZE,
				"Pass, (fw_ver : 0x%x)\n", ts->fw_version);
	return count;
}

static unsigned char
i2c_burst_read(struct i2c_client *client, int int_addr,
...
static unsigned char
i2c_burst_write(struct i2c_client *client, int int_addr,
				unsigned short bWriteLen, unsigned char *bValue)
{
	unsigned char result = ERROR;
	int ret = 0;
	...
	while (u16_len < bWriteLen) {
		ret =
			raydium_i2c_pda_write(client,
					int_addr, bValue + u16_len, u8_w_len);
		if (ret < 0)
			return result;
		u16_len += u8_w_len;
...
RETRY_I2C:
	while (i16_time_out > 0 && u8_is_pass != 1) {
		...
		while (writeIndex < u32_test_length) {
			if (i2c_burst_write(client,
				addr + writeIndex, i2cWriteLen, wData) == 0) {
				i16_time_out--;
		...
		while (readLength < u32_test_length) {
			if (i2c_burst_read
				(client, addr + readLength,
				 i2cReadLength, readData) == 0) {
				i16_time_out--;
				if (raydium_hw_reset_fun(client) == ERROR)
					goto NG_CASE;
				if (raydium_selftest_stop_mcu(client) < 0)
 ...
 * example:echo "offset num_of_bin
 * length *_app.bin [length *_app.bin]" > raydium_fw_upgrade_mode
 */
...
/* I2C pda mode (R/W)
 *  example:
 * cat raydium_i2c_pda_access ==> read pda address provided by the following cmd
 * echo ADDRinHEX [DATAinHEX] >
 * raydium_i2c_pda_access ==> write pda address [data]
 */
...
/* I2C pda2 mode (R/W)
 *  example:
 * cat raydium_i2c_pda2_access ==>
 * read pda2 address provided by the following cmd
 * echo ADDRinHEX [DATAinHEX] >
 * raydium_i2c_pda2_access ==> write pda2 address [data]
 */
 ...
 *  example:
 * cat raydium_i2c_raw_data ==>
 * read raw data with specific length of
 * corresponding type provided by the following cmd
 * echo DataTypeinHEX RawDataLengthinHEX >
 * raydium_i2c_raw_data ==> set raw data type and its length
 */
...
raydium_read_touchdata(struct raydium_ts_data *data)
{
	...
	for (i = 0; i < MAX_TOUCH_NUM; i++) {
		if (gst_slot_status[i].need_update) {
			offset = gst_slot_status[i].
				pt_report_offset * LENGTH_PT;
			s16_x =
				buf[POS_X_L + offset] |
				buf[POS_X_H +
...
raydium_ts_interrupt(int irq, void *dev_id)
{
	...
	//For bootloader wrt/erase flash and software reset interrupt
		...
		if (!work_pending(&raydium_ts->work)) {
			result = queue_work(raydium_ts->workqueue,
					&raydium_ts->work);
			//queue_work fail
			if (result == false)
				dev_err(&raydium_ts->client->dev,
						"[touch]queue_work fail.\n");
			else {
...
raydium_ts_do_suspend(struct raydium_ts_data *ts)
{
	...
	if (cancel_work_sync(&ts->work))
		dev_info(&ts->client->dev, "[touch]workqueue is not empty!\n");
...
raydium_ts_do_resume(struct raydium_ts_data *ts)
{
	...
	if (cancel_work_sync(&ts->work))
		dev_info(&ts->client->dev, "[touch]workqueue is not empty!\n");
	if (ts->is_close != 1) {
		dev_info(&ts->client->dev, "[touch]Already in open state\n");
		raydium_irq_control(ts, ENABLE);
	}
...
raydium_ts_open(struct input_dev *input_dev)
{
	...
	dev_err(&ts->client->dev, "[touch]ts->blank:%x\n", ts->blank);
	...
	if (ts->is_sleep == 1) {
		...
		raydium_irq_control(ts, ENABLE);
		ts->is_sleep = 0;
		dev_err(&ts->client->dev, "[touch]disable touch lock.\n");
	}
...
raydium_ts_early_suspend(struct early_suspend *handler)
{
	struct raydium_ts_data *raydium_ts = container_of(handler,
				 struct raydium_ts_data, early_suspend);
...
raydium_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (!raydium_ts) {
		dev_err(&client->dev,
				"[touch]failed to allocate inputdriver data\n");
		return -ENOMEM;
	}
	...
	if (ret) {
		dev_err(&client->dev, "[touch]raydium_power_initfailed\n");
		goto exit_regulator_failed;
	}

	ret = raydium_power_on(raydium_ts, true);
	if (ret) {
		dev_err(&client->dev, "[touch]raydium_power_onfailed\n");
		goto pwr_deinit;
	}
	...
	if (ret) {
		dev_err(&client->dev,
		"[touch]raydium_ts_probe: failed to register input_device: %s\n",
		dev_name(&client->dev));
	...
	//disable_irq then enable_irq for avoid Unbalanced enable for IRQ warn
	raydium_irq_control(raydium_ts, DISABLE);
	raydium_irq_control(raydium_ts, ENABLE);

	dev_err(&client->dev, "[touch]Raydium Touch driver version :0x%04X\n",
		   RAYDIUM_VER);
...
err_gpio_req:
	...
	if (raydium_ts->ts_pinctrl) {
		if (IS_ERR_OR_NULL(raydium_ts->pinctrl_state_release)) {
			...
			if (ret)
				pr_err(
				"[touch]failed to select relase pinctrl state\n"
				);
		}
	}
#endif //end of MSM_NEW_VER
...
static const struct of_device_id raydium_match_table[] = {
	{.compatible = "raydium,raydium-ts-ray-390p",},
	{},
};
#else
