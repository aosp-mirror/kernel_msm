...
static int FT_SC_Enter_Factory_Mode(void)
{
	...
	if ((val_read == FACTORY_MODE) && (try_cnt < 10)) {
		FTS_DBG("[FTS] Enter Factory Mode\n");
	} else {
		FTS_DBG("[FTS] Enter Factory Mode Fail\n");
...
static int FT_SC_Enter_Normal_Mode(void)
{
	...
	if ((val_read == NORMAL_MODE) && (try_cnt < 10)) {
		FTS_DBG("[FTS] Enter Normal Mode\n");
	} else {
		FTS_DBG("[FTS] Enter Normal Mode Fail\n");
				...
				struct device_attribute *attr, char *buf)
{
	...
	if (err < 0) {
		num_read_chars += snprintf(buf, 128,
			"get tp fw version fail!, i2c err :%d\n", err);
	} else {
		if (fwver == 255)
			num_read_chars += snprintf(buf, 128,
				"get tp fw version is wrong!\n");
		else
			num_read_chars += snprintf(buf, 128,
				"tp fw version : 0x%x\n", fwver);
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (0 != retval) {
		dev_err(dev,
		"Could not convert the input to a number. input : %s\n", buf);
	...
	if (2 == num_read_chars) {
		/* read register */
		regaddr = wmreg;
		pr_info("[fts][test](0x%02x)\n", regaddr);
		if (fts_read_reg(client, regaddr, &regvalue) < 0)
			pr_err("[fts] %s : Could not read the reg(0x%02x)\n",
				__func__, regaddr);
		else
			pr_info("[fts] %s : the reg(0x%02x) is 0x%02x\n",
		...
		if (fts_write_reg(client, regaddr, regvalue) < 0)
			dev_err(dev,
				"[fts] %s : Could not write reg(0x%02x)\n",
				__func__, regaddr);
		else
			dev_dbg(dev,
				"[fts] %s : Write 0x%02x into reg(0x%02x)OK\n",
...
* Return: char number
***********************************************************************/
static ssize_t fts_tptestchnum_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	...
	if (FT_SC_Enter_Factory_Mode() == ERR_ENTER_FACTORY) {
		FTS_DBG("[FTS]Failed, Test CH and Key num enter FTM mode\n");
		tmp_chars = snprintf(tmp_buf, 128,
			"Failed, Test CH and Key Num enter FTM mode\n");
		strlcat(buf, tmp_buf, 768);
		...
		if ((val_chnl_num != MAX_CHANNEL_NUMBER) ||
			(val_key_num != MAX_KEY_NUMBER)) {
			FTS_DBG("[FTS]Failed, Test ch num: %d, key num: %d\n",
				val_chnl_num, val_key_num);
			tmp_chars = snprintf(tmp_buf, 128,
				"Failed, num ch num: %d, key num: %d\n",
				val_chnl_num, val_key_num);
			strlcat(buf, tmp_buf, 768);
			num_read_chars += tmp_chars;
		} else {
			FTS_DBG("[FTS]Pass, ch num: %d, key num: %d\n",
				val_chnl_num, val_key_num);
			tmp_chars = snprintf(tmp_buf, 128,
				"Pass, ch_num: %d, key_num: %d\n",
				val_chnl_num, val_key_num);
			strlcat(buf, tmp_buf, 768);
			num_read_chars += tmp_chars;
		}
	...
	if (FT_SC_Enter_Normal_Mode() == ERR_ENTER_NORMAL) {
		FTS_DBG("[FTS]Failed, Test CH and Key num into normal mode\n");
		tmp_chars = snprintf(tmp_buf, 128,
			"Failed, Test Channel and Key num into normal mode\n");
		strlcat(buf, tmp_buf, 768);
		num_read_chars += tmp_chars;
	}
	mutex_unlock(&fts_input_dev->mutex);
...
* Return: EPERM
***********************************************************************/
static ssize_t fts_tptestchnum_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	/* place holder for future use */
	return -EPERM;
...
* Return: char number
***********************************************************************/
static ssize_t fts_tptestrawdata_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	ssize_t num_read_chars = 0, tmp_chars = 0;
	int try_cnt = 0;
	...
	if (FT_SC_Enter_Factory_Mode() == ERR_ENTER_FACTORY) {
		FTS_DBG("[FTS] Fail at Test RawData enter factory mode\n");
		tmp_chars = snprintf(tmp_buf, 128,
			"Fail at Test RawData enter factory mode\n");
		strlcat(buf, tmp_buf, 768);
		num_read_chars += tmp_chars;
	} else {
		...
		if (val_read != 0) {
			FTS_DBG("[FTS]Fail at Test RawData ready\n");
			tmp_chars = snprintf(tmp_buf, 128,
					"Fail at Test RawData ready\n");
			strlcat(buf, tmp_buf, 768);
			num_read_chars += tmp_chars;
			goto ERROR_RETURN;
		}
		...
		val_write = 0;
		/*tell touch ic, read rawdata from addr 0x00 -->
			write 0x00 on register 0x34 */
		fts_write_reg(fts_i2c_client, reg_val, val_write);
		msleep(500);

		reg_val = RAW_DATA_BUF;
		/* start to read from register 0x35.
			the data has CHANNEL_NUMBER * 2 bytes. */
		fts_i2c_read(fts_i2c_client, &reg_val, 1,
				rawdata_packet_buf, MAX_CHANNEL_NUMBER * 2);
		...
		for (i = 0; i < MAX_CHANNEL_NUMBER * 2; i += 2) {
			rawdata_buf[i/2] = (((u16)rawdata_packet_buf[i]) << 8)
					| ((u16)rawdata_packet_buf[i+1]);
...
ERROR_RETURN:
	...
	if (FT_SC_Enter_Normal_Mode() == ERR_ENTER_NORMAL) {
		FTS_DBG("[FTS]Fail at Test RawData return normal mode\n");
		tmp_chars = snprintf(tmp_buf, 128,
				"Fail at Test RawData return normal mode\n");
		strlcat(buf, tmp_buf, 768);
		num_read_chars += tmp_chars;
	}
	mutex_unlock(&fts_input_dev->mutex);
...
* Return: EPERM
***********************************************************************/
static ssize_t fts_tptestrawdata_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	/* place holder for future use */
	return -EPERM;
...
* Return: char number
***********************************************************************/
static ssize_t fts_tptestcbdata_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	ssize_t num_read_chars = 0, tmp_chars = 0;
	int try_cnt = 0;
	...
	if (FT_SC_Enter_Factory_Mode() == ERR_ENTER_FACTORY) {
		FTS_DBG("[FTS]Fail at Test CBDATA enter factory mode\n");
		tmp_chars = snprintf(tmp_buf, 128,
			"Fail at Test CBDATA enter factory mode\n");
		strlcat(buf, tmp_buf, 768);
		num_read_chars += tmp_chars;
	} else {
		...
		do {
			...
			msleep(50);
			/* need to get 0x00 on register 0x08
				 ready for getting cbdata */
			fts_read_reg(fts_i2c_client, reg_val, &val_read);
			try_cnt++;
		} while ((val_read != 0) && (try_cnt < 10));

		if (val_read != 0) {
			FTS_DBG("[FTS]Fail at Test CBData ready\n");
			tmp_chars = snprintf(tmp_buf, 128,
					"Fail at Test CBData ready\n");
			strlcat(buf, tmp_buf, 768);
			num_read_chars += tmp_chars;
			goto ERROR_RETURN;
		}
		...
		val_write = 0;
		/* tell touch ic, read CB data from addr 0x00 -->
			write 0x00 on register 0x33 */
		fts_read_reg(fts_i2c_client, reg_val, &val_read);
		msleep(500);

		reg_val = CB_DATA_BUF;
		/* start read from register 0x39.
			the data has CHANNEL_NUMBER * 2 bytes. */
		fts_i2c_read(fts_i2c_client, &reg_val, 1,
				cbdata_packet_buf, MAX_CHANNEL_NUMBER * 2);
		...
		for (i = 0; i < MAX_CHANNEL_NUMBER * 2; i += 2) {
			cb_data[i/2] = cbdata_buf[i/2] =
				(((u16)cbdata_packet_buf[i]) << 8) |
				((u16)cbdata_packet_buf[i+1]);
		...
		do {
			...
			msleep(50);
			/* need to get 0x00 on register 0x08
				ready for getting cb delta data */
			fts_read_reg(fts_i2c_client, reg_val, &val_read);
			try_cnt++;
		} while ((val_read != 0) && (try_cnt < 10));

		if (val_read != 0) {
			FTS_DBG("[FTS]Fail at Test CB Delta Data ready\n");
			tmp_chars = snprintf(tmp_buf, 128,
			"Fail at Test CB Delta Data ready\n");
		...
		/* TODO: checking, take CB Delta data is same way
			with rawdata or not? */
		reg_val = CB_DATA_ADDR_R;
		val_write = 0;
		/* tell touch ic, read CB Delta data from addr 0x00 -->
			write 0x00 on register 0x33 */
		fts_write_reg(fts_i2c_client, reg_val, val_write);
		msleep(500);
		...
		/*start read from register 0x39.
			the data has CHANNEL_NUMBER * 2 bytes. */
		fts_i2c_read(fts_i2c_client, &reg_val, 1,
			cbDeltadata_packet_buf, MAX_CHANNEL_NUMBER * 2);
		...
		for (i = 0; i < MAX_CHANNEL_NUMBER * 2; i += 2) {
			cbDeltadata_buf[i/2] = abs(cbdata_buf[i/2] -
				((((u16)cbDeltadata_packet_buf[i]) << 8) |
				((u16)cbDeltadata_packet_buf[i+1])));
...
ERROR_RETURN:
	...
	if (FT_SC_Enter_Normal_Mode() == ERR_ENTER_NORMAL) {
		FTS_DBG("[FTS]Fail at Test CBDATA return normal mode\n");
		tmp_chars = snprintf(tmp_buf, 128,
				"Fail at Test CBDATA return normal mode\n");
		strlcat(buf, tmp_buf, 768);
		num_read_chars += tmp_chars;
	}
	mutex_unlock(&fts_input_dev->mutex);
...
* Return: EPERM
***********************************************************************/
static ssize_t fts_tptestcbdata_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	/* place holder for future use */
	return -EPERM;
...
static int ft3207_GetInISize(char *config_name)
{
	...
	snprintf(filepath, 128, "%s%s", FT3207_CFG_FILEPATH, config_name);
	if (NULL == pfile)
		pfile = filp_open(filepath, O_RDONLY, 0);
	if (IS_ERR(pfile)) {
		pr_err("[FTS] %s : err while opening file %s.\n",
			__func__, filepath);
		return -EIO;
	}
...
static int ft3207_get_testparam_from_ini(char *config_name)
{
	...
	pr_err("[FTS] %s : inisize = %d\n", __func__, file_size);
	if (file_size <= 0) {
		pr_err("[FTS]%s, err : Get firmware size failed\n", __func__);
		return -EIO;
	}
	...
	if (ft3207_ReadInIData(config_name, config_data)) {
		pr_err("[FTS]%s, err: request_firmware failed\n", __func__);
		kfree(config_data);
		return -EIO;
	} else {
		pr_info("[FTS]%s, ft3207_ReadInIData successful\n", __func__);
	}

	if (set_param_data(config_data) < 0) {
		pr_err("[FTS]%s, The IC type err in this testing\n", __func__);
		return -EIO;
	}
	return 0;
}
...
static struct file *fts_selftest_file_open(void)
{
	...
	memset(filepath, 0, sizeof(filepath));
	snprintf(filepath, 128, "%s%s",
		FT3207_RESULT_FILEPATH, "selftest.csv");
...
static int fts_fw_check(u8 *tmp_buf)
{
	u8 fw_ver = 0;
	int err = -1;
	u16 reg_addr = FTS_REG_FW_VER;
	int result = FW_CHECK_PASS;

	err = fts_read_reg(fts_i2c_client, reg_addr, &fw_ver);

	if (err < 0) {
		*tmp_buf = fw_ver = fts_wq_data->fw_ver[0];
		pr_err("[FTS] i2c Read Fail, use fw ver on probe: 0x%x\n",
				fw_ver);
	} else {
		*tmp_buf = fw_ver;
	}

	if (fts_wq_data->shipping_fw_version == 0) {
		pr_err("[FTS] Fail, shipping fw = 0, tp_id : 0x%x, wrong\n",
			fts_wq_data->tp_vendor_id);
		result = TP_ID_IS_WRONG;
	} else if (fts_wq_data->shipping_fw_version == fw_ver) {
		pr_info("[FTS]Pass(fw_ver: 0x%x, fw_id: 0x%x, tp_id: 0x%x)\n",
		fw_ver, fts_wq_data->fw_vendor_id, fts_wq_data->tp_vendor_id);
	} else {
		pr_err("[FTS]Fail, shipping fw = 0x%x, fw_ver : 0x%x\n",
			fts_wq_data->shipping_fw_version, fw_ver);
		pr_err("[FTS]Fail, fw_id : 0x%x, tp_id : 0x%x\n",
			fts_wq_data->fw_vendor_id, fts_wq_data->tp_vendor_id);
...
static ssize_t fts_selftest_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = container_of(dev, struct i2c_client, dev);
	...
	u8 fw_check_result = 1;	/* default Fail */
	u8 fw_ver;
	bool item_result[SELFTEST_ITEM_NUM] = {0};

	int count = 0;

	selft_test_result = 1;	/* default Fail */

	fw_check_result = fts_fw_check(&fw_ver);
	if (fw_check_result)
		goto fw_check_fail;
	...
	case TP_ID_PFW2_BLACK:
		snprintf(config_file, 128, TP_INI_FILE_PFW2_BLACK);
		break;

	case TP_ID_PFW2_GOLD:
		snprintf(config_file, 128, TP_INI_FILE_ID_PFW2_GOLD);
		break;

	case TP_ID_PFW2_ROSE_GOLD:
		snprintf(config_file, 128, TP_INI_FILE_ID_PFW2_ROSE_GOLD);
		break;

	case TP_ID_PFW2_SILVER:
		snprintf(config_file, 128, TP_INI_FILE_ID_PFW2_SILVER);
		break;

	case TP_ID_PFW2_SABLE:
		snprintf(config_file, 128, TP_INI_FILE_ID_PFW2_SABLE);
		break;

	case TP_ID_PFW2_NAVY:
		snprintf(config_file, 128, TP_INI_FILE_ID_PFW2_NAVY);
		break;

	case TP_ID_PFW3_BLACK:
		snprintf(config_file, 128, TP_INI_FILE_ID_PFW3_BLACK);
		break;

	case TP_ID_PFW4_BLACK:
		snprintf(config_file, 128, TP_INI_FILE_ID_PFW4_BLACK);
		break;

	case TP_ID_PFW5_BLACK:
		snprintf(config_file, 128, TP_INI_FILE_ID_PFW5_BLACK);
		break;
	...
	pr_info("[FTS]%s : loading config file : %s\n", __func__, config_file);

	init_i2c_read_func(FTS_I2c_Read);
	init_i2c_write_func(FTS_I2c_Write);

	if (ft3207_get_testparam_from_ini(config_file) < 0) {
		pr_err("[FTS]%s : get testparam from ini failure\n", __func__);
	} else {
		pr_info("[FTS]%s : tp test Start...\n", __func__);

		if (FT3207_StartTest(item_result)) {
			pr_info("[FTS]%s : tp test pass\n", __func__);
			selft_test_result = 0;
		} else {
			pr_err("[FTS]%s : tp test failure\n", __func__);
			selft_test_result = 1;
		}
		...
		if (!w_file) {
			pr_err("[FTS]%s : Open log file fail !\n", __func__);
		} else {
			w_len =  FT3207_get_test_data(&w_buf);
			fts_selftest_file_write(w_file, w_buf, w_len);

			w_len = snprintf(result_buf, 128,
				"FW ver:0x%x, TP ID:0x%x\nSelftest %s\nEND\n",
				fw_ver, fts_wq_data->tp_vendor_id,
...
fw_check_fail:
	...
	if (selft_test_result)
		pr_err("[FTS] %s: Selftest FAIL\n", __func__);
	else
		pr_info("[FTS] %s : Selftest PASS\n", __func__);

	count += snprintf(buf + count, 128, "[FTS] : Selftest %s;\n",
				selft_test_result ? "FAIL" : "PASS");
	...
	if (fw_check_result != 0) {
		...
		if (!w_file) {
			...
			w_len += snprintf(result_buf, 128,
				"FW Check Fail;TP Ver:0x%x(0x%x);TP ID:0x%x\n",
				fw_ver, fts_wq_data->shipping_fw_version,
				fts_wq_data->tp_vendor_id);

			w_len += snprintf(result_buf + w_len, 128,
				"[FTS] : Selftest %s\n=====END=====\n",
				selft_test_result ? "FAIL" : "PASS");
			fts_selftest_file_write(w_file, result_buf, w_len);
			fts_selftest_file_close(w_file);
	...
	for (i = 0; i < SELFTEST_ITEM_NUM; i++) {
		if (item_result[i] == false) {
			...
			case ENTER_FACTORY_MODE_TEST:
				count += snprintf(buf + count, 128,
						"TP EnterFTM Fail\n");
				pr_info("[fts]%s : TP EnterFTM Fail\n",
					__func__);
				return count;
				break;
			...
			case CB_TEST:
				count += snprintf(buf + count, 128,
						"CB Fail;\n");
				pr_info("[fts]%s : TP CB Fail\n", __func__);
				break;

			case DIFF_CB_TEST:
				count += snprintf(buf + count, 128,
						"DiffCB Fail\n");
				pr_info("[fts]%s :TP DiffCB Fail\n", __func__);
				break;
...
static ssize_t fts_fw_check_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int count = 0;
	u8 fw_ver = 0;
	u8 fw_check_result = 0;

	fw_check_result = fts_fw_check(&fw_ver);

	switch (fw_check_result) {
	case FW_CHECK_PASS:
		count += snprintf(buf + count, 128,
			"Pass, (fw_ver : 0x%x, fw_id : 0x%x, tp_id : 0x%x)\n",
			fw_ver, fts_wq_data->fw_vendor_id,
			fts_wq_data->tp_vendor_id);
	break;
	case FW_CHECK_FAIL:
		count += snprintf(buf + count, 128,
			"Fail, (shipping_fw_ver = 0x%x, fw_ver : 0x%x, fw_id : 0x%x, tp_id : 0x%x)\n",
			fts_wq_data->shipping_fw_version, fw_ver,
			fts_wq_data->fw_vendor_id, fts_wq_data->tp_vendor_id);
	break;
	case TP_ID_IS_WRONG:
		count += snprintf(buf + count, 128,
			"Fail, shipping fw = 0, tp_id : 0x%x, TP ID is wrong\n",
			fts_wq_data->tp_vendor_id);
	break;
	default:
	break;
...
int check_TP_ID(char *buf, ssize_t buf_len, u8 tp_id)
{
	...
	case TP_ID_PFW2_BLACK:
		buf_len += snprintf(buf + buf_len,
				128, "TP is PFW2 Black, 1.1 mm\n");
	break;
	case TP_ID_PFW2_GOLD:
		buf_len += snprintf(buf + buf_len,
				128, "TP is PFW2 Gold, 1.1 mm\n");
	break;
	case TP_ID_PFW2_ROSE_GOLD:
		buf_len += snprintf(buf + buf_len,
				128, "TP is PFW2 Rose Gold, 1.1 mm\n");
	break;
	case TP_ID_PFW2_SILVER:
		buf_len += snprintf(buf + buf_len,
				128, "TP is PFW2 Sliver, 1.1 mm\n");
	break;
	case TP_ID_PFW2_SABLE:
		buf_len += snprintf(buf + buf_len,
				128, "TP is PFW2 Sable, 1.1 mm\n");
	break;
	case TP_ID_PFW2_NAVY:
		buf_len += snprintf(buf + buf_len,
				128, "TP is PFW2 Navy, 1.1 mm\n");
	break;
	case TP_ID_PFW3_BLACK:
		buf_len += snprintf(buf + buf_len,
				128, "TP is PFW3 Black, 1.1 mm, 2.5D, R0.2\n");
	break;
	case TP_ID_PFW4_BLACK:
		buf_len += snprintf(buf + buf_len,
				128, "TP is PFW4 Black, 1.1 mm, 2.5D, R0.25\n");
	break;
	case TP_ID_PFW5_BLACK:
		buf_len += snprintf(buf + buf_len,
				128, "TP is PFW5 Black, 1.8 mm, step\n");
	break;
	default:
		buf_len += snprintf(buf + buf_len,
				128, "TP isn't correct\n");
	break;
	}
...
static ssize_t fts_getTPinfo_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	ssize_t num_read_chars = 0;
	u8 fwver = 0, tp_id = 0;
...
/* dean add get TP infomation include fw ver, tp id from chip by i2c */
static ssize_t fts_TPupgradeAPP_for_i_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	ssize_t num_read_chars = 0;
	struct i2c_client *client = container_of(dev, struct i2c_client, dev);
	...
	err = fts_ctpm_auto_upgrade_for_cci(fts_i2c_client,
				fts_wq_data->tp_vendor_id, true);

	if (err == 0) {
		num_read_chars += snprintf(buf + num_read_chars,
			128, "TP Upgrade is OK, FW ver : 0x%x, TP ID : 0x%x\n",
			fts_wq_data->fw_ver[0], fts_wq_data->tp_vendor_id);
	} else {
		num_read_chars += snprintf(buf + num_read_chars,
				128, "TP Upgrade is Fail\n");
	}
...
/* dean add Dynamically on/off debug msg, start */
static ssize_t show_tp_debug_tag(struct device *dev,
		struct device_attribute *attr, char *buf)
{
#ifndef FTS_GESTRUE_EN
	return snprintf(buf, 128, "TP down event log : %s\n",
				print_point?"ON":"OFF");
#else
	return snprintf(buf, 128, "TP down event log : %s, gesture log: %s\n",
				print_point?"ON":"OFF",
				print_gesture?"ON":"OFF");
#endif
}

static ssize_t store_tp_debug_tag(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	pr_info("[fts]%s, buf : %d\n", __func__, buf[0]);
	...
	switch (buf[0]) {
	case DISABLE_TAG:
		pr_info("[fts]%s, TP debug log disable\n", __func__);
		print_point = false;
	...
	case ENABLE_POINT_TAG:
		pr_info("[fts]%s, TP down event log enable\n", __func__);
		print_point = true;
	break;
#ifdef FTS_GESTRUE_EN
	case ENABLE_GESTURE_TAG:
		pr_info("[fts]%s, TP gesture log enable\n", __func__);
		print_gesture = true;
	break;
#endif
	default:
		pr_info("[fts]%s, invalid format\n", __func__);
	break;
	}
...
#ifdef FTS_GESTRUE_EN
static ssize_t show_tp_enable_big_area_event_tag(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	pr_info("[fts]%s, Enable TP big area event into ambient mode\n",
			__func__);
	en_big_area_func = true;
	return snprintf(buf, 128, "TP big area function : %s\n",
			en_big_area_func?"ON":"OFF");
}

static ssize_t show_tp_disable_big_area_event_tag(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	pr_info("[fts]%s, Disable TP big area event into ambient mode\n",
			__func__);
...
static ssize_t tp_enable_irq_func(struct device *dev,
		struct device_attribute *attr, char *buf)
{
...
static ssize_t tp_disable_irq_func(struct device *dev,
		struct device_attribute *attr, char *buf)
{
...
static ssize_t show_tp_irq_debug_info(struct device *dev,
		struct device_attribute *attr, char *buf)
{
...
static DEVICE_ATTR(ftstpdriverver, S_IRUGO|S_IWUSR,
...
static DEVICE_ATTR(ftstptestchnum, S_IRUGO|S_IWUSR,
		fts_tptestchnum_show, fts_tptestchnum_store);
static DEVICE_ATTR(ftstptestrawdata, S_IRUGO|S_IWUSR,
		fts_tptestrawdata_show, fts_tptestrawdata_store);
static DEVICE_ATTR(ftstptestcbdata, S_IRUGO|S_IWUSR,
		fts_tptestcbdata_show, fts_tptestcbdata_store);
...
static DEVICE_ATTR(fts_TPupgradeAPP_for_i, S_IRUGO,
		fts_TPupgradeAPP_for_i_show, NULL);
static DEVICE_ATTR(en_msg_report_data, S_IRUGO | S_IWUSR,
		show_tp_debug_tag, store_tp_debug_tag);

#ifdef FTS_GESTRUE_EN
static DEVICE_ATTR(enable_big_area_event, S_IRUGO,
		show_tp_enable_big_area_event_tag, NULL);
static DEVICE_ATTR(disable_big_area_event, S_IRUGO,
		show_tp_disable_big_area_event_tag, NULL);

#endif

static DEVICE_ATTR(enable_tp_irq, S_IRUGO,
		tp_enable_irq_func, NULL);
static DEVICE_ATTR(disable_tp_irq, S_IRUGO,
		tp_disable_irq_func, NULL);
static DEVICE_ATTR(tp_irq_debug_info, S_IRUGO,
		show_tp_irq_debug_info, NULL);
