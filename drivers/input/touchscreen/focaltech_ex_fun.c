...
static ssize_t fts_selftest_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	sprintf(config_file, TP_INI_FILE_FNW1);
...
int check_TP_ID(char *buf, ssize_t buf_len, u8 tp_id) {
	...
	switch(tp_id) {
		...
		case TP_ID_1:
			buf_len += sprintf(buf + buf_len, "TP is DLW1\n");
			break;
		case TP_ID_2:
			buf_len += sprintf(buf + buf_len, "TP is DLW2\n");
			break;
		case TP_ID_3:
			buf_len += sprintf(buf + buf_len, "TP is DLW3\n");
			break;
		default:
