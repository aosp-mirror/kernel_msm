...
#define FT5X0X_CFG_FILEPATH "/system/res/TP/"
#define FT5X0X_RESULT_FILEPATH "/sdcard/"
...
static int ft5x0x_get_testparam_from_ini(char *config_name)
{
	...
	if (ft5x0x_ReadInIData(config_name, config_data)) {
		...
		pr_info("[FTS][Touch] %s : ft5x0x_ReadInIData successful \n", __func__);
	}
...
static ssize_t fts_selftest_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, 128, "[FTS] : Selftest %s\n", selft_test_result ? "FAIL" : "PASS");
}
...
static struct file *fts_selftest_file_open(void)
{
	...
	memset(filepath, 0, sizeof(filepath));
	sprintf(filepath, "%s%s", FT5X0X_RESULT_FILEPATH, "selftest.csv");
...
static ssize_t fts_selftest_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (ft5x0x_get_testparam_from_ini(config_file) < 0) {
		...
		if(!w_file)
			...
			w_buf = kmalloc(1*1024, GFP_KERNEL);
			if(!w_buf)
...
static DEVICE_ATTR(ftstpselftest, S_IRUGO | S_IWUSR, fts_selftest_show, fts_selftest_store);
