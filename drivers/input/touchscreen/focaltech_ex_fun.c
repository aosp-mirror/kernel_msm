...
static int FT_SC_Enter_Factory_Mode(void)
{
	...
	while ((val_read != FACTORY_MODE) && (try_cnt < 10))
	{
		...
		FTS_DBG("[FACTORY][%d] >>>>> val_read: 0x%x\n", try_cnt, val_read);
		msleep(10);
		try_cnt++;
	}
	...
	{
		FTS_DBG("[FACTORY] >>>>> Enter Factory Mode\n");
	}
	else
	{
		FTS_DBG("[FACTORY] >>>>> Enter Factory Mode Fail\n");
...
static int FT_SC_Enter_Normal_Mode(void)
{
	...
	{
		FTS_DBG("[FACTORY] >>>>> Enter Normal Mode\n");
	}
	else
	{
		FTS_DBG("[FACTORY] >>>>> Enter Normal Mode Fail\n");
...
int fts_create_apk_debug_channel(struct i2c_client * client)
{
	...
	#else
		fts_proc_entry = create_proc_entry(PROC_NAME, 0440, NULL);
	#endif
...
static ssize_t fts_tptestchnum_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	{
		FTS_DBG("[FACTORY] >>>>> Fail at Test Channel and Key Number enter factory mode\n");
	...
	else
	{
		...
		{
			FTS_DBG("[FACTORY] >>>>>Failed Test Channel and Key Number ch_num: %d, key_num: %d\n",val_chnl_num,val_key_num);
		...
		{
			FTS_DBG("[FACTORY] >>>>>Pass Test_Channel_and_Key_Number ch_num: %d, key_num: %d\n",val_chnl_num,val_key_num);
	...
	{
		FTS_DBG("[FACTORY] >>>>>Fail at Test Channel and Key Number return normal mode\n");
...
static ssize_t fts_tptestrawdata_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (FT_SC_Enter_Factory_Mode() == ERR_ENTER_FACTORY)
	{
		FTS_DBG("[FACTORY] >>>>> Fail at Test RawData enter factory mode\n");
	...
	else
	{
		...
		if (val_read != 0)
		{
			FTS_DBG("[FACTORY] >>>>>Fail at Test RawData ready\n");
...
ERROR_RETURN:
	...
	if (FT_SC_Enter_Normal_Mode() == ERR_ENTER_NORMAL)
	{
		FTS_DBG("[FACTORY] >>>>>Fail at Test RawData return normal mode\n");
...
static ssize_t fts_tptestcbdata_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (FT_SC_Enter_Factory_Mode() == ERR_ENTER_FACTORY)
	{
		FTS_DBG("[FACTORY] >>>>>Fail at Test CBDATA enter factory mode\n");
	...
	else
	{
		...
		if (val_read != 0)
		{
			FTS_DBG("[FACTORY] >>>>>Fail at Test CBData ready\n");
		...
		if (val_read != 0)
		{
			FTS_DBG("[FACTORY] >>>>>Fail at Test CB Delta Data ready\n");
...
ERROR_RETURN:
	...
	if (FT_SC_Enter_Normal_Mode() == ERR_ENTER_NORMAL)
	{
		FTS_DBG("[FACTORY] >>>>>Fail at Test CBDATA return normal mode\n");
...
static ssize_t show_tp_enable_record_tp_data_tag(struct device *dev,
						 ...
						 char *buf)
{
	printk(KERN_DEBUG "[fts]%s, Enable record_tp_data_tag\n", __func__);

	memset(TPdata_buf, 0, RECORD_SIZE);
	TPdata_buf_len = 0;
	en_record_tp_data_func = true;
	return sprintf(buf, "TP record_tp_data_tag : %s\n",
		       en_record_tp_data_func ? "ON" : "OFF");
}

static ssize_t show_tp_disable_record_tp_data_tag(struct device *dev,
						  struct device_attribute *attr,
						  char *buf)
{
	mm_segment_t oldfs;
	...
	memset(filepath, 0, sizeof(filepath));
	sprintf(filepath, "%s%s", FT3207_RESULT_FILEPATH, "tp_rawdata.csv");
	...
	filp->f_op->write(filp, TPdata_buf, TPdata_buf_len, &filp->f_pos);

	set_fs(oldfs);
	filp_close(filp, NULL);

	printk(KERN_DEBUG "[fts]%s, Disable record_tp_data_tag\n", __func__);
	en_record_tp_data_func = false;
	return sprintf(buf, "TP record_tp_data_tag : %s\n",
		       en_record_tp_data_func ? "ON" : "OFF");
}
#endif
