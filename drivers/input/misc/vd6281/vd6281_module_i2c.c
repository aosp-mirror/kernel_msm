...
{
	data->vio = devm_regulator_get(dev, "vio");
	if (IS_ERR(data->vio)) {
		data->vio = NULL;
				   ...
				   const struct i2c_device_id *id)
{
	...
	/* i2c power up */
	rc = regulator_enable(vd6281_data->vio);
	if (rc)
		dev_err(&client->dev, "Failed to enable vio\n");

	retry = 0;
	do {
		/* Reading device ID at 0x00 */
		device_id = i2c_smbus_read_byte_data(client, 0x00);

		if (device_id < 0) {
			dev_warn(&client->dev, "wait for i2c driver probe %d",
				device_id);
			rc = device_id;
		} else {
			dev_info(&client->dev, "probe success, device id 0x%x",
			device_id);
		}
		retry++;
	...
	rc = regulator_disable(vd6281_data->vio);
	if (rc)
		dev_err(&client->dev, "Failed to disable vio\n");
