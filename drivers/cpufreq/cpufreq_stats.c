...
static int cpufreq_stats_update(unsigned int cpu)
{
	...
	if (stat->time_in_state) {
		int last_index = atomic_read(&stat->last_index);

		stat->time_in_state[last_index] += cur_time - stat->last_time;
		if (all_stat)
			all_stat->time_in_state[last_index] +=
...
void acct_update_power(struct task_struct *task, cputime_t cputime) {
	...
	last_index = atomic_read(&stats->last_index);

	/* This function is called from a different context
	 * Interruptions in between reads/assignements are ok
	 */
	if (all_freq_table && cpufreq_all_freq_init && task->time_in_state) {
		last_cputime = atomic_read(&task->time_in_state[last_index]);
		atomic_set(&task->time_in_state[last_index],
	...
	curr = powerstats->curr[last_index];
	...
	int cpu, struct cpufreq_frequency_table *table, int count)
{
	...
	atomic_set(&stat->last_index, freq_table_get_index(stat, policy->cur));
	spin_unlock(&cpufreq_stats_lock);
	return 0;
error_alloc:
	sysfs_remove_group(&policy->kobj, &stats_attr_group);
		...
		unsigned long val, void *data)
{
	...
	old_index = atomic_read(&stat->last_index);
	...
	spin_lock(&cpufreq_stats_lock);
	atomic_set(&stat->last_index, new_index);
#ifdef CONFIG_CPU_FREQ_STAT_DETAILS
