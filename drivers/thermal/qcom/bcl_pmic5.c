...
static void convert_vbat_low_to_adc_val(int *val)
{
	*val = (*val * 2000) / BCL_VBAT_SCALING_UV;
}

static void convert_vbat_comp_low_to_adc_val(int *val)
{
	*val = *val / BCL_VBAT_SCALING;
}
...
static int bcl_set_vbat(void *data, int low, int high)
{
	...
	case BCL_VBAT_LVL0:
		addr = BCL_VBAT_ADC_LOW;
		convert_vbat_low_to_adc_val(&thresh_value);
		val = (int8_t)thresh_value;
		break;
	case BCL_VBAT_LVL1:
		addr = BCL_VBAT_COMP_LOW;
		thresh_value = thresh_value - 2250;
		convert_vbat_comp_low_to_adc_val(&thresh_value);
		val = (int8_t)thresh_value;
		break;
	case BCL_VBAT_LVL2:
		addr = BCL_VBAT_COMP_TLOW;
...
static irqreturn_t bcl_handle_irq(int irq, void *data)
{
	...
	if (irq_status & perph_data->status_bit_idx) {
		...
		for (i = 0; i < bcl_perph->num_count; i++)
			of_thermal_handle_trip(bcl_perph->virtual_tz_dev[i]);
	}
