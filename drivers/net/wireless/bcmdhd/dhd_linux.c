 ...
 *
 * $Id: dhd_linux.c 611507 2016-01-11 11:14:39Z $
 */
...
bool dhd_update_fw_nv_path(dhd_info_t *dhdinfo)
{
	...
	struct wifi_platform_data *plat_data = adapter->wifi_plat_data;
	...
	if (plat_data->get_fw_nv_path)
		plat_data->get_fw_nv_path(&fw, &nv);
             ...
             int8 max_rssi, int8 min_rssi)
{
	...
	err = dhd_iovar(&dhd->pub, 0, "rssi_monitor", (char *)&rssi_monitor, sizeof(rssi_monitor), TRUE);
...
#define STORE_WKLOCK_RECORD(wklock_type) \
	do { \
		...
		if (wklock_info) { \
			...
			if (!wklock_info) {\
				printf("Can't allocate wk_trace_record \n"); \
			} else { \
...
wklock_trace_onoff(struct dhd_info *dev, const char *buf, size_t count)
{
	...
	spin_unlock_irqrestore(&dhd->wakelock_spinlock, flags);
	if (trace_wklock_onoff) {
		printf("ENABLE WAKLOCK TRACE\n");
	} else {
		printf("DISABLE WAKELOCK TRACE\n");
	}
