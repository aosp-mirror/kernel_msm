...
static int st21nfc_dev_open(struct inode *inode, struct file *filp)
{
	...
	pr_info("%s : device_open = %d", __func__, device_open);
			      ...
			      unsigned long arg)
{
	...
	case ST21NFC_GET_WAKEUP:
		...
		pr_info("%s get gpio result %d, active_polarity is %d\n", __func__, ret, st21nfc_dev->platform_data.active_polarity);
		break;
...
static ssize_t st21nfc_read_three_bytes(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
    ...
    count = 3;
    //ret = i2c_master_recv(data->platform_data.client, tmp, count);
    for(i = 0;i < count;i++) {
		ret = i2c_master_recv(data->platform_data.client, &(tmp[i]), 1);
		pr_info("%s : st21nfc register %d:  0x%x \n", __func__, i, tmp[i]);
	}

	if (ret > 0)
		return sprintf(buf, "0x%x 0x%x 0x%x\n", tmp[0], tmp[1], tmp[2]);
	return 0;
}				/* st21nfc_read_three_bytes*/

static ssize_t st21nfc_read_64_bytes(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	struct st21nfc_dev *data = dev_get_drvdata(dev);
	size_t count = 0;
...
#else
    for(i = 0;i < count;i++) {
		ret = i2c_master_recv(data->platform_data.client, tmp, 1);
		pr_info("%s : st21nfc register %d:  0x%x \n", __func__, i, tmp[0]);
	}
#endif
	if (ret > 0) {
		return sprintf(buf, "please see the result in serial log\n");
	}
	return 0;
}				/* st21nfc_read_64_bytes*/

static ssize_t st21nfc_read_irq_gpio(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	struct st21nfc_dev *data = dev_get_drvdata(dev);
	int ret = 0;
	...
	pr_info("%s : irq gpio status: %d\n", __func__, ret);

	return sprintf(buf, "%d\n", ret);
}				/* st21nfc_read_irq_gpio */
...
static DEVICE_ATTR(i2c_read_three_bytes, S_IRUGO, st21nfc_read_three_bytes, NULL);
static DEVICE_ATTR(i2c_read_64_bytes, S_IRUGO, st21nfc_read_64_bytes, NULL);
static DEVICE_ATTR(read_irq_gpio, S_IRUGO, st21nfc_read_irq_gpio, NULL);

static struct attribute *st21nfc_attrs[] = {
	&dev_attr_i2c_addr.attr,
	&dev_attr_version.attr,
	&dev_attr_i2c_read_three_bytes.attr,
	&dev_attr_i2c_read_64_bytes.attr,
	&dev_attr_read_irq_gpio.attr,
	NULL,
};
...
static int nfc_parse_dt(struct device *dev, struct st21nfc_platform_data *pdata)
{
...
#if 0
	pdata->ena_gpio = of_get_named_gpio(np, "st,ena_gpio", 0);
	if ((!gpio_is_valid(pdata->ena_gpio))) {
		pr_err("[dsc]%s: fail to get ena_gpio\n", __func__);
		return -EINVAL;
	}
#else
	pdata->ena_gpio = 0;
	...
	pr_err("[dsc]%s : get reset_gpio[%d], irq_gpio[%d], ena_gpio[%d] polarity_mode[%d]\n",
	       __func__, pdata->reset_gpio, pdata->irq_gpio, pdata->ena_gpio, pdata->polarity_mode);
	return r;
}
#else
static int nfc_parse_dt(struct device *dev, struct st21nfc_platform_data *pdata)
			 ...
			 const struct i2c_device_id *id)
{
	...
	ret = gpio_request(platform_data->irq_gpio, "st21nfc_irq");
	pr_err("%s : irq gpio_request ret is %d\n", __FILE__,ret);
	if (ret) {
	...
	if (platform_data->reset_gpio != 0) {
		...
		/* low active */
		gpio_set_value(st21nfc_dev->platform_data.reset_gpio, 1);
	pr_err("%s : nfc reset done\n", __FILE__);
	}

	ret = gpio_get_value(st21nfc_dev->platform_data.irq_gpio);
	pr_err("%s : irq gpio status  is %d, line is %d\n", __FILE__,ret,__LINE__);
	...
	if (platform_data->ena_gpio != 0) {
		...
		//ret = gpio_direction_output(platform_data->ena_gpio, 0);//TODO
		if (ret) {
	...
	pr_err("%s : irq gpio status  is %d, line is %d\n", __FILE__,ret,__LINE__);
		...
		if (st21nfc_dev->platform_data.reset_gpio != 0) {
			...
			gpio_set_value(st21nfc_dev->platform_data.reset_gpio,
				       1);
			pr_info("%s done Pulse Request converse\n", __func__);
		}
#else
		if (st21nfc_dev->platform_data.reset_gpio != 0) {
			/* pulse low for 20 millisecs */
	...
	count = 64;
	for(i=0;i<count;i++) {
		ret = i2c_master_recv(st21nfc_dev->platform_data.client, tmp, 1);
		pr_err("%s : st21nfc register %d:  0x%x \n", __func__, i, tmp[0]);
	}
...
static int st21nfc_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	if (client) {
		...
		if (st21nfc_dev && st21nfc_dev->platform_data.ena_gpio != 0) {
			/* ON */
			//gpio_set_value(st21nfc_dev->platform_data.ena_gpio, 0);//TODO
			return 0;
		}
...
static int st21nfc_resume(struct i2c_client *client)
{
	...
	if (client) {
		...
		if (st21nfc_dev && st21nfc_dev->platform_data.ena_gpio != 0) {
			/* ON */
			gpio_set_value(st21nfc_dev->platform_data.ena_gpio, 0);
			return 0;
		}
		pr_debug("%s : failing no st21 context %p !!!\n", __func__,
			 st21nfc_dev);
