				...
				unsigned int cmd, unsigned long arg)
{
	...
	case pn548_SET_PWR:
		if (arg == 2) {
			...
			/* power on */
			pr_debug(PN548_DRV_NAME ":%s power on\n", __func__);
			...
			/* power off */
			pr_debug(PN548_DRV_NAME ":%s power off\n", __func__);
			if (sPowerState == NFC_POWER_ON) {
				gpio_set_value(pn548_dev->firm_gpio, 0);
				gpio_set_value(pn548_dev->ven_gpio, 0);
				msleep(10);
...
static void pn548_parse_dt(struct device *dev, struct pn548_dev *pn548_dev)
{
				...
				const struct i2c_device_id *id)
{
	...
	ret = gpio_request_one(pn548_dev->irq_gpio, GPIOF_IN,  "nfc_int");
	if (ret) {
		pr_err(PN548_DRV_NAME ": nfc_int request failed!\n");
		goto err_int;
	}

	ret = gpio_request_one(pn548_dev->ven_gpio,
	...
	ret = gpio_request_one(pn548_dev->firm_gpio,
