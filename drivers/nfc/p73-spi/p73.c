...
static void p61_start_throughput_measurement(unsigned int type)
{
	if (type == READ_THROUGH_PUT) {
		...
		P61_DBG_MSG("p61_start_throughput_measurement:"
					"wrong type = %d", type);
	}
}
...
static void p61_stop_throughput_measurement(unsigned int type, int no_of_bytes)
{
	if (type == READ_THROUGH_PUT) {
		...
		if(p61_through_put_t.total_through_put_rtime
			...
			printk(" **************** Read Throughput: **************");
			printk(" No of Read Bytes = %ld",
				p61_through_put_t.total_through_put_rbytes);
			printk(" Total Read Time (uSec) = %ld",
				p61_through_put_t.total_through_put_rtime);
		...
		if(p61_through_put_t.total_through_put_wtime
				>= MXAX_THROUGH_PUT_TIME) {
			printk(" **************** Write Throughput: **************");
			printk(" No of Write Bytes = %ld",
				p61_through_put_t.total_through_put_wbytes);
			printk(" Total Write Time (uSec) = %ld",
				p61_through_put_t.total_through_put_wtime);
		...
		printk(" Total Write Time (uSec) = %ld",
		 ...
		 printk(" p61_stop_throughput_measurement:"
				"wrong type = %d", type);
	}
}
							...
							unsigned long arg)
{
	...
	case P61_SET_PWR:
		if (arg == 2) {
			...
			if (p61_regulator != NULL) {
				...
				P61_ERR_MSG("ERROR:p61_regulator not enabled");
			}
#endif
			...
			msleep(50);
			ret = spi_read(p61_dev->spi, (void *)buf,
						sizeof(buf));
			msleep(50);
			gpio_set_value(p61_dev->rst_gpio, 1);
			msleep(20);
	...
	case P61_GET_SPM_STATUS:
		P61_DBG_MSG("P61_GET_SPM_STATUS: enter");
		ret = pn544_dev_ioctl(filp, P61_GET_PWR_STATUS, arg);
		P61_DBG_MSG("P61_GET_SPM_STATUS: exit");
		break;
	case P61_SET_DWNLD_STATUS:
		P61_DBG_MSG("P61_SET_DWNLD_STATUS: enter");
		ret = pn544_dev_ioctl(filp, PN544_SET_DWNLD_STATUS,
						arg);
		P61_DBG_MSG("P61_SET_DWNLD_STATUS: =%d exit",ret);
		break;
	...
	case P61_GET_ESE_ACCESS:
		P61_DBG_MSG("P61_GET_ESE_ACCESS: enter");
		ret = pn544_dev_ioctl(filp, P544_GET_ESE_ACCESS, arg);
		P61_DBG_MSG("P61_GET_ESE_ACCESS ret: %d exit",ret);
		break;
	case P61_SET_POWER_SCHEME:
		P61_DBG_MSG("P61_SET_POWER_SCHEME: enter");
		ret = pn544_dev_ioctl(filp, P544_SET_POWER_SCHEME, arg);
		P61_DBG_MSG("P61_SET_POWER_SCHEME ret: %d exit",ret);
		break;
	case P61_INHIBIT_PWR_CNTRL:
		P61_DBG_MSG("P61_INHIBIT_PWR_CNTRL: enter");
		ret = pn544_dev_ioctl(filp, P544_SECURE_TIMER_SESSION, arg);
		P61_DBG_MSG("P61_INHIBIT_PWR_CNTRL ret: %d exit", ret);
		break;
	default:
		P61_DBG_MSG("Error case");
		ret = -EINVAL;
	}

	P61_DBG_MSG("p61_dev_ioctl-exit %u arg = %ld\n", cmd, arg);
	return ret;
}
...
/**
 * ingroup spi_driver
 * brief Used to read data from P61 in Poll/interrupt mode
	...
	if (p61_dev->enable_poll_mode) {
...
#ifdef P61_IRQ_ENABLE
		P61_DBG_MSG("%s Interrupt Mode Enabled \n", __FUNCTION__);
		if (!gpio_get_value(p61_dev->irq_gpio)) {
			...
			while (1) {
				...
				ret = wait_event_interruptible(p61_dev->read_wq,
					!p61_dev->irq_enabled);
				p61_disable_irq(p61_dev);
				if (ret) {
					P61_ERR_MSG("wait interruptible fail\n");
					goto fail;
				}
		...
		ret = spi_read(p61_dev->spi, (void *)&rx_buffer[0], count);
		if (0 > ret) {
			P61_DBG_MSG("SPI_READ returned 0x%x", ret);
			ret = -EIO;
			goto fail;
		}
	}
...
/**
 * ingroup spi_driver
 * brief It will configure the GPIOs required for soft reset,
 * 			read interrupt & regulated power supply to P61.
 *
...
#else
	p61_regulator = regulator_get( &spi->dev, "vdd");
#endif
...
#else
		P61_ERR_MSG(" Error to get vdd (error code) = %d\n", ret);
#endif
		return  -ENODEV;
	} else {
...
/**
 * ingroup spi_driver
 * brief To probe for P61 SPI interface.
 * 		If found initialize the SPI clock,
 * 		bit rate & SPI mode.
...
static int p61_probe(struct spi_device *spi)
{
	...
	if (platform_data == NULL) {
		...
		/* TBD: This is only for Panda as we are
		  passing NULL for platform data */
	...
	printk(KERN_ERR "%s exit nfc spi probe sucessful\n", __func__);
	return ret;
#ifdef P61_IRQ_ENABLE
err_exit1:
...
/**
 * ingroup spi_driver
 * brief Will get called when the device is removed
...
MODULE_DEVICE_TABLE(of, p61_dt_match);
#endif
...
#if DRAGON_P61
				.of_match_table = of_match_ptr(p61_dt_match),
#endif
		},
		.probe =  p61_probe,
		.remove = p61_remove,
