...
struct pn544_dev    {
	...
	/* gpio used by SPI to provide power to p61 via NFCC */
	unsigned int        ese_pwr_gpio;
#ifdef ISO_RST
	unsigned int        iso_rst_gpio; /* ISO-RST pin gpio*/
#endif
	unsigned int clkreq_gpio;
	/* stores the current P61 state */
	p61_access_state_t  p61_current_state;
	/* stores the VEN pin state powered by Nfc */
...
static int nqx_clock_select(struct pn544_dev *nqx_dev);
/*clock disable function*/
static int nqx_clock_deselect(struct pn544_dev *nqx_dev);
...
static STATUS svdd_sync_onoff(long nfc_service_pid, p61_access_state_t origin)
{
	...
	pr_info("%s: Exit\n", __func__);
	ret = (pn544_dev->dwpLinkUpdateStat == 0x00) ? STATUS_SUCCESS : STATUS_FAILED;
	return ret;
}
...
static STATUS dwp_OnOff(long nfc_service_pid, p61_access_state_t origin)
{
	...
	if (nfc_service_pid) {
		if (0 == signal_handler(origin, nfc_service_pid)) {
			if (wait_for_completion_timeout(&dwp_onoff_sema,
					tempJ) == 0) {
...
static int release_dwpOnOff_wait(void)
{
	...
	complete(&dwp_onoff_sema);
	{
		...
		/*release JNI only after all the SPI On
		...
		unsigned long arg)
{
	...
	int r = 0;
	pr_info("%s :enter cmd = %u, arg = %ld P544_SET_NFC_SERVICE_PID %u\n",
			__func__, cmd, arg, P544_SET_NFC_SERVICE_PID);
	...
	case PN544_SET_PWR:
	{
		...
		if (arg == 2) {
			...
			&& (pn544_dev->chip_pwr_scheme != PN80T_EXT_PMU_SCHEME)) {
				/* NFCC fw/download should not be
				  allowed if p61 is used by SPI
				*/
			...
			/*add for nfc clk enable*/
			usleep_range(10000, 10100);
			if (pn544_dev->clk_pin_voting) {
				r = nqx_clock_select(pn544_dev);
				if (r < 0)
					dev_err(&pn544_dev->client->dev, "unable to enable clock\n");
			}
			...
			if (pn544_dev->firm_gpio) {
				if ((current_state
					& (P61_STATE_WIRED | P61_STATE_SPI | P61_STATE_SPI_PRIO))== 0){
					p61_update_access_state(pn544_dev,
						P61_STATE_IDLE, true);
				}
				gpio_set_value(pn544_dev->firm_gpio, 0);
			...
			/*add for nfc clk disable*/
			usleep_range(10000, 10100);
			if (pn544_dev->clk_pin_voting) {
				r = nqx_clock_deselect(pn544_dev);
				if (r < 0)
					dev_err(&pn544_dev->client->dev, "unable to disable clock\n");
			}
	...
	case P61_SET_SPI_PWR:
    {
		...
		p61_get_access_state(pn544_dev, &current_state);
		if (pwrLevel == 1) {
			pr_info("%s : PN61_SET_SPI_PWR - power on ese %x\n",
					__func__, current_state);
			if (((current_state & (P61_STATE_SPI|P61_STATE_SPI_PRIO)) == 0)
				|| (current_state & P61_STATE_SPI_FAILED)) {
				...
				&& !(current_state & P61_STATE_JCP_DWNLD)) {
					if(pn544_dev->nfc_service_pid) {
						STATUS stat = dwp_OnOff(pn544_dev->nfc_service_pid,
								P61_STATE_SPI);
						pr_info("nfc service pid %s   ---- %ld",
							__func__, pn544_dev->nfc_service_pid);
						if(stat != STATUS_SUCCESS) {
							pr_info(" %s DWP link activation failed. Returning..",
								__func__);
							p61_update_access_state(pn544_dev,
								P61_STATE_SPI_FAILED, true);
							return stat;
						}
					} else {
						pr_info("%s invalid nfc service pid .signalling failed - %ld\n",
							__func__, pn544_dev->nfc_service_pid);
					}
				}
				...
				if (pn544_dev->nfc_ven_enabled == false) {
					/* provide power to NFCC
					 if NFC service not provided */
					gpio_set_value(pn544_dev->ven_gpio, 1);
					msleep(10);
				}
				...
				/* Delay (10ms) after SVDD_PWR_ON to
				 allow JCOP to bootup
				 (5ms jcop boot time + 5ms guard time) */
				usleep_range(10000, 12000);
				if(current_state & P61_STATE_SPI_FAILED) {
					p61_update_access_state(pn544_dev,
							P61_STATE_SPI_FAILED, false);
				}
				p61_update_access_state(pn544_dev,
						P61_STATE_SPI, true);
				if (pn544_dev->nfc_service_pid) {
				...
				pr_info("%s PN61_SET_SPI_PWR - power on ese exit\n",
						__func__);

			} else if ((current_state
				& (P61_STATE_SPI|P61_STATE_SPI_PRIO))
				&& (gpio_get_value(pn544_dev->ese_pwr_gpio))
				&& (gpio_get_value(pn544_dev->ven_gpio))) {
				/* Returning success if SET_SPM_POWER called
				 while already SPI is open */
				return 0;
			} else {
			...
			/* if secure timer is running, Delay the SPI close
			 by 25ms after sending End of Apdu to enable eSE go into DPD
			...
			if (!(current_state & P61_STATE_WIRED)
				...
				/* Delay (2.5ms) after SVDD_PWR_OFF for
				  the shutdown settlement time */
			...
			if ((pn544_dev->nfc_ven_enabled == false)
				...
				if (!(current_state & P61_STATE_WIRED)
					...
					if(isSignalTriggerReqd) {
						if(pn544_dev->nfc_service_pid){
							STATUS stat = svdd_sync_onoff(pn544_dev->nfc_service_pid,
									...
									| P61_STATE_SPI_END);
							if(stat != STATUS_SUCCESS) {
								pr_info(" %s DWP link deactivation Failed. Returning",
									__func__);
								p61_update_access_state(pn544_dev,
									P61_STATE_SPI_FAILED, true);
								return stat;
					...
					/* if secure timer is running, Delay the SPI
					  close by 25ms after sending End of
					  Apdu to enable eSE go into DPD gracefully
					  (20ms after EOS + 5ms DPD settlement time) */
					if(pn544_dev->secure_timer_cnt)
						usleep_range(25000, 30000);
...
#ifndef JCOP_4X_VALIDATION
						gpio_set_value(pn544_dev->ese_pwr_gpio, 0);
						/* Delay (2.5ms) after SVDD_PWR_OFF
						 for the shutdown settlement time */
						usleep_range(2500, 3000);
#endif
						if(current_state
							& P61_STATE_SPI_FAILED) {
							p61_update_access_state(pn544_dev,
									P61_STATE_SPI_FAILED, false);
						}
						if(current_state
					...
					/*If JCOP3.2 or 3.3 for handling
					 triple mode protection
					 signal NFC service */
					if(isSignalTriggerReqd) {
						if (!(current_state & P61_STATE_JCP_DWNLD)) {
							if(pn544_dev->nfc_service_pid){
						...
						} else if (pn544_dev->chip_pwr_scheme
									== PN80T_LEGACY_PWR_SCHEME) {
							svdd_sync_onoff(pn544_dev->nfc_service_pid,
								P61_STATE_SPI_SVDD_SYNC_START);
						}
					...
					if(pn544_dev->chip_pwr_scheme
						...
						if(current_state
							...
							if(isSignalTriggerReqd)
								svdd_sync_onoff(pn544_dev->nfc_service_pid,
									P61_STATE_SPI_SVDD_SYNC_END);
								pr_info("PN80T legacy ese_pwr_gpio off %s",
									__func__);
							}
						}
						pn544_dev->spi_ven_enabled = false;
						...
						pr_err("%s : PN61_SET_SPI_PWR - failed,"
							"current_state = %x \n",
							__func__, pn544_dev->p61_current_state);
				...
				if ((current_state
					...
					if(pn544_dev->chip_pwr_scheme
						...
						if (pn544_dev->nfc_ven_enabled
								== false) {
							/* provide power to NFCC if,
							 NFC service not provided */
							gpio_set_value(pn544_dev->ven_gpio, 1);
							msleep(10);
						}
						/* pull the gpio to high once
						 NFCC is power on*/
						gpio_set_value(pn544_dev->ese_pwr_gpio, 1);

						/* Delay (10ms) after SVDD_PWR_ON
						 to allow JCOP to bootup
						 (5ms jcop boot time + 5ms guard time) */
						usleep_range(10000, 12000);
					}
				}else {
				...
				if (current_state & P61_STATE_SPI_PRIO) {
					...
					p61_update_access_state(pn544_dev,
						P61_STATE_SPI_PRIO, false);
					/*after SPI prio timeout, the state
					  is changing from SPI prio to SPI */
					p61_update_access_state(pn544_dev,
						P61_STATE_SPI, true);
					if (current_state & P61_STATE_WIRED) {
						if(pn544_dev->nfc_service_pid){
...
static void secure_timer_workqueue(struct work_struct *Wq)
{
	...
	/* Locking the critical section: ESE_PWR_OFF
	  to allow eSE to shutdown peacefully :: START */
	...
	if((current_state & (P61_STATE_SPI|P61_STATE_SPI_PRIO)) == 0) {
		printk( KERN_INFO "secure_timer_callback:"
			"make se_pwer_gpio low, state = %d", current_state);
		gpio_set_value(pn544_dev->ese_pwr_gpio, 0);
		/* Delay (2.5ms) after SVDD_PWR_OFF
		  for the shutdown settlement time */
		usleep_range(2500, 3000);
		if(pn544_dev->nfc_service_pid == 0x00) {
			gpio_set_value(pn544_dev->ven_gpio, 0);
			printk( KERN_INFO "secure_timer_callback :make ven_gpio low,"
				"state = %d", current_state);
		}
	}
	pn544_dev->secure_timer_cnt = 0;
	/* Locking the critical section: ESE_PWR_OFF
	   to allow eSE to shutdown peacefully :: END */
	release_ese_lock(P61_STATE_WIRED);
	return;
}
...
{
	/* Flush and push the timer callback
	 event to the bottom half(work queue)
	...
	struct pn544_i2c_platform_data *data)
{
	...
	int errorno = 0;
...
#if !NEXUS5x
		data->irq_gpio = of_get_named_gpio(np,
			"nxp,pn544-irq", 0);
		if ((!gpio_is_valid(data->irq_gpio)))
			return -EINVAL;

		data->ven_gpio = of_get_named_gpio(np,
			"nxp,pn544-ven", 0);
		...
		data->firm_gpio = of_get_named_gpio(np,
			"nxp,pn544-fw-dwnld", 0);
		if ((!gpio_is_valid(data->firm_gpio))) {
			errorno =  -EINVAL;
			goto err;
		}

		data->ese_pwr_gpio = of_get_named_gpio(np,
			"nxp,pn544-ese-pwr", 0);
...
#ifdef ISO_RST
		data->iso_rst_gpio = of_get_named_gpio(np,
			"nxp,pn544-iso-pwr-rst", 0);
	   ...
	   data->firm_gpio = of_get_named_gpio_flags(np,
		...
		data->irq_gpio = of_get_named_gpio_flags(np,
			"nxp,gpio_irq", 0, NULL);
#endif
	if (of_property_read_string(np, "nfc,clk-src", &data->clk_src_name))
		data->clk_pin_voting = false;
	else
		data->clk_pin_voting = true;

	data->clkreq_gpio = of_get_named_gpio(np, "nfc,nq-clkreq", 0);

	pr_info("%s: %d, %d, %d, %d, %d error:%d\n", __func__,
		data->irq_gpio, data->ven_gpio,
		data->firm_gpio, data->iso_rst_gpio,
	   ...
	   const struct i2c_device_id *id)
{
	...
	pn544_dev->clk_pin_voting = platform_data->clk_pin_voting;
	...
	* hardware info is required during probe*/
	check_hw_info();
#endif
	printk(KERN_ERR "%s exit nfc i2c probe sucessfull\n", __func__);
	return 0;
	...
	if (ret == 4) {
		...
		* */
		...
		ret = i2c_master_send(pn544_dev->client,
				get_version_cmd, get_version_len);
