	...
	if (GG_Ram.reg.GG_Status != GG_RUNNING) {
		...
		if (BattData.Vmode == 0) {
			...
			if (BattData.AvgCurrent < 0 &&
			   ...
			   BattData.SOC < 20 &&
			   BattData.Voltage >
			   APP_MIN_VOLTAGE+APP_MIN_THRESHOLD) {
				if (g_debug)
					pr_err("Lately empty compen:SOC=%d\n",
						BattData.SOC);
				BattData.SOC = 20;
...
* 5. Do SOC scale
* - If UI_SOC >= SCALE_SOC, show 100%
* - If UI_SOC < SCALE_SOC, do SOC scaling
*/
#define SCALE_SOC 97
void UI_soc_adjustment(struct stc311x_chip *chip)
{
	...
	/* Scale SOC */
	if (chip->batt_soc >= SCALE_SOC)
		scale_soc = 100;
	else
		scale_soc = DIV_ROUND_CLOSEST(chip->batt_soc * 100, SCALE_SOC);
	...
	case POWER_SUPPLY_STATUS_FULL:
		if (scale_soc > g_ui_soc)
			g_ui_soc++;
		//charger exist but current not enough
		else if (chip->batt_current <= 0) {
			if ((g_ui_soc - scale_soc > 2) ||
				(chip->batt_voltage < APP_MIN_VOLTAGE))
				g_ui_soc--;
		}
		break;
	...
	pr_debug("ST_SOC: %d, scale_soc: %d, UI: %d",
		chip->batt_soc, scale_soc, g_ui_soc);
}
			 ...
			 const struct i2c_device_id *id)
{
	...
	if (g_debug)
		...
		pr_info("SOC=%d, reg_soc=%d, voltage=%d, OCV=%d, temp=%d\n",
			chip->batt_soc, g_reg_soc, chip->batt_voltage, g_ocv,
			chip->Temperature);
