...
#define LOW_BATTERY_COUNT_THRESHOLD  6 //10
...
static const char *charger_name = "charger";
static int low_battery_counter;
...
#define STC3100_BATTERY_FULL 100//95
#define STC311x_DELAY	 3000//500  //1000
#define STC311x_SOC_THRESHOLD 5
#define STC311x_DELAY_LOW_BATT 500
...
int stc3117_get_battery_present(void)
{
	int ret;

	//refer from GasGauge_Task()
	ret = STC311x_Status();
	...
	pr_debug("stc3117_get_battery_present():  %d \n", BattData.BattOnline);
	return BattData.BattOnline;
}

int charger_get_charger_present(void)
{
	union power_supply_propval ret = {0,};
	struct power_supply *charger_psy = power_supply_get_by_name((char *)charger_name);

	pr_debug("charger_get_charger_present() \n");
	if (!charger_psy) {
		...
		charger_psy->get_property(charger_psy,
					POWER_SUPPLY_PROP_STATUS, &ret);

		if(ret.intval == POWER_SUPPLY_STATUS_UNKNOWN)
			return false; //charger not exist
		else
			return true;
	}
...
static struct stc311x_platform_data stc3117_data = {
	.battery_online = stc3117_get_battery_present,
	/* used in stc311x_get_status()*/
	.charger_online = charger_get_charger_present,
	/* used in stc311x_get_status()*/
	.charger_enable = NULL,
...
/*
* if charger doesn't exist, status = discharging 
* if charger exists, status = charging or not charging
*/
static void stc311x_get_status(struct i2c_client *client)
{
	...
	{
		/* refer STC311x_ReadBatteryData, read STC3117 registers 6-7 (current) */
		ret = STC31xx_Read(2, 6, data);
		if (ret < 0)
		{
		...
		/* result in mA */
		BattData.Current = conv(curr, BattData.CurrentFactor);
		
		pr_debug("stc311x_get_status() - current = %d \n", BattData.Current);
	
		if(BattData.Current > 0)
			chip->status = POWER_SUPPLY_STATUS_CHARGING;
		else if(BattData.Current == 0)
			chip->status = POWER_SUPPLY_STATUS_NOT_CHARGING;
		else
			chip->status = POWER_SUPPLY_STATUS_DISCHARGING;

		/* For system UI, when in FULL status, it always shows "plugged in" */
		/* Report FULL only when charger exist and soc = 100 */
	...
	pr_debug("stc311x_get_status(): charger status = %s \n", charge_status[chip->status]);
}
...
static void STC311x_SetParam(void)
{
	...
	for (ii = 0; ii < SOCTAB_SIZE; ii++)
			STC31xx_WriteByte(STC311x_REG_SOCTAB+ii, (BattData.SOCValue[ii]*2));
...
int GasGauge_Task(struct GasGauge_DataTypeDef *GG)
{
	...
	if (GG_Ram.reg.GG_Status != GG_RUNNING) {
		...
		if (GG->AvgCurrent < APP_MIN_CURRENT) {
			...
			/* in minutes */
			value = GG->ChargeValue * 60 / (-GG->AvgCurrent);
			if (value < 0)
				value = 0;
			GG->RemTime = value;
		} else {
...
static void stc311x_work(struct work_struct *work)
{
	...
	/* work around to avoid 0% = 3.1v or 3.2v, default 0% = 3.3v */
	/* If battery voltage < 3.3v, re-check 30 sec and shutdown        */
	if(chip->batt_voltage <= 3300) {
		low_battery_counter++;
		if(low_battery_counter >= LOW_BATTERY_COUNT_THRESHOLD)
			chip->batt_soc = 0;
	}
	else
	...
	if(chip->batt_soc < STC311x_SOC_THRESHOLD) //(chip->batt_voltage < 3320)
		//schedule_delayed_work(&chip->work, 300); //if low battery, detect battery per 3s
		schedule_delayed_work(&chip->work, STC311x_DELAY_LOW_BATT); //if low battery, detect battery per 5s
	else
			 ...
			 const struct i2c_device_id *id)
{
	...
	if (!chip) {
		pr_err("Out of memory to create ");
		pr_err("client structure for stc311x\n");
