...
* if charger exists, status = charging
*/
static void stc311x_get_charging_status(struct stc311x_chip *chip)
{
	int rc, soc;
	union power_supply_propval ret = {0,};
	struct power_supply *charger_psy;
	u8 reg[2], charger_online = 0;

	charger_psy = power_supply_get_by_name("charger");
	if (charger_psy == NULL)
		pr_err("stc311x can't find charger device \n");
	else {
	...
	if (charger_online) {
		...
		/* read STC3117 registers 2-3 (SOC) */
		rc = stc311x_read_raw(chip, 0x02, reg, 2);
		if (rc < 0)
			return;
		...
		chip->chg_status = POWER_SUPPLY_STATUS_DISCHARGING;
}
...
static int stc311x_process_fg_task(struct stc311x_chip *chip)
{
	...
	stc311x_get_charging_status(chip);
	pr_info("stc3117: SOC = %d, voltage = %d, current = %d, status = %d, temp = %d \n", chip->soc /10, chip->voltage_mv, chip->current_ma * -1, chip->chg_status, chip->temperature);
				  ...
				  const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		pr_info("stc311x_set_property: status = %d \n", val->intval);
				...
				const struct i2c_device_id *id)
{
	...
	chip->bms_psy.type = POWER_SUPPLY_TYPE_BATTERY;
	chip->bms_psy.get_property = stc311x_get_property;
	chip->bms_psy.set_property = stc311x_set_property;
