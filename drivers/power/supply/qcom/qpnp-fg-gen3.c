...
#define DEFAULT_TWM_SOC_VALUE	12
...
static void fg_get_ibatt_sensing_cfg(long phase_id, u32 *temp)
{
	if (phase_id >= PHASE_ID_EVT && phase_id <= PHASE_ID_MP) {
		...
		pr_info("%s(%s)\n", ibat_sensing_cfg[*temp],
				device_buildphase[phase_id]);
	} else {
		pr_err("Can't find the phase ID, use default BATFET_SMB\n");
		*temp = SRC_SEL_BATFET_SMB;
	}
}
...
static int __init get_board_id(char *buf)
{
	...
	device_board_id = 0;
	result = kstrtol(buf, 10, &device_board_id);
	return 0;
}
