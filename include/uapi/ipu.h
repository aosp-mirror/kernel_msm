...
#define IPU_ALLOCATE_CMD_QUEUE _IO('i', 1)

/* On success will return 0, otherwise will return -1 with errno set. */
#define IPU_ALLOCATE_RESOURCES _IOW('i', 2, \
		struct ipu_resource_allocate_request)

/* On success will return 0, otherwise will return -1 with errno set. */
#define IPU_RELEASE_RESOURCES _IOW('i', 3, \
		struct ipu_resource_release_request)
 ...
 */
#define IPU_REGISTER_DMA_BUF _IOWR('i', 4, struct ipu_dma_buf_register_req)
 ...
 * and errno will be set.
 */
#define PB_UNREGISTER_DMA_BUF _IOW('i', 5, unsigned int)
